...
2.shufflenetv2.py：官方模型用于转换pth文件到onnx文件

3.shufflenetv2_wock_op_woct.py：NPU下训练模型用于转换pth文件到onnx文件
...
（1） 从Torchvision下载shufflenetv2模型或者指定自己训练好的pth文件路径，通过pth2onnx.py脚本转化为onnx模型
...
（4）./benchmark.x86_64 -model_type=vision -batch_size=16 -device_id=0 -input_text_path=./BinaryImageNet.info -input_width=224 -input_height=224 -om_path=./shufflenetv2_bs16.om -useDvpp=False
