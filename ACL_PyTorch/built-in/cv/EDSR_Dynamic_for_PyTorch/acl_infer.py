        ...
        for i in range(size):
            if not self.dynamic:
                ...
                if des == "in":
                    ...
                    temp_buffer_size = AclNet.gen_data_size(input_size, dtype=ACL_DTYPE.get(self.model_input_data_type[i]))
                elif des == "out":
                    temp_buffer_size = AclNet.gen_data_size(data, dtype=ACL_DTYPE.get(self.model_output_data_type[i]))
    ...
    def _gen_dataset_output_host(self, size, des, data=None):
        ...
        for i in range(size):
            if not self.dynamic:
                ...
                temp_buffer_size = AclNet.gen_data_size(data, ACL_DTYPE.get(self.model_output_data_type[i]))
