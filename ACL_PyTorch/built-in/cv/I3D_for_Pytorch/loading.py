...
class RawFrameDecode:
    ...
    def __check_file_exist(self, filepath):
        import os
        if os.path.exists(filepath):
            return filepath

        file_path_list = filepath.rsplit('/', 1)
        file_list = file_path_list[1].split('.')[0].split('_')
        id = int(file_list[1]) - 1
        if id < 10:
            file_name = '0000' + str(id) + '.jpg'
        elif id < 100:
            file_name = '000' + str(id) + '.jpg'
        elif id < 1000:
            file_name = '00' + str(id) + '.jpg'
        else:
            file_name = '0' + str(id) + '.jpg'
        filepath = file_path_list[0] + '/' + 'img_' + file_name
        return filepath
    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                filepath = self.__check_file_exist(filepath)
