...
2. 数据预处理，将原始数据集转换为模型输入的数据。
	 ...
	 运行preprocess.py脚本，完成数据预处理。
   ```shell
   python3 preprocess.py --image_folder_path ./data/coco/val2017 --bin_folder_path input_data
   ```
    - 参数说明：
...
1. 模型转换。
   ...
   1. 获取权重文件。
       ...
       [Retinanet_r50_fpn_1x权重下载路径](https://download.openmmlab.com/mmdetection/v2.0/retinanet/retinanet_r50_fpn_1x_coco/retinanet_r50_fpn_1x_coco_20200130-c2398f9e.pth)
   ...
   2. 导出onnx文件。
      ...
      1. 使用pytorch2onnx.py导出onnx文件。
         ...
         ```shell
         python mmdetection/tools/deployment/pytorch2onnx.py mmdetection/configs/retinanet/retinanet_r50_fpn_1x_coco.py retinanet_r50_fpn_1x_coco_20200130-c2398f9e.pth --shape 1216 --output-file retinanet.onnx
         ```
          - 参数说明：
            - 第一个参数为配置文件路径。
            - 第二个参数为权重文件路径。
      ...
      2. 优化ONNX文件。
         ...
         ```
         python3 -m auto_optimizer optimize retinanet.onnx retinanet_new.onnx -k 4,8
         ```

         获得retinanet_new.onnx文件。
...
2. 开始推理验证。
   ...
   3. 精度验证。
      - 运行get_info.py脚本，生成图片数据info文件。
         ...
         ```shell
         python get_info.py jpg ./data/coco/val2017 val2017.info
         ```
      ...
      - 执行后处理脚本，计算 mAP 精度：
         ...
         注：如果报错'COCO' object has no attribute 'get_cat_ids'，请运行以下命令安装pycocotools：
         ```shell
   ...
   4. 性能验证。
      ...
      ```shell
      python -m ais_bench --model retinanet.om --loop 20
      ```
...
| --------- | ---------------- | ---------- | ---------- | --------------- |
|  310P3    |         1        |   COCO2017 | mAP：36.3%  |     15.48      |
该模型仅支持batch size为1
