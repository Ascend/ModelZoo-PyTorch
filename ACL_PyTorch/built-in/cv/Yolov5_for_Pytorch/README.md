...
```
代码示例导出为静态shape模型，若增加--dynamic参数，导出动态shape模型。
...
--input-shape images:1,3,640,640 该参数为缺省值，若onnxsim不支持动态shape模型简化，则使用该值导出静态shape模型
...
--anchors：训练数据集聚类得到的anchors，不同数据集聚类得到的anchors不同，需根据实际数据集修改，默认使用yolo提供的anchors，参见yolo官方代码的[yaml文件](https://github.com/ultralytics/yolov5/blob/master/models/yolov5s.yaml)  
