...
def img2bin(input_path: str, output_path: str):
...
if __name__ == "__main__":
    ...
    for l in images:
        temp = os.listdir(os.path.join(input_img_dir, l))
        for image_name in temp:
            if not image_name.lower().endswith(".jpeg"):
                continue
            print("start to process image {}....".format(image_name))
            path_image = os.path.join(input_img_dir, l, image_name)
            img2bin(path_image, output_img_dir)
