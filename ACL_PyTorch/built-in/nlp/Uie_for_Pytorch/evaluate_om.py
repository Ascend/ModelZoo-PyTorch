...
def evaluate(model, metric, data_loader, multilingual=False):
    ...
    for batch in data_loader:
        ...
        pad_size = args.batch_size - size
        if multilingual:
            start_prob, end_prob = model(batch["input_ids"], batch["position_ids"])
        else:
            batch['input_ids'] = np.pad(batch['input_ids'].cpu().numpy(), [(0, pad_size), (0, 0)])
            batch['token_type_ids'] = np.pad(batch['token_type_ids'].cpu().numpy(), [(0, pad_size), (0, 0)])
            batch['position_ids'] = np.pad(batch['position_ids'].cpu().numpy(), [(0, pad_size), (0, 0)])
            batch['attention_mask'] = np.pad(batch['attention_mask'].cpu().numpy(), [(0, pad_size), (0, 0)])
            start_prob, end_prob = model.infer(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size per NPU for training.")
    parser.add_argument("--max_seq_len", type=int, default=512, help="The maximum total input \
                        sequence length after tokenization.")
    parser.add_argument("--debug", action='store_true', help="Precision, recall and F1 score are \
                        calculated for each class separately if this option is enabled.")
    parser.add_argument("--multilingual", action='store_true', help="Whether is the multilingual model.")
    parser.add_argument("--schema_lang", choices=["ch", "en"], default="ch", help="Select the \
                        language type for schema.")
    parser.add_argument("--om_path", type=str, help="The path of om model that you want to load.")
    parser.add_argument("--device_id", type=int, default=0, help="The device id of npu that you want to use.")
