...
def postprocess(result_dir, gt_dir, batch_size, seq_length, num_datas):
    all_logits = []
    all_labels = []
    for step in tqdm(range(num_datas)):
        data_path = os.path.join(result_dir, "{}_0.bin".format(step))
        label_path = os.path.join(gt_dir, "{}.bin".format(step))
        logits = load_data(data_path, [batch_size, seq_length])
