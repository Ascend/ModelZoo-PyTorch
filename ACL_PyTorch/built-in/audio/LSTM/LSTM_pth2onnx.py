...
def run_epoch(epoch_id, model, data_iter, loss_fn, device, opts, sum_writer, optimizer=None, print_every=20, is_training=True):
    if is_training:
        model.train()
    else:
        model.eval()
    ...
    for i, data in enumerate(data_iter):
        ...
        with torch.autograd.profiler.profile(record_shapes=True, use_cuda=True) as prof:
            ...
            print("input_sizes:", input_sizes.shape)
            print("target_sizes:", target_sizes.shape)
        ...
        if is_training:
            print('Epoch: [%d] [%d / %d], Time %.6f Data %.6f s, total_loss = %.5f s, total_wer = %.5f' % (epoch_id,
                                    i+1, steps_per_epoch, batch_time / (i+1), data_time / (i+1), total_loss / (i+1), total_errs / total_tokens ))
        end = time.time()
