...
def performance(model, input_shape=None, input_data=None, dynamic_shape=False, dynamic_dims=False, dims=None,
                device_id=0, loop=10, batch_size=1, output_data_shape=None):
    if loop == 0:
    ...
    if dynamic_shape:
        model = AclNet(model_path=model, device_id=device_id, input_data_shape=None, dynamic=dynamic_shape,
    ...
    if len(input_data) > 0:
        ...
        for key in model.model_input_shape.keys():
            ...
            now_input_data = np.fromfile(input_data[key],
                                         dtype=NP_DTYPE[model.model_input_data_type[idx]])  # static model
        ...
        for key2 in model.model_input_shape.keys():
            if not dynamic_dims and len(dims) == 0 and not dynamic_shape:
                tmp = np.random.random(model.model_input_shape[key2]).astype(
                    ACL_DTYPE[model.model_input_data_type[idx]])  # static model
                _input_data.append(tmp)
                idx += 1
            elif dynamic_shape:
                if key2 not in input_shape.keys():
                    raise RuntimeError('the model has not input named ' + key2)
                tmp = np.random.random(input_shape[key2]).astype(ACL_DTYPE[model.model_input_data_type[idx]])
                _input_data.append(tmp)
                idx += 1
...
def check_input_type(input_type, model_input_type):
    for i in range(len(input_type)):
        if ACL_DTYPE_INDEX.get(input_type[i]) != model_input_type[i]:
            raise Exception(
                "the input {} input data type is {}, actual need model input type is {}".format(i, input_type[i],
                                                                                                ACL_DTYPE.get(
                                                                                                    model_input_type[
                                                                                                        i])))
...
class AclNet(object):
    def __init__(self, model_path, device_id, check_input=False, input_data_shape=None, output_data_shape=None,
                 dynamic=None, auto_malloc=True):
    ...
    def _data_interaction(self, dataset, policy=ACL_MEMCPY_HOST_TO_DEVICE, output_shape=None):
    ...
    def _data_from_host_to_device(self, images, input_shapes):
    ...
    def forward(self, input_data, dims=None):
        ...
        self._data_from_host_to_device(input_data, input_shapes)
    ...
    def get_result(self, output_data, data, output_shape):
        ...
        for i in range(len(output_data)):
            ...
            np_array = np.frombuffer(
                bytearray(data), dtype=ftype, count=data_len)
