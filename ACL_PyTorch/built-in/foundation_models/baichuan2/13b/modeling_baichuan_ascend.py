...
class BaichuanModel(BaichuanPreTrainedModel):
    ...
    def init_ascend_operations(self, config: BaichuanConfig):
        self.acl_param = json.dumps({"headNum": config.num_attention_heads,
                                     "rmsNormEps": config.rms_norm_eps,
                                     "dk": config.hidden_size // config.num_attention_heads,
                                     "layerNum": config.num_hidden_layers})

        self.acl_encoder_operation = torch.classes.ModelTorch.ModelTorch("BaiChuan213BEncoderModel")
        self.acl_decoder_operation = torch.classes.ModelTorch.ModelTorch("BaiChuan213BDecoderModel")
        ...
        self.acl_encoder_operation_inputs = [None] * 3
        self.acl_decoder_operation_inputs = [None] * (2 + 2*self.num_layers)
    ...
    def prepare_inputs_for_ascend(self, input_ids, seq_length, batch_size, attention_mask=None, past_key_values=None):
        if not past_key_values:
            ...
            if len(attention_mask.size()) == 4:
                self.acl_encoder_operation_inputs[1] = attention_mask
            else:
                self.acl_encoder_operation_inputs[1] = attention_mask.unsqueeze(0)
            self.acl_encoder_operation_inputs[2] = self.seqlen_tensor
            return self.acl_encoder_operation_inputs, self.acl_param
        else:
            past_keys, past_values = map(list, zip(*past_key_values))
            self.acl_decoder_operation_inputs[0] = input_ids
            if len(attention_mask.size()) == 4:
                self.acl_decoder_operation_inputs[1] = attention_mask[:, :, -1:, :]
            else:
                self.acl_decoder_operation_inputs[1] = attention_mask[:, -1:, :].unsqueeze(0)
            self.acl_decoder_operation_inputs[2: 2 + self.num_layers] = past_keys
            self.acl_decoder_operation_inputs[2+self.num_layers: 2+2*self.num_layers] = past_values
            return self.acl_decoder_operation_inputs, self.acl_param

    def execute_ascend_operator(self, acl_model, input_ids, seq_length_with_past, attention_mask=None, past_key_values=None):
        batch_size, seq_length = input_ids.shape
        acl_inputs, acl_param = self.prepare_inputs_for_ascend(input_ids, seq_length_with_past, batch_size,
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    ...
    def chat(self, tokenizer, messages: List[dict], stream=False, device_id: Optional[int] = 0,
