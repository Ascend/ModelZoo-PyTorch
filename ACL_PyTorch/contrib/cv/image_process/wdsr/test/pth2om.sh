...
rm -rf Wdsr_bs1.om Wdsr_bs4.om Wdsr_bs8.om
...
atc --framework=5 --model=wdsr.onnx --output=wdsr_bs4 --input_format=NCHW --input_shape="image:4,3,1020,1020" --log=debug --soc_version=Ascend310
...
if [ -f "wdsr_bs1.om" ] && [ -f "wdsr_bs8.om" ] && [ -f "wdsr_bs4.om" ]; then
    echo "success"
else
    echo "fail!"
fi
