...
def get_bin_info(file_path, info_name, width, height):
    """generate bin information of images

    Args:
        file_path (str): image file path
        info_name (str): bin file name
...
def get_jpg_info(file_path, info_name):
    """get information of jpg images

    Args:
        file_path (str): path of image files
        info_name (str): name of information file
    """
        ...
        for image_name in image_names:
            if len(image_name) == 0:
                ...
                for index, img in enumerate(image_name):
                    ...
                    content = ' '.join(
                        [str(index), img, str(width), str(height)])
                    file.write(content)
                    file.write('\n')
...
if __name__ == '__main__':
    ...
    if file_type == 'bin':
        ...
        height = sys.argv[5]
        assert len(
            sys.argv) == 6, 'The number of input parameters must be equal to 5'
        get_bin_info(file_path, info_name, width, height)
    elif file_type == 'jpg':
        assert len(
            sys.argv) == 4, 'The number of input parameters must be equal to 3'
