...
def gen_input_bin(mode_type, file_batches, batch):
    ...
    for file in file_batches[batch]:
        ...
        img -= np.array(model_config[mode_type]['mean'], dtype=np.float16)[:, None, None] # Normalize: mean
        img /= np.array(model_config[mode_type]['std'], dtype=np.float16)[:, None, None] # Normalize: std
