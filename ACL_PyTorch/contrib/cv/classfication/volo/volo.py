...
def fold(input, output_size, kernel_size, dilation=1, padding=0, stride=1):
    return col2im_op(input, _pair(output_size), _pair(kernel_size), _pair(dilation), _pair(padding), _pair(stride))
    

def col2im_op(x, output_size, kernel_size, dilation, padding, stride):
    out = Col2ImOp.apply(x, output_size, kernel_size, dilation, padding, stride)
    return out

class Col2ImOp(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, output_size, kernel_size, dilation, padding, stride):
        out = torch.randn(x.shape[0], x.shape[1], output_size[0], output_size[1]).to(x.dtype)
        return out
    @staticmethod
    def symbolic(g, x, output_size, kernel_size, dilation, padding, stride):
        out = g.op('Col2im', x, output_size_i = output_size, kernel_size_i = kernel_size,  dilation_i = dilation, padding_i = padding, stride_i = stride)
        return out
...
class OutlookAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (attn @ v).permute(0, 1, 4, 3, 2).reshape(
            B, C, self.kernel_size * self.kernel_size , h * w)
        x = fold(x, output_size=(H, W), kernel_size=self.kernel_size,
