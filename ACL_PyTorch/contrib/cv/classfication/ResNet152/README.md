...
1. 模型转换。
   ...
   1. 获取权重文件。
      ...
      从PyTorch开源框架获取经过训练的Resnet152权重文件```resnet152-394f9c45.pth```
      ```bash
   ...
   2. 导出onnx文件。
      ...
      将.pth文件转换为.onnx文件，执行如下命令在当前目录生成```resnet152.onnx```模型文件。
      
      ```bash
      python3 resnet152_pth2onnx.py --checkpoint ./resnet152-394f9c45.pth --save_dir ./resnet152.onnx
      ```
      参数说明：
   ...
   3. 使用ATC工具将ONNX模型转OM模型。
      ...
      2. 使用atc将onnx模型转换为om模型文件。
         ...
         atc --framework=5 --model=./resnet152.onnx --output=resnet152_bs${bs} --input_format=NCHW --input_shape="image:${bs},3,224,224" --log=debug --soc_version=${chip_name} --insert_op_conf=aipp.config --enable_small_channel=1
         ```
...
2. 开始推理验证。
   ...
   ```bash
   python3 -m ais_bench --model ./resnet152_bs${bs}.om --input ./prep_dataset/ --output ./result/ --output_dir resnet152_bs${bs} --outfmt TXT
   ```
   ...
   运行成功后会在result/resnet152_bs${bs}下生成推理输出的txt文件。
   ...
   ```bash
   python3 imagenet_acc_eval.py result/resnet152_bs${bs} ./ImageNet/val_label.txt ./ result.json
   ```
...
3. 执行纯推理验证性能。
   ```bash
   python3 -m ais_bench --model resnet152_bs${bs}.om --device 1 --loop 100
   ```
   - 参数说明：
      - --model：om文件路径
      - --device：NPU设备编号
...
官方的参考精度为：Top1: 78.312%， Top5: 94.046%
