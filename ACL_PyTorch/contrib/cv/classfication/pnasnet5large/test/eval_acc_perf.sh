...
./benchmark.${arch} -model_type=vision -device_id=0 -batch_size=1 -om_path=pnasnet5large_bs1.om -input_text_path=./pnasnet5large_prep_bin.info -input_width=331 -input_height=331 -output_binary=False -useDvpp=False
if [ $? != 0 ]; then
    echo "fail!"
    exit -1
fi
rm -rf result/dumpOutput_device1
./benchmark.${arch} -model_type=vision -device_id=1 -batch_size=16 -om_path=pnasnet5large_bs16.om -input_text_path=./pnasnet5large_prep_bin.info -input_width=331 -input_height=331 -output_binary=False -useDvpp=False
if [ $? != 0 ]; then
    echo "fail!"
    exit -1
fi
