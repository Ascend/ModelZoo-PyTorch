...
# onnx2om
source env.sh
rm -rf deit_small_bs1.om deit_small_bs16.om
...
atc --framework=5 --model=./deit_small_patch16_224_bs16_onnxsim.onnx --input_format=NCHW --input_shape="image:16,3,224,224" --output=deit_small_bs16 --log=debug --soc_version=Ascend310

if [ -f "deit_small_bs1.om" ] && [ -f "deit_small_bs16.om" ]; then
    echo "success"
else
    echo "fail!"
fi