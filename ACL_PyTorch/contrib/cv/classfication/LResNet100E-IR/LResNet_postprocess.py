...
def evaluation_om(result_dir, target_list_path):
    ...
    result_ids = next(os.walk(result_dir))[2]
    n = len(result_ids)//2

    embeddings = np.zeros([n, 512], dtype=np.float32)
    for idx in tqdm(range(n)):
        result_path = os.path.join(result_dir, f'{idx}_1.txt')
        result_flip_path = os.path.join(result_dir, f'{idx}_flip_1.txt')
...
if __name__ == '__main__':
    evaluation_om(sys.argv[1], sys.argv[2])
