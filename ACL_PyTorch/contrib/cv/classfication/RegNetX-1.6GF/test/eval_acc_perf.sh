...
./benchmark.${arch} -model_type=vision -device_id=0 -batch_size=1 -om_path=RegNetX-1.6GF_bs1.om -input_text_path=./RegNetX-1.6GF_prep_bin.info -input_width=224 -input_height=224 -output_binary=False -useDvpp=False
if [ $? != 0 ]; then
    echo "fail!"
    exit -1
fi
rm -rf result/dumpOutput_device1
./benchmark.${arch} -model_type=vision -device_id=1 -batch_size=16 -om_path=RegNetX-1.6GF_bs16.om -input_text_path=./RegNetX-1.6GF_prep_bin.info -input_width=224 -input_height=224 -output_binary=False -useDvpp=False
if [ $? != 0 ]; then
    echo "fail!"
    exit -1
fi
