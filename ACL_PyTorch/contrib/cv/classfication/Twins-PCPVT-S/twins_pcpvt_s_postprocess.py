...
def create_visualization_statistical_result(prediction_file_path,
    ...
    for tfile_name in os.listdir(prediction_file_path): #举例：ILSVRC2012_val_00000001_output_0.txt
        if (tfile_name == 'summary.json'):
            continue
        count += 1
        temp = tfile_name.split('.')[0]                 #temp = ILSVRC2012_val_00000001_output_0
        index = temp.rfind('_', 0, 26)                         #index = 30
        img_name = temp[:index]                         #img_name = ILSVRC2012_val_00000001_output
...
if __name__ == '__main__':
    ...
    sub_folder = os.listdir(folder_davinci_target)
    folder_davinci_target = os.path.join(folder_davinci_target, sub_folder[0])
