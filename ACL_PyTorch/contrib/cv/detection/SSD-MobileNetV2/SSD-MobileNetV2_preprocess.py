...
def gen_input_bin(mode_type, src_path, file_batches, batch, save_path):
    i = 0
...
def preprocess(mode_type, src_path, save_path):
    ...
    for batch in range(len(file_batches)):
        thread_pool.apply_async(gen_input_bin, args=(mode_type, src_path, file_batches, batch, save_path))
    thread_pool.close()
    thread_pool.join()
