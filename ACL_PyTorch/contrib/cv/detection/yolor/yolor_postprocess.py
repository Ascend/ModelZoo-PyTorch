...
def test(data,
    ...
    device = select_device(npu = opt.npu, batch_size=batch_size)
...
#        inf_data = inf_data.reshape(1, 112455, 85)
        ...
        # img_np = np.fromfile(data_path, dtype=np.float32)
        # img_np = img_np.reshape(1, 3, 1344, 1344)
        ...
        with torch.no_grad():
            ...
            # loc = ort_session.run(None, {'image': img_np})
            # inf_out = torch.from_numpy(loc[0])

            inf_out = torch.from_numpy(inf_data).to(device)
            ...
            # Run NMS
            t = time_synchronized()
            output = non_max_suppression_ori(inf_out, conf_thres=conf_thres, iou_thres=iou_thres)
        ...
        #     f = save_dir / f'test_batch{epoch}_{batch_i}_labels.jpg'  # filename
        #     plot_images(img, targets, paths, f, names)  # labels
        #     f = save_dir / f'test_batch{epoch}_{batch_i}_pred.jpg'
