...
import sys
sys.path.append('./BSN-boundary-sensitive-network.pytorch/Evaluation')
...
def plot_metric(average_nr_proposals, average_recall, recall, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    #plt.show()    
    plt.savefig("./output/evaluation_result.jpg")
...
def evaluation_proposal():
    ...
    uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid = run_evaluation(
        "./BSN-boundary-sensitive-network.pytorch/Evaluation/data/activity_net_1_3_new.json",
        "./output/result_proposal.json",
        max_avg_nr_proposals=100,
        tiou_thresholds=np.linspace(0.5, 0.95, 10),
        subset='validation')
    
    plot_metric(uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid)
    print('AR@1 is \t{}'.format(np.mean(uniform_recall_valid[:,0])))
    print('AR@5 is \t{}'.format(np.mean(uniform_recall_valid[:,4])))
    print('AR@10 is \t{}'.format(np.mean(uniform_recall_valid[:,9])))
    print('AR@100 is \t{}'.format(np.mean(uniform_recall_valid[:,-1])))
