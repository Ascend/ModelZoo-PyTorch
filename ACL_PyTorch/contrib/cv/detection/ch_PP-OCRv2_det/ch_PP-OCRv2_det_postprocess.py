...
def main(config, device, logger, vdl_writer):
    ...
    results_path = config['Global']['infer_results']
    ...
    for im_file in get_image_file_list(config['Global']['infer_img']):
        ...
        result = paddle.to_tensor(np.load(os.path.join(results_path, result_name)))
        preds = {
            'Teacher': {"maps": result}
            }
        
        post_preds = post_process_class(preds, shape_list)
        
        src_img = cv2.imread(im_file)
        
        boxes = post_preds['Teacher'][0]['points']
        
        det_results = os.path.join(results_path, 'det_results')
        
        draw_det_res(boxes, config, src_img, im_file, det_results)
