    ...
    for root, sub_dirs, files in os.walk(output_path):
        ...
        for i, output_name in enumerate(files):
            if not output_name.endswith('bin'):
                continue
            print('DeeplabV3 metric [' + str(i + 1) + '/' + len + '] on process: ' + output_name)
            ...
            if seg_map_name is not None:
                ...
                output = np.fromfile(output_path, dtype=np.uint64).reshape(1024, 2048)
