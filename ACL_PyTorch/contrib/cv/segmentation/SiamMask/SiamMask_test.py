...
def siamese_init(im, target_pos, target_sz, anchors, anchor_num, hp=None):
    ...
    p.scales = anchors['scales']
    p.ratios = anchors['ratios']
    p.anchor_num = anchor_num
    p.anchor = generate_anchor(anchors, p.score_size)
    ...
    if mask_enable:
        ...
        if refine_enable:
            ...
            p0 = torch.nn.functional.pad(f0, [16, 16, 16, 16])[:, :, 4 * pos[0]:4 * pos[0] + 61,
                 4 * pos[1]:4 * pos[1] + 61]
            p1 = torch.nn.functional.pad(f1, [8, 8, 8, 8])[:, :, 2 * pos[0]:2 * pos[0] + 31,
                 2 * pos[1]:2 * pos[1] + 31]
...
def track_vot(video, cfg, hp=None, mask_enable=False, refine_enable=False):
    ...
    # save result
    name = 'Custom_mask_refine_SiamMask_VOT'
...
def main():
    ...
    for v_id, video in enumerate(dataset.keys(), start=1):
        ...
        lost, speed = track_vot(dataset[video], cfg, cfg['hp'] if 'hp' in cfg.keys() else None,
