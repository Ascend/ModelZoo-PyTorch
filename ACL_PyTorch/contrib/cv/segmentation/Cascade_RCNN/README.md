...
1. 模型转换。
   ...
   2. 导出onnx文件。
      ...
      1. 使用detectron2/tools/deploy/export_model.py导出onnx文件。
         ...
         --export-method tracing --format onnx MODEL.WEIGHTS model_final_480dd8.pkl MODEL.DEVICE cpu
         ```
         - 参数说明：
            -  --config-file : 配置文件
            -  --output: 输出onnx模型
...
2. 开始推理验证。
   ...
   4. 性能验证。
      ...
      ```
      python3 -m ais_bench --model ./cascadercnn_detectron2_npu.om --loop 100
      ```
