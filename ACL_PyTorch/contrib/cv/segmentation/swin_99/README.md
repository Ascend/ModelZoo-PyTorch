...
+ 参考实现  
    ...
    model_name = upernet_swin_base_patch4_window7_512x512_160k_ade20k_pretrain_224x224_1K
    ```
...
1. PyTroch 模型转 ONNX 模型  
    ...
    step1: 下载pth权重文件  
    本推理项目使用开源仓提供的预训练好的 [权重文件](https://download.openmmlab.com/mmsegmentation/v0.5/swin/upernet_swin_base_patch4_window7_512x512_160k_ade20k_pretrain_224x224_1K/upernet_swin_base_patch4_window7_512x512_160k_ade20k_pretrain_224x224_1K_20210526_192340-593b0e13.pth )，下载完成后将权重 pth 文件存放于 swin99 目录下。
    ```shell
...
2. 离线推理  
    ...
    cd ais_infer
    python3 ais_infer.py --model ../swin99/base_slide_bs1.om --input ../swin99/val_bin/ --output ../swin99/val_bs1_out/ --batchsize 1
    cd ..
    ```
    参数说明：
    + --model: OM模型路径。
    ...
    运行成功后，在 swin99/val_bs1_out/ 下，会生成一个以执行开始时间`%Y_%m_%d-%H_%M_%S`来命名的子目录，每个预处理 bin 文件会对应生成一个推理结果 bin 文件存放在此目录下。
...
4. 性能验证  
    ...
    ```shell
    cd ais_infer
    python3 ais_infer.py --model ../swin99/base_slide_bs1.om --loop 100 --batchsize 1
    cd ..
    ```
    ...
    $$throughput =\frac{batchsize}{mean} * 1000 = 14.22(fps)$$
...
2. 精度对比
    ...
    <tr>
    <td rowspan="2">swin_99</td>
    <td>1</td>
    <td rowspan="2">48.29%</td>
    <td rowspan="2"><a href="https://github.com/open-mmlab/mmsegmentation/tree/v0.28.0/configs/swin#ade20k">47.99%</a></td>
    </tr>
    <tr>
    <td>4</td>
    </tr>
