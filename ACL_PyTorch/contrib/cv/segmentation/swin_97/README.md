...
2. ONNX 模型转 OM 模型  
   ...
   参数说明：
    ...
    命令中的`${bs}`表示模型输入的 batchsize，比如将`${bs}`设为 1，则运行结束后会在 swin97 目录下生成 tiny_slide_bs1.om
...
2. 离线推理  
    ...
    cd ais_infer
    python3 ais_infer.py --model ../swin97/tiny_slide_bs1.om --input ../swin97/val_bin/ --output ../swin97/val_bs1_out/ --batchsize 1
    cd ..
    ```
    参数说明：
    + --model: OM模型路径。
    ...
    运行成功后，在 swin97/val_bs1_out/ 下，会生成一个以执行开始时间`%Y_%m_%d-%H_%M_%S`来命名的子目录，每个预处理 bin 文件会对应生成一个推理结果 bin 文件存放在此目录下。
...
4. 性能验证  
    ...
    ```shell
    cd ais_infer
    python3 ais_infer.py --model ../swin97/tiny_slide_bs1.om --loop 100 --batchsize 1
    cd ..
    ```
