...
2. 开始推理验证。
   ...
   2. 执行推理(${ais_infer_path}请根据实际的推理工具路径填写)。
      ...
      ```
      mkdir result
      python3 ${ais_infer_path}/ais_infer.py --model=./yolact_bs1.om --input=./prep_dataset --output=./result --output_dirname=bs1 --outfmt=BIN --batchsize=1 --device=0
      ```
   ...
   4. 性能验证。
      ...
      ```
      python3 ${ais_infer_path}/ais_infer.py --model=${om_model_path} --loop=20 --batchsize=${batch_size} --device=${device_id}
      ```
