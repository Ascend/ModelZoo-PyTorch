...
2. 数据预处理，将原始数据集转换为模型输入的数据。
   ...
   ```
   python3 preprocess.py VoxCeleb1 input_bs4/ speaker_bs4/ 4 
   ```
...
1. 模型转换。
   ...
   1. 获取权重文件。
      ...
      ```
      python3 get_originroc.py checkpoint VoxCeleb1 4
      ```
...
2. 开始推理验证。
   ...
   3. 执行推理。
        ...
        ```
        python3 -m ais_bench --model "om/ecapa_tdnn_bs4.om" --input "input_bs4/" --output "result" --outfmt TXT  
        ```
   ...
   4. 精度验证。
      ...
      根据第四步中获取的结果result/${output_dir}和第三步中产生的speaker_bs4标签文件，得到推理精度。

      ```
      python3 postprocess.py result/${output_dir} speaker_bs4 4 4648
      ```

      - 参数说明：

        - result/${output_dir}：为推理结果所在路径
