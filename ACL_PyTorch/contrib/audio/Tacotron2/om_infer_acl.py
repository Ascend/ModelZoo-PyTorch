...
class Tacotron2():
    ...
    def infer(self, batch_size, sequences, sequence_lengths, max_decode_iter):
        ...
        decoder_input = aclruntime.Tensor(np.zeros((batch_size, 80), dtype=np.float32))
        decoder_input.to_device(self.device_id)
        attention_hidden = aclruntime.Tensor(np.zeros((batch_size, 1024), dtype=np.float32))
        attention_hidden.to_device(self.device_id)
        attention_cell = aclruntime.Tensor(np.zeros((batch_size, 1024), dtype=np.float32))
        attention_cell.to_device(self.device_id)
        decoder_hidden = aclruntime.Tensor(np.zeros((batch_size, 1024), dtype=np.float32))
        decoder_hidden.to_device(self.device_id)
        decoder_cell = aclruntime.Tensor(np.zeros((batch_size, 1024), dtype=np.float32))
        decoder_cell.to_device(self.device_id)
        attention_weights = aclruntime.Tensor(np.zeros((batch_size, sequence_lengths[0]), dtype=np.float32))
        attention_weights.to_device(self.device_id)
        attention_weights_cum = aclruntime.Tensor(np.zeros((batch_size, sequence_lengths[0]), dtype=np.float32))
        attention_weights_cum.to_device(self.device_id)
        attention_context = aclruntime.Tensor(np.zeros((batch_size, 512), dtype=np.float32))
        ...
        mel_output_input = aclruntime.Tensor(np.zeros((batch_size, 80, 1), dtype=np.float32))
        mel_output_input.to_device(self.device_id)
        gate_output_input = aclruntime.Tensor(np.zeros((batch_size, 1, 1), dtype=np.float32))
