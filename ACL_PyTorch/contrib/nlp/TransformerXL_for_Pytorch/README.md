...
2. 模型推理
   ...
   利用ais_bench推理工具进行推理，ais_bench推理工具安装参考该[链接](https://gitee.com/ascend/tools/tree/master/ais-bench_workload/tool/ais_bench)。由于该模型的特殊性，无法直接使用ais_infer.py文件进行推理，故调用aclruntime的接口进行推理验证。
