...
from .onnx_node import NULL_TYPE
...
def _add_obj_to_dict(key, obj, res):
    if key in res:
        if isinstance(obj, OXNode) and obj.op_type == INPUT_TYPE:
            mlog("ignore input node, name:{} already exist".format(obj.name),
...
def _add_name_if_null(cur_ox_node: OXNode):
    if cur_ox_node.name == "":
        cur_ox_node.set_name(f"{cur_ox_node.op_type}_{cur_ox_node.out_name[0]}")
...
class OXGraph(BaseGraph):
    def __init__(self, path: str):
        ...
        self.dummy_node = OXNode(NodeProto(op_type=NULL_TYPE, name="NULL"))
        for node in self.mod.graph.node:
            cur_ox_node = OXNode(node)
            _add_name_if_null(cur_ox_node)
        ...
        for node in self.mod.graph.initializer:
            ...
            _add_name_if_null(cur_ox_node)
            _add_obj_to_dict(node.name, cur_ox_node, self.name_node_map)
        for node in self.mod.graph.input:
            cur_ox_node = OXNode(node)
            _add_name_if_null(cur_ox_node)
            _add_obj_to_dict(node.name, cur_ox_node, self.name_node_map)
    ...
    def get_net_edge(self) -> Dict[str, List[Set[Tuple]]]:
        """
        key is node name, value is list of out connect's node names
        the list's length equal to node output length
        """
        res = {}
        for node in self.mod.graph.node:
            cur_out_name = node.name
            for in_idx, in_name in enumerate(node.input):
                cur_in_node = self.get_node(in_name)
                out_idx = cur_in_node.out_name.index(in_name)
                cur_in_name = cur_in_node.name
                if cur_in_name not in res:
                    res[cur_in_name] = [set()]
                while len(res[cur_in_name]) <= out_idx:
                    res[cur_in_name].append(set())

                res[cur_in_name][out_idx].add((cur_out_name, in_idx))
        return res
    ...
    def node_remove(self, nodes_name: Union[List, Tuple, Set]):
        ...
        for node in nodes_name:
            ...
            if ox_node.op_type == INPUT_TYPE:
                ...
                self.remove_names.add(ox_node.name)
            elif ox_node.op_type == NULL_TYPE:
                continue
            else:
    ...
    def node_add_forward(self, node_original: Union[str, OXNode],
        ...
        node_ori.set_input_node(0, [node_add for _ in range(0, ori_in_num)],
    ...
    def node_add_behind(self, node_original: Union[str, OXNode],
        ...
        node_add.set_input_node(0, [node_ori for _ in range(0, ori_out_num)],
    ...
    @staticmethod
    def _update_io_map_by_output(node: OXNode, io_map: dict, src_out):
