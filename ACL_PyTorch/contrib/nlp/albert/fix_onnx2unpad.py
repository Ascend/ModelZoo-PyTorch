...
def modify_self_attention(graph, input_node, layer):
    ...
    for node in nodes4:
        ...
        if node.op_type == "MatMul":
            ...
            if "query" in bias.inputs[1]:
                weight_bias["query"] = [node.inputs[1], bias.inputs[1]]
            elif "key" in bias.inputs[1]:
                weight_bias["key"] = [node.inputs[1], bias.inputs[1]]
            elif "value" in bias.inputs[1]:
                weight_bias["value"] = [node.inputs[1], bias.inputs[1]]
    ...
    # dense
    dense_bias_name = dense_bias_node.inputs[1]
    dense_weight_name = graph.get_prev_node(dense_bias_node.inputs[0]).inputs[1]
