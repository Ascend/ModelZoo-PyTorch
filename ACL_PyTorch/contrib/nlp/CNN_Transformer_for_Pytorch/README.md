...
2. 开始推理验证。
   ...
   3. 执行推理。
        ...
        ```
        python3 ${path_to_ais-infer}/ais_infer.py --model ./models/wav2vec2-base-960h.om --input ./pre_data/validation/ --auto_set_dymdims_mode 1 --output ${out_dir}  
        ```
        ...
        -   参数说明：
         ...
         `${path_to_ais-infer}`为ais_infer.py脚本的存放路径，`${out_dir}`为推理输出文件夹
   ...
   3. 精度验证。
      ...
      - 参数说明：
        - --bin_file_path：ais-infer工具推理结果存放路径；
        - --res_save_path：后处理结果存放txt文件;
        - --which_dataset：精度统计所用的数据集，参看CNN_Transformer_Preprocess.py的参数说明；
      
      注：--bin_file_path指定的result_bs16/2022_09_01-18_51_23/路径不是固定，具体路径为ais-infer工具推理命令中，--output指定目录下的生成推理结果所在路径

      精度验证完成后，在--res_save_path指定目录下生成后处理结果存放的txt文件，同时在txt文件的同级目录下生成精度统计文件wer.txt 

   4. 性能验证。

      由于TensorRT无法运行wav2vec2-base-960h.onnx模型，所以性能测试以ais-infer工具得到的om推理性能和pytorch在线推理性能作比较。
