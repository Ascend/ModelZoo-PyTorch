...
do
  ckpt_path=$(grep 'save checkpoint path : ' ${output_path}/train_${ASCEND_DEVICE_ID}.log | head -$seed_i | awk 'END {print $NF}')
  echo $ckpt_path
  ...
  do
    taskset -c $PID_START-$PID_END python3 -u test.py \
      ...
      --random-seed=${seed_j} \
      --ckpt-path=${ckpt_path} \
      >>${output_path}/test_${ASCEND_DEVICE_ID}.log 2>&1
    wait
  done
done  
...
# 输出训练精度,需要模型审视修改
train_accuracy=$(grep "average test reward : " ${output_path}/test_${ASCEND_DEVICE_ID}.log | awk -F ':' '{print $2}' | awk '{sum+=$1} END {print sum/NR}')
