    ...
    **Available attributes:**
    ...
    def _prepare_one(self, obj, first_pass=False, device_placement=None):
        ...
        if first_pass:
            if isinstance(obj, torch.utils.data.DataLoader):
                ...
                optimizer = self.prepare_optimizer_oss(obj, device_placement=device_placement)
                return optimizer
    ...
    def prepare(self, *args, device_placement=None):
        ...
        if self.distributed_type == DistributedType.DEEPSPEED:
            ...
            if len(args) == 2 and isinstance(args[0], torch.nn.Module) and isinstance(args[1], torch.optim.Optimizer):
                pre_optimizer = self.prepare_optimizer_oss(args[1], device_placement=device_placement[1])
                pre_model = self.prepare_model(args[0], device_placement=device_placement[0], optimizer=pre_optimizer)
                result = tuple((pre_model, pre_optimizer))
            else:
                result = tuple(
    ...
    def prepare_model(self, model: torch.nn.Module, device_placement: bool = None, evaluation_mode: bool = False, optimizer=None):
        """
    ...
    def prepare_optimizer_oss(self, optimizer: torch.optim.Optimizer, device_placement=None):
        from .optimizer import move_to_device
        if device_placement is None:
            device_placement = self.device_placement

        if device_placement:
            state_dict = optimizer.state_dict(all_ranks=True)
            state_dict = move_to_device(state_dict, self.device)
