...
def _set_npu_rng_state(new_state, device=-1):
    """Sets the random number generator state of the current GPU.

    Argumentss:
        new_state (torch.ByteTensor): The desired state
    This function is adapted from PyTorch repo (torch.npu.set_rng_state)
    with a single change: the input state is not cloned. Cloning caused
    major performance issues for +4 GPU cases.
    """
    if hasattr(_C, '_npu_setRNGState') and callable(_C._npu_setRNGState):
        # older PyTorch
        def cb():
            with device_ctx_manager(device):
                _C._npu_setRNGState(new_state)
    else:
        # newer PyTorch
        if device == -1:
...
class CudaRNGStatesTracker:
    ...
    def add(self, name, seed):
        ...
        _set_npu_rng_state(orig_rng_state)

    @contextlib.contextmanager
    def fork(self, name=_MODEL_PARALLEL_RNG_TRACKER_NAME):
        """Fork the npu rng state, perform operations, and exit with
        the original state."""
        # Check if we have added the state
        if name not in self.states_:
            raise Exception('npu rng state {} is not added'.format(name))
        # Store current rng state.
        orig_npu_rng_state = torch.get_rng_state()
        # Set rng state to the desired one
        _set_npu_rng_state(self.states_[name])
