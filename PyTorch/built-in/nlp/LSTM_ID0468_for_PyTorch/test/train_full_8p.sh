...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
fps=`grep "Epoch:" $test_path_dir/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $11}'|sed 's/,$//'`
FPS=`awk 'BEGIN{printf "%.2f\n",'${fps}'*8}'`
#打印，不需要修改
echo "Final Performance item/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep "acc is:" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $10}'`
#打印，不需要修改
