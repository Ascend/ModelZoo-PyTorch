...
class NpuFusedBertAdamV2(NpuFusedBertAdam):
    def _group_step(self, group_index):
        ...
        for combined_param, combined_grad, combined_param_state in zip(combined_group_params, combined_group_grads,
            ...
            if torch.__version__ >= "1.8":
                combined_param.data, exp_avg, exp_avg_sq = torch_npu.npu_bert_apply_adam(lr_scheduled, beta1, beta2,
                ...
                combined_param.data, exp_avg, exp_avg_sq = torch_npu.npu_bert_apply_adam(combined_param.data, exp_avg,
...
def main():
    ...
    parser.add_argument('--use_npu',
                        default=True,
                        action='store_true',
                        help='whether to use npu')
    parser.add_argument('--npu_id',
                        type=int, default=0,
                        help='npu device id.')
    parser.add_argument('--num_npu',
                        type=int, default=1,
                        help='number of npu devices to use.')
    ...
    if args.local_rank != -1 and (os.getenv("ALLOW_FP32") or os.getenv("ALLOW_HF32")):
    ...
    if args.do_train:
        ...
        all_segment_ids = torch.tensor([f.segment_ids for f in train_features], dtype=torch.int32)
        all_start_positions = torch.tensor([f.start_position for f in train_features], dtype=torch.int32)
        all_end_positions = torch.tensor([f.end_position for f in train_features], dtype=torch.int32)
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size * n_npu,
                                      num_workers=2, pin_memory=True, drop_last=True, **kwargs)
