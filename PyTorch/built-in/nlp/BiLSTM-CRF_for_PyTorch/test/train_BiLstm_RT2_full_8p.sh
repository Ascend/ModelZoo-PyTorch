...
# 训练开始时间
start_time=$(date +%s)
let distributed_bs=${batch_size}/${RANK_SIZE}
...
# 输出训练精度
train_accuracy=$(grep "avg/total" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F " " '{print $2}' | awk 'END{print $1}')
