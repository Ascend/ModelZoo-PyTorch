...
def _stable_argsort(vector, dim):
    ...
    scaled_vector = scaled_vector.float()
...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        """
        b, size, dim = idxs.shape
        dim1_idx = torch.arange(b * size, device=vectors.device) * dim
        flat_idxs = idxs.reshape(-1, dim)
        flat_idxs = (flat_idxs + dim1_idx[:, None]).flatten()
        vectors = vectors.reshape(-1, vectors.shape[-1])
        vectors = vectors.index_select(0, flat_idxs)
        return vectors.reshape(b, size, dim, -1)
