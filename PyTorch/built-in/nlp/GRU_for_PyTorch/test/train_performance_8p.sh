...
then
    ...
    let p_start=0+20*i
    let p_end=19+20*i
    taskset -c $p_start-$p_end python3 ${cur_path}/../gru_8p.py \
        ...
        --amp  > $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
    done
else
    for i in $(seq 0 7)
    do
...
grep Epoch $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep npu |awk -F 'Loss' '{print $2}'|awk '{print $1}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
