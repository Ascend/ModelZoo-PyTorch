...
train_accuracy=`grep -a 'valid_mlm_acc ->' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F " " '{print $12}'|awk 'BEGIN { max = 0 } { if ($1 > max) max = $1; fi} END{printf "%f\n", max}'`
#打印，不需要修改
