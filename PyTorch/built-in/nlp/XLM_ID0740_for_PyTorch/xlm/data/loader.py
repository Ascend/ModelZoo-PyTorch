...
def load_binarized(path, params):
    ...
    if getattr(params, 'multi_npu', False):
...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if splt == 'train' and params.split_data and 1 < params.n_npu_per_node <= data['mono_stream'][lang][splt].n_batches:
                n_batches = data['mono_stream'][lang][splt].n_batches // params.n_npu_per_node
            ...
            if lang in params.ae_steps or lang in params.bt_src_langs:
                ...
                # if there are several processes on the same machine, we can split the dataset
                if splt == 'train' and params.n_npu_per_node > 1 and params.split_data:
                    n_sent = len(dataset) // params.n_npu_per_node
