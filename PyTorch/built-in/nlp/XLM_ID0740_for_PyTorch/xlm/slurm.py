...
def init_distributed_mode(params):
    """
    Handle single and multi-NPU / multi-node / SLURM jobs.
    ...
    if params.is_slurm_job:
        ...
        params.n_npu_per_node = params.world_size // params.n_nodes
    ...
    # multi-NPU job (local or multi-node) - jobs started with torch.distributed.launch
        ...
        params.n_npu_per_node = int(os.environ['NNPU'])

        # number of nodes / node ID
        params.n_nodes = params.world_size // params.n_npu_per_node
        params.node_id = params.global_rank // params.n_npu_per_node
    ...
    # local job (single GPU)
        ...
        params.world_size = 1
        params.n_npu_per_node = 1

    # sanity checks
    assert params.n_nodes >= 1
    assert 0 <= params.node_id < params.n_nodes
    #assert 0 <= params.local_rank <= params.global_rank < params.world_size
    assert params.world_size == params.n_nodes * params.n_npu_per_node

    # define whether this is the master process / if we are in distributed mode
    params.is_master = params.node_id == 0 and params.local_rank == 0
    params.multi_node = params.n_nodes > 1
    params.multi_npu = params.world_size > 1
    ...
    print(PREFIX + "GPUs per node  : %i" % params.n_npu_per_node)
    print(PREFIX + "Master         : %s" % str(params.is_master))
    print(PREFIX + "Multi-node     : %s" % str(params.multi_node))
    print(PREFIX + "Multi-NPU      : %s" % str(params.multi_npu))
    print(PREFIX + "Hostname       : %s" % socket.gethostname())

    # set GPU device
    torch.npu.set_device(params.local_rank)

    # initialize multi-NPU
    if params.multi_npu:
