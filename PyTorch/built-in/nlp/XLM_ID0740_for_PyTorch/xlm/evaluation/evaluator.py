...
class Evaluator(object):
    ...
    def mask_out(self, x, lengths, rng):
        ...
        for i in range(bs):
            ...
            if not np.any(to_predict[:lengths[i] - 1, i]):
                ...
                to_predict[v, i] = 1
        pred_mask = torch.from_numpy(to_predict)
    ...
    def evaluate_clm(self, scores, data_set, lang1, lang2):
        ...
        model.eval()
        model = model.module if params.multi_npu else model
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        self.encoder.eval()
        self.decoder.eval()
        encoder = self.encoder.module if params.multi_npu else self.encoder
        decoder = self.decoder.module if params.multi_npu else self.decoder
