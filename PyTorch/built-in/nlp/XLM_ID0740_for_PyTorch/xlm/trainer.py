...
class Trainer(object):
    ...
    def init_amp(self):
        ...
        models, optimizers = apex.amp.initialize(
            ...
            opt_level=('O%i' % params.amp), combine_grad=True,
            combine_ddp=True if params.multi_npu else False
        )
            ...
            if self.n_iter % params.accumulate_gradients == 0:
                ...
                if params.clip_grad_norm > 0:
                    for name in names:
                        ...
                        clip_grad_norm_(apex.amp.master_params(self.optimizers[name]), params.clip_grad_norm, self.optimizers)
    ...
    def print_stats(self):
        ...
        """
        if self.n_total_iter % self.print_freq != 0:
            return
    ...
    def mask_out(self, x, lengths):
        ...
        if params.sample_alpha == 0:
            pred_mask = torch.ones(slen, bs)
            target_num = int(16 * (slen * bs * params.word_pred // 16))
        else:
        ...
        if params.sample_alpha == 0:
            pred_mask_tmp = pred_mask.flatten().nonzero().squeeze().numpy()
            pred_mask_tmp = np.random.choice(pred_mask_tmp, target_num, replace=False)
            pred_mask_zero = torch.zeros_like(pred_mask).flatten()
            pred_mask_zero[pred_mask_tmp] = 1
            pred_mask_zero = pred_mask_zero.bool().reshape(*pred_mask.shape)
            pred_mask = pred_mask_zero
    ...
    def pc_step(self, lang1, lang2, lambda_coeff):
        ...
        emb = (model.module if params.multi_npu else model).embeddings.weight
...
class EncDecTrainer(Trainer):
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        params = self.params
        _encoder = self.encoder.module if params.multi_npu else self.encoder
        _decoder = self.decoder.module if params.multi_npu else self.decoder
