    ...
    with torch.no_grad():
        for i in range(len(dico)):
            ...
            if idx is None:
                continue
            n_found += 1
            model.embeddings.weight[i] = embeddings[idx].npu()
            model.pred_layer.proj.weight[i] = embeddings[idx].npu()
...
def build_model(params, dico):
    ...
    if params.encoder_only:
        ...
        if params.reload_model != '':
            ...
            reloaded = torch.load(params.reload_model, map_location=lambda storage, loc: storage.npu(params.local_rank))['model']
        ...
        if params.reload_model != '':
            ...
            if enc_path != '':
                ...
                enc_reload = torch.load(enc_path, map_location=lambda storage, loc: storage.npu(params.local_rank))
            ...
            if dec_path != '':
                ...
                dec_reload = torch.load(dec_path, map_location=lambda storage, loc: storage.npu(params.local_rank))
        ...
        return encoder.npu(), decoder.npu()
