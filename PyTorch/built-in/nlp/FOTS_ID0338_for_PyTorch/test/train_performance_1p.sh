...
#训练开始时间，不需要修改
start_time=$(date +%s)
sed -i "s|# if iter == 12: exit()|if iter == 12: exit()|g" train.py
...
#单迭代训练时长
TrainingTime=`grep "step_time" $test_path_dir/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "step_time " '{print $2}'|tail -n +2|head -n -1|awk '{sum+=$1}END {print"",sum/NR}'|sed s/[[:space:]]//g`
