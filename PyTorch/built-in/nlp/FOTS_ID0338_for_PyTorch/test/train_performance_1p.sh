...
#单迭代训练时长
TrainingTime=`grep "step_time" $path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "step_time " '{print $2}'|tail -n +2|head -n -1|awk '{sum+=$1}END {print"",sum/NR}'|sed s/[[:space:]]//g`
