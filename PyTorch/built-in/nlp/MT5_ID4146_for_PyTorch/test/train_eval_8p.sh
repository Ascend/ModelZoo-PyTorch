...
#输出训练精度，需要模型审视修改
train_accuracy=$(grep "eval_bleu" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print}' | awk -F "=" '{print $2}' | sed 's/^[ \t]*//g')

#打印，不需要修改
echo "Final eval_bleu: ${train_accuracy}"
...
#最后一个迭代loss值，不需要修改
ActualLoss=`grep "eval_loss" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print}' | awk -F "=" '{print $2}' | sed 's/^[ \t]*//g'`
