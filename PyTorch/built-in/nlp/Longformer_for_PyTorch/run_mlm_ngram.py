...
@dataclass
class DataCollatorForNGram(DataCollatorForLanguageModeling):
    """
    ...
    def mask_tokens(
        ...
        for ind in range(n):
            nn = len(masked_indices[ind])
            for i in range(nn-1, -1, -1):
                if masked_indices[ind][i]:
                    step = np.random.choice(ngrams, p=pvals)
                    for ii in range(1, step):
                        if i+ii >= nn or masked_indices[ind][i+ii]:
                            break
                        masked_indices[ind][i+ii] = True
...
def main():
    ...
    for index in random.sample(range(len(tokenized_datasets['train'])), 3):
