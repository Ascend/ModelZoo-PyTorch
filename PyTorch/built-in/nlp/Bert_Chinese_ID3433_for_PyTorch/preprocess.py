...
def build_dataloader(dataset, tokenizer, bs):
...
def preprocess(data_files, model_dir_inp, save_dir_inp, bs, max_seq_length):
    dataset = load_dataset("text", data_files={"validation": data_files})
    dataset, tokenizer = tokenize_process(dataset,
                                          tokenizer_name=model_dir_inp,
                                          max_seq_length=max_seq_length)
    dataloader = build_dataloader(dataset, tokenizer, bs)

    for step, inputs in enumerate(dataloader):
        for input_name in inputs:
            save_path = os.path.join(save_dir_inp, input_name, "{}.bin".format(step))
