...
python3 -m torch.distributed.launch --nproc_per_node=1 examples/run_xfun_re.py \
...
#输出训练精度,需要模型审视修改
train_acc=$(grep "eval_f1" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print $7}')

#loss值，不需要修改
ActualLoss=$(grep "eval_loss" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print $7}')
