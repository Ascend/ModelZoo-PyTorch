...
def clip_grad_norm_(parameters: _tensor_or_tensors, max_norm: float, norm_type: float = 2.0) -> torch.Tensor:
    ...
    if norm_type == inf:
        total_norm = max(p.grad.float().detach().abs().max().to(device)
                         for p in parameters)
    else:
        total_norm = torch.norm(torch.stack([torch.norm(
            p.grad.float().detach(), norm_type).to(device) for p in parameters]), norm_type)
...
def clip_param_grad_value_(param, clip_value):
    r"""Clips gradient of an iterable of parameters at specified value.
    ...
    # ipdb.set_trace()
    clip_value = float(clip_value)
    # if abs(param.grad.data.item())>clip_value:
