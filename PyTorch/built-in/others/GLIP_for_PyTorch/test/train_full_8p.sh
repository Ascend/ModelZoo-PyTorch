...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep -a 'fps' ${WORK_DIR}/train_${ASCEND_DEVICE_ID}.log|awk -F "fps: " '{print $2}'|awk -F "(" '{print $1}'| tail -10000 | awk '{a+=$1} END {if (NR != 0) printf("%.3f",a/NR)}'`
# 打印，不需要修改
