...
class Linear(nn.Module):
    def __init__(self, feature_columns, feature_index, init_std=0.0001, device='cpu'):
        ...
        if len(self.dense_feature_columns) > 0:
            self.linear = nn.Linear(sum(fc.dimension for fc in self.dense_feature_columns), 1, bias=False)
    ...
    def forward(self, X, sparse_feat_refine_weight=None):
        ...
        if len(dense_value_list) > 0:
            dense_value_logit = self.linear(torch.cat(dense_value_list, dim=-1))
...
class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, l2_reg_linear=1e-5, l2_reg_embedding=1e-5,
                 init_std=0.0001, seed=1024, task='binary', device='cpu', gpus=None, dist=False, sparse_features_len=26):
        ...
        self.dnn_feature_columns = dnn_feature_columns
        self.sparse_features_len = sparse_features_len
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            validation_data=None, shuffle=True, callbacks=None, args=None):
        """
        ...
        for idx, k in enumerate(x):
            if idx < self.sparse_features_len:
        ...
        if not args.init_checkpoint == "":
            print('Loading checkpoint: ', args.init_checkpoint)
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            for step, p_train in enumerate(train_loader):
                ...
                if step > 5:
                    ...
                    avg_time = total_time / (total_step - 5)
                    avg_sample_per_sec = batch_size * num_rank / avg_time
                else:
                    avg_time = 0
                    avg_sample_per_sec = 0
    ...
    def input_from_feature_columns(self, X, feature_columns, embedding_dict, support_dense=True):
        ...
        dense_value_list = [X[idx + len(sparse_feature_columns)] for idx, feat in enumerate(dense_feature_columns)]
