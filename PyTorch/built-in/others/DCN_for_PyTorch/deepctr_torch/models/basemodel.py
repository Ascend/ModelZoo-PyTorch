...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, lr=0.0001, use_fp16=True, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            validation_data=None, shuffle=True, callbacks=None, early_stop_step=-1):
        """
        ...
        train_loader = DataLoader(
            dataset=train_tensor_data, shuffle=shuffle, batch_size=batch_size, num_workers=4, drop_last=True)
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            for step, p_train in enumerate(train_loader):
                x_list = [p.to(self.device) for p in p_train[:-1]]
                y_train = p_train[-1]
                y = y_train.float().to(self.device)
                ...
                if use_fp16:
                    ...
                    total_loss.backward()
                optim.step()
                self.adjust_learning_rate_cosine(optim, expected_steps, total_step, lr)
                ...
                time_start = time.time()
                if step > 5:
                    ave_time = total_time / (total_step - 5)
                    ave_sample_per_sec = batch_size * num_rank / ave_time
                else:
                    ave_time = 0
                    ave_sample_per_sec = 0
                if (self.dist and torch.distributed.get_rank() == 0) or not self.dist:
                    print(
                        'epoch={}, step={}, total_step={}, loss={:.4f}, lr={:.6f}, step_time={:.4f}, ave_time={:.4f}, sample_per_sec={:.4f},'
                        ' ave_sample_per_sec={:.4f}'.format(
                            epoch, step, total_step, total_loss.item(), optim.param_groups[0]['lr'], cost_time, ave_time,
                            batch_size * num_rank / cost_time, ave_sample_per_sec))
                ...
                total_step += 1
                if early_stop_step > 0 and total_step > early_stop_step:
                    break
            if early_stop_step > 0 and total_step > early_stop_step:
                break
        ...
        if (self.dist and torch.distributed.get_rank() == 0) or not self.dist:
            ...
            torch.save(ckpt, 'dcn_' + str(total_step) + '.pth')
            print('pth saved...')
    ...
    def input_from_feature_columns(self, X, feature_columns, embedding_dict, support_dense=True):
        ...
        sparse_embedding_list = [embedding_dict[feat.embedding_name](X[idx]) for
    ...
    def _get_optim(self, optimizer):
        if isinstance(optimizer, str):
            if optimizer == "sgd":
                ...
                import apex
                optim = apex.optimizers.NpuFusedAdam(self.parameters(), lr=0.0008, weight_decay=1e-4) # 0.0001/0.0008
    ...
    def adjust_learning_rate_cosine(self, optimizer, expected_steps, step, lr):
        alpha = 0
        cosine_decay = 0.5 * (1 + np.cos(np.pi * step / expected_steps))
        decayed = (1 - alpha) * cosine_decay + alpha
        optimizer.param_groups[0]['lr'] = lr * decayed
