...
from espnet.nets.pytorch_backend.nets_utils import make_non_pad_mask, pad_tensor
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        """
        # 1. forward encoder
        xs_pad = pad_tensor(xs_pad, 0, 512)
        xs = torch.zeros((ilens.shape[0], xs_pad.shape[1]))
        src_mask = make_non_pad_mask(ilens.tolist(), xs).to(xs_pad.device).unsqueeze(-2)
        hs_pad, hs_mask, hs_intermediates = self.encoder(xs_pad, src_mask)
        self.hs_pad = hs_pad

        # 2. forward decoder
        if self.decoder is not None:
            ys_pad = pad_tensor(ys_pad, self.ignore_id, 48)
