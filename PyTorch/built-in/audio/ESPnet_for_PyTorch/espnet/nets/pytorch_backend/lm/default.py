...
from espnet.nets.pytorch_backend.nets_utils import pad_tensor, NpuDropout
...
class DefaultRNNLM(BatchScorerInterface, LMInterface, nn.Module):
    ...
    def forward(self, x, t):
        ...
        for i in range(sequence_length):
            ...
            loss_batch_sum_list.append(loss_batch)
        
        zero = torch.zeros_like(loss_batch)
        for i in range(sequence_length, x.size(1)):
            loss_batch_sum_list.append(zero)
        
        non_zeros = torch.sum(x != 0, dim=0, dtype=loss_batch.dtype)
        logp = torch.stack(loss_batch_sum_list).sum(dim=1).mul(non_zeros).sum()
        loss = logp / batch_size
        count = non_zeros.sum()
        return loss / batch_size, loss, count.to(torch.int32)
