...
for /f "tokens=2 delims==" %%a in ('type ..\..\..\url.ini ^| findstr "azure_pytorch_url="') do set azure_pytorch_url=%%a
for /f "usebackq tokens=*" %%i in (`curl -so NUL -w "%%{http_code}" -u %VSTS_AUTH% %azure_pytorch_url%`) do (
    set STATUS_CODE=%%i
)
