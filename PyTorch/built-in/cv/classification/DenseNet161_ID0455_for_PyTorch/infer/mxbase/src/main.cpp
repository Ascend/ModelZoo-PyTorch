...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/densenet161 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/densenet161.om";
    auto densenet161 = std::make_shared<densenet161Classify>();
    APP_ERROR ret = densenet161->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "densenet161Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(binDir)) {
        ...
        ret = densenet161->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "densenet161Classify process failed, ret=" << ret << ".";
            densenet161->DeInit();
            return ret;
        }
    }
