...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/mobilenetv1 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/MobileNetV1.om";
    auto mobilenetv1 = std::make_shared<Mobilenetv1Classify>();
    APP_ERROR ret = mobilenetv1->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Mobilenetv1Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(binDir)) {
        ...
        ret = mobilenetv1->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "Mobilenetv1Classify process failed, ret=" << ret << ".";
            mobilenetv1->DeInit();
            return ret;
        }
    }
