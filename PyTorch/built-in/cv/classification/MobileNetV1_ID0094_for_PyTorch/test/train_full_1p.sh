...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a '* Prec@1' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print$3}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
