...
for((RANK_ID=$RANK_ID_START;RANK_ID<$NUMS;RANK_ID++));
do
    #设置环境变量，不需要修改
...
FPS=`cat ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${CaseName}_fps.log | awk '{a+=$1} END {if (NR != 0) printf("%.3f",a*4/NR)}'`
# 打印，不需要修改
