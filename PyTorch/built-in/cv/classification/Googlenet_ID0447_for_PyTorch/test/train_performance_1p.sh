...
nohup python3 train.py  \
    ...
    --print-freq 1 \
    --max_steps 100 > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep -a 'Epoch:'  $test_path_dir/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "img/s: " '{print $2}'|awk '{print $1}'|awk 'NR>2'| awk '{sum+=$1} END {print  sum/NR}'`
#打印，不需要修改
