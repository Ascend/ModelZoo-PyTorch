...
IF NOT EXIST "%SRC_DIR%\temp_build\cuda_9.0.176_windows.7z" (
    for /f "tokens=2 delims==" %%a in ('type ..\..\..\url.ini ^| findstr "cuda_9.0.176_url="') do set url=%%a
    curl -k -L %url% --output "%SRC_DIR%\temp_build\cuda_9.0.176_windows.7z"
...
IF NOT EXIST "%SRC_DIR%\temp_build\cudnn-9.0-windows7-x64-v7.zip" (
    for /f "tokens=2 delims==" %%a in ('type ..\..\..\url.ini ^| findstr "cudnn_9.0_url="') do set url=%%a
    curl -k -L %url% --output "%SRC_DIR%\temp_build\cudnn-9.0-windows7-x64-v7.zip"
    if errorlevel 1 exit /b 1
...
IF NOT EXIST "%SRC_DIR%\temp_build\cuda_9.1.85_windows.7z" (
    for /f "tokens=2 delims==" %%a in ('type ..\..\..\url.ini ^| findstr "cuda_9.1.85_url="') do set url=%%a
    curl -k -L %url% --output "%SRC_DIR%\temp_build\cuda_9.1.85_windows.7z"
    if errorlevel 1 exit /b 1
...
IF NOT EXIST "%SRC_DIR%\temp_build\cudnn-9.1-windows7-x64-v7.zip" (
    for /f "tokens=2 delims==" %%a in ('type ..\..\..\url.ini ^| findstr "cudnn_9.1_url="') do set url=%%a
    curl -k -L %url% --output "%SRC_DIR%\temp_build\cudnn-9.1-windows7-x64-v7.zip"
    if errorlevel 1 exit /b 1
...
IF NOT EXIST "%SRC_DIR%\temp_build\NvToolsExt.7z" (
    for /f "tokens=2 delims==" %%a in ('type ..\..\..\url.ini ^| findstr "NvToolsExt_url="') do set url=%%a
    curl -k -L %url% --output "%SRC_DIR%\temp_build\NvToolsExt.7z"
    if errorlevel 1 exit /b 1
)
