...
do       
     python3.7 -u 8p_main_med.py \
        ...
        --local_rank=$i \
        --benchmark 0 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep FPS ${cur_path}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $(NF-2)}'|awk '{sum+=$1} END {print  sum/NR}'`
