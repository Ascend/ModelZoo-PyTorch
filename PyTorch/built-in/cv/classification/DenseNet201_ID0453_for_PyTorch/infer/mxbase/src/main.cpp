...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/densenet201 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/densenet201.om";
    auto densenet201 = std::make_shared<densenet201Classify>();
    APP_ERROR ret = densenet201->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "densenet201Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(binDir)) {
        ...
        ret = densenet201->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "densenet201Classify process failed, ret=" << ret << ".";
            densenet201->DeInit();
            return ret;
        }
    }
