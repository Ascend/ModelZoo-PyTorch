...
parser.add_argument('--loss_scale', default=32, type=int, help='loss_scale for amp')
...
def train(train_loader, model, criterion, optimizer, epoch, args, nnpus_per_node):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        target = target.int()
        images, target = images.to('npu:' + str(args.npu), non_blocking=True), target.to('npu:' + str(args.npu),
                                                                                         non_blocking=True)
