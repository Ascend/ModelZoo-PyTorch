...
set BUILD_VISION=1
for /f "tokens=2 delims==" %%a in ('type ..\..\url.ini ^| findstr "pytorch_whl_url="') do set pytorch_whl_url=%%a
set TORCH_WHEEL=torch -f %pytorch_whl_url%/%CUVER%/stable.html --no-index
...
rmdir /s /q conda
del miniconda.exe
for /f "tokens=2 delims==" %%a in ('type ..\..\url.ini ^| findstr "miniconda3_windows_url="') do set miniconda3_windows_url=%%a
curl -k %miniconda3_windows_url% -o "%miniconda_exe%"
