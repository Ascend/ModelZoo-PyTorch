...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/densenet169 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/densenet169.om";
    auto densenet169 = std::make_shared<densenet169Classify>();
    APP_ERROR ret = densenet169->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "densenet169Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(binDir)) {
        ...
        ret = densenet169->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "densenet169Classify process failed, ret=" << ret << ".";
            densenet169->DeInit();
            return ret;
        }
    }
