...
nohup taskset -c 0-32 python3.7 ./tools/train.py ./configs/resnet/resnet50_8xb16_cifar100_perf.py > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'Epoch(val)' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "top-1: " '{print $2}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
