...
def gen_input_bin(mode_type_inp, file_batches, batch, src_path_inp, save_path_inp):
    for file in tqdm(file_batches[batch]):
        # RGBA to RGB
        image = Image.open(os.path.join(src_path_inp, file)).convert('RGB')
        ...
        img.tofile(os.path.join(save_path_inp, file.split('.')[0] + ".bin"))
...
def preprocess(mode_type_inp, src_path_inp, save_path_inp):
    ...
    for batch in range(len(file_batches)):
        thread_pool.apply_async(gen_input_bin, args=(mode_type_inp, file_batches, batch, src_path_inp, save_path))
    thread_pool.close()
    thread_pool.join()
