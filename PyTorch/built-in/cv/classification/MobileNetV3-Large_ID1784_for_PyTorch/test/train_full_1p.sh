...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep Epoch:  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Avg" '{print$2}' | awk '{print$1}' | tail -n+2 | awk '{sum+=$1} END {print"",sum/NR}' | sed s/[[:space:]]//g`
#打印，不需要修改
