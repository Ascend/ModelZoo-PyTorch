...
from StreamManagerApi import StreamManagerApi, InProtobufVector, \
    MxProtobufIn, StringVector, MxDataInput
...
def w2txt(file_path, filename_list, data):
    ...
    for i in range(data.shape[0]):
        with open(os.path.join(file_path, filename_list[i].split('.')[0] + ".txt"), "a") as file:
            s = ' '.join(str(format(num, 'd')) for num in data[i])
            file.write(s + "\n")
...
def run():
    ...
    with open("../data/config/gluon_resnet50_v1b.pipeline", 'rb') as f:
        pipelineStr = f.read()
    ...
    stream_name = b'gluon_resnet50_v1b'
    ...
    for file in file_list:
        ...
        for i in range(resCnt):
            if (str(realLabel) == str(sort_index[i])):
                count_hit[i] += 1
                break
        res_sents.append(sort_index[:resCnt])

    file_path = "sdk_result"
    res_sents = np.array(res_sents)
