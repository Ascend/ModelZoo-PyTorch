...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep Train  $test_path_dir/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep -v '1/'|grep -v '2/' |grep -v '3/'|awk -F 'fps:' '{print $2}'|awk '{print $1}'|awk '{sum+=$1} END {print  sum/NR}'`
#打印，不需要修改
