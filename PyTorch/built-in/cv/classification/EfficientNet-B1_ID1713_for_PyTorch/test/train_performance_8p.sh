...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="EfficientNet-B1_ID1713_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=2048
...
sed -i "s|i > len(val_loader)|i > $test_steps|g"  ${test_path_dir}/../examples/imagenet/main.py
wait
...
sed -i "s|i > $test_steps|i > len(val_loader)|g"  ${test_path_dir}/../examples/imagenet/main.py
wait
#结果打印，不需要修改
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
