...
nohup  python3.7 ${cur_path}/../train_deepmar_resnet50.py \
      --save_dir=$data_path \
      --workers=32 \
      ...
      --loss_scale 512 \
      --amp \
      --opt_level O2  > $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep FPS ${cur_path}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep -v loss|awk '{print $NF}'|awk '{sum+=$1} END {print  sum/NR}'`

#输出训练精度，需要模型审视修改
train_accuracy=`grep -a 'Acc' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Acc" '{print $2}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
