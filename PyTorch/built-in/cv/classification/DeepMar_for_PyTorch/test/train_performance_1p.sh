...
nohup  taskset -c 0-23 python3 ${cur_path}/train_deepmar_resnet50.py \
      ...
      --loss_scale 512 \
      --amp \
      --opt_level O2  > ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
...
#输出编译时间
CompileTime=`grep FPS ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| head -2 |awk -F "batchtime:" '{print $2}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
