...
nohup python3 -u tools/train_siammask.py \
                ...
                --log experiments/siammask_base/logs/log.txt > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_1p_perf.log 2>&1 &
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
time=`grep "Progress:" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_1p_perf.log | tail -n 1 | awk -F "Speed: " '{print $2}' | awk -F " " '{print $1}'`
...
grep 'siammask_loss:' ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}_1p_perf.log|awk -F "siammask_loss:" '{print $2}' |awk -F " " '{print $1}' >> ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
