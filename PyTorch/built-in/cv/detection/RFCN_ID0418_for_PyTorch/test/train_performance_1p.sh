...
#获取编译时间
CompileTime=`grep "batch time:" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -2 | awk -F "batch time:" '{print $2}' | awk -F "," '{print $1}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
