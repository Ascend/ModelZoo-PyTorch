...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                output = torch.cat([reg_output, obj_output, cls_output], 1).float()
                ...
                output = torch.cat(
                    [reg_output, obj_output.float().sigmoid(), cls_output.float().sigmoid()], 1
                )
    ...
    def get_output_and_grid(self, output, k, stride, dtype, device):
        ...
        if grid.shape[2:4] != output.shape[2:4]:
            ...
            grid = torch.stack((xv, yv), 2).view(1, 1, hsize, wsize, 2).to(device).to(dtype)
            self.grids[k] = grid
    ...
    def decode_outputs(self, outputs, dtype, device):
        ...
        grids = torch.cat(grids, dim=1).to(device).to(dtype)
        strides = torch.cat(strides, dim=1).to(device).to(dtype)
    ...
    def get_losses(
        ...
        num_fg = max(num_fg, 1)
        loss_iou = (
            self.iou_loss(bbox_preds.view(-1, 4)[fg_masks].float(), reg_targets.float())
        ).sum() / num_fg
        loss_obj = (
