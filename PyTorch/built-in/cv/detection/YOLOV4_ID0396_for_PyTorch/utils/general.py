...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    tcls, tbox, indices, anchors, target_mask = build_targets(p, targets, model)  # targets
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if mask_sum:
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                t[torch.arange(n, dtype=int, device=device), tcls[i]] = cp
                #lcls += BCEcls(ps[:, 5:], t)  # BCE
                lcls += (BCEcls(ps[:, 5:], t) * mask.view(-1, 1)).sum() / mask_sum / t.shape[1]
...
def build_targets(p, targets, model):
    ...
    return tcls, tbox, indices, anch, targets_mask
