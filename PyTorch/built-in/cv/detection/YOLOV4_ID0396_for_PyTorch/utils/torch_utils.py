...
def select_device(device='', batch_size=None):
    ...
    if device and not cpu_request:  # if device requested other than 'cpu'
        ...
        assert torch.npu.is_available(), 'NPUZH unavailable, invalid device %s requested' % device  # check availablity

    npu = False if cpu_request else torch.npu.is_available()

    return torch.device('npu:{}'.format(device) if npu else 'cpu')
