...
if [ x"${etp_running_flag}" != x"true" ];then
    ...
    sed -i "s|./coco/annotations/instances_val|$data_path/annotations/instances_val|g" test.py
else
...
#评测
if [ x"${etp_running_flag}" != x"true" ];then
    nohup taskset -c 0-23  python3.7 test.py --img-size $image_size --conf 0.001 --batch ${batch_size} --device ${ASCEND_DEVICE_ID} --data coco.yaml --cfg cfg/yolov4.cfg --weights runs/evolve/weights/best.pt >> ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
fi
wait
...
if [ x"${etp_running_flag}" = x"true" ];then
    ...
    sed -i "s|$data_path/annotations/instances_val|./coco/annotations/instances_val|g" test.py
fi
wait
