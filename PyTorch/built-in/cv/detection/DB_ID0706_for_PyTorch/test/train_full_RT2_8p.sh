...
# 输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'precision' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log | awk 'END {print}' | awk -F ' ' '{print $(NF-1)}'`
train_recall=`grep -a 'recall' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log | awk 'END {print}' | awk -F ' ' '{print $(NF-1)}'`
train_fmeasure=`grep -a 'fmeasure' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log | awk 'END {print}' | awk -F ' ' '{print $(NF-1)}'`
...
grep -a 'messages:Epoch:' ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk -F 'Loss' '{print $NF}' | awk '{print $1}'|awk 'NR%8==0' > ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
