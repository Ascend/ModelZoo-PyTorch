...
if [[ $1 == --help || $1 == --h ]];then
   echo "usage:bash test/train_full_8p.sh --data_path=data_dir --batch_size=32"
   exit 1
fi
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'Acc_t:' ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk 'END {print}'|awk -F "Acc_t:" '{print $NF}'|awk -F " " '{print $1}'`
