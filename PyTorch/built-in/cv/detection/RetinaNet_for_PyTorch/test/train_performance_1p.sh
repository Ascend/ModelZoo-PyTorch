...
cp train_retinanet_1p.sh $cur_path/../
...
do
    ...
    bash train_retinanet_1p.sh > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1
...
##获取性能数据
grep "time:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log > traintime.log
