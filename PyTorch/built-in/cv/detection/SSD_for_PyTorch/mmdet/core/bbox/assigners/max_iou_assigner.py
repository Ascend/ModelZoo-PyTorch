...
class MaxIoUAssigner(BaseAssigner):
    ...
    def batch_iou(self, concat_gt_bboxes, concat_bboxes, **kwargs):
        num_images, num_gts = concat_gt_bboxes.size(0), concat_gt_bboxes.size(1)
        num_bboxes = concat_bboxes.size(1)
        concat_overlaps = torch.zeros((num_images, num_gts, num_bboxes),
                                      dtype=concat_bboxes.dtype,
                                      device=concat_bboxes.device)
        for index_images in range(num_images):
            concat_overlaps[index_images] = self.iou_calculator(concat_gt_bboxes[index_images],
                                                                concat_bboxes[index_images],
                                                                **kwargs)
        return concat_overlaps

    def concat_assign(self, concat_bboxes,
                      concat_gt_bboxes,
                      concat_gt_bboxes_ignore=None,
                      concat_gt_labels=None,
                      concat_bboxes_ignore_mask=None):
        concat_overlaps = self.batch_iou(concat_gt_bboxes, concat_bboxes)
        concat_overlaps = set_index(concat_overlaps, concat_bboxes_ignore_mask, -1)
        if self.ignore_iof_thr > 0 and concat_gt_bboxes_ignore is not None:
            if self.ignore_wrt_candidates:
                concat_ignore_overlaps = self.batch_iou(concat_bboxes, concat_gt_bboxes_ignore, mode='iof')
                ...
                concat_ignore_overlaps = self.batch_iou(concat_gt_bboxes_ignore, concat_bboxes, mode='iof')
                concat_ignore_overlaps = set_index(concat_ignore_overlaps, concat_bboxes_ignore_mask, -1)
        ...
        concat_assign_result = self.concat_assign_wrt_overlaps(concat_overlaps, concat_gt_labels)
        return concat_assign_result

    def concat_assign_wrt_overlaps(self, concat_overlaps, concat_gt_labels=None):
        ...
        if isinstance(self.neg_iou_thr, float):
            ...
            assert len(self.neg_iou_thr) == 2
            concat_neg_mask = ((concat_max_overlaps >= self.neg_iou_thr[0]) &
                               (concat_max_overlaps < self.neg_iou_thr[1])).int()
        else:
            concat_neg_mask = torch.zeros(concat_max_overlaps.size(), dtype=torch.int,
                                          device=concat_max_overlaps.device)
        ...
        return {
            "concat_pos_mask": concat_pos_mask,
            "concat_neg_mask": concat_neg_mask,
            "concat_anchor_gt_indes": concat_argmax_overlaps,
            "concat_anchor_gt_labels": concat_anchor_gt_labels,
        }
