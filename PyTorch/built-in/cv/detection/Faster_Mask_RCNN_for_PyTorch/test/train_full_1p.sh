...
nohup python3.7 tools/train_net.py \
        --config-file  configs/COCO-Detection/mask_rcnn_R_101_FPN_3x.yaml \
        AMP 1 \
        OPT_LEVEL O2 \
        LOSS_SCALE_VALUE 64 \
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "Average Precision" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "=" '{print $NF}'|tail -1`
#打印，不需要修改
