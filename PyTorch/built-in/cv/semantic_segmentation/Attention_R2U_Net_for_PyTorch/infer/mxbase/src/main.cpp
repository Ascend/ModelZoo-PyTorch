...
APP_ERROR ReadImagefromTxt(const std::string &txtPath, std::vector<std::vector<float>> &test_image,
    ...
    auto test_images_path = txtPath + "test_images.txt";
    auto test_GT_images_path = txtPath + "test_GT_images.txt";
...
void Attention_R2U_Net_Param(InitParam *initParam) {
    initParam->deviceId = 0;
    initParam->modelPath = "../data/model/R2AttU_Net.om";
}

int main(int argc, char* argv[]) {
    InitParam initParam;
    Attention_R2U_Net_Param(&initParam);
    auto attention_r2u_net = std::make_shared<Attention_R2U_Net>();
    APP_ERROR ret = attention_r2u_net->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Attention_R2U_Net init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    if (ret != APP_ERR_OK) {
        LogError << "Attention_R2U_Net read image failed, ret=" << ret << ".";
        attention_r2u_net->DeInit();
        return ret;
    }
    for (uint32_t i = 0; i < test_images.size(); i++) {
        ret = attention_r2u_net->Process(test_images[i]);
        if (ret != APP_ERR_OK) {
            LogError << "Attention_R2U_Net process failed, ret=" << ret << ".";
            attention_r2u_net->DeInit();
            return ret;
        }
    }
