...
do
  ...
  nohup taskset -c $PID_START-$PID_END python3 -u tools/train.py configs/bisenetv1/bisenetv1_r50-d32_4x4_1024x1024_160k_cityscapes_8p_perf.py \
    ...
    --seed 36436756 \
    --opt-level O1 > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_8p_perf.log 2>&1 &
done
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
time=`grep "mmseg - INFO - Iter " ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}_8p_perf.log | tail -n 205 | head -n 200 | awk -F "time:" '{print $2}' | awk -F "," '{print $1}' | awk '{sum+=$1} END {print sum/200}'`
...
grep "mmseg - INFO - Iter " ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}_8p_perf.log | awk -F "loss: " '{print $2}' >> ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
