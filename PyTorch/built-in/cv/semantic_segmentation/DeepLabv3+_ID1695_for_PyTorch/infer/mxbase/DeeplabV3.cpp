...
APP_ERROR DeeplabV3::FixScaleCrop(const cv::Mat &srcImgMat, cv::Mat &dstImgMat, 
    int cropSize, MxBase::ResizedImageInfo &resizedImageInfo)
{
    int w = srcImgMat.cols;
    int h = srcImgMat.rows;
    int oh = 0;
    int ow = 0;
    if (w > h) {
        oh = cropSize;
    ...
    // resize
    cv::resize(srcImgMat, dstImgMat, cv::Size(ow, oh));
    int x1 = (int)((ow - cropSize) / 2);
    int y1 = (int)((oh - cropSize) / 2);

    resizedImageInfo.heightOriginal = cropSize;
    resizedImageInfo.heightResize = cropSize;
    resizedImageInfo.widthOriginal = cropSize;
    resizedImageInfo.widthResize = cropSize;
    resizedImageInfo.resizeType = RESIZER_MS_KEEP_ASPECT_RATIO;

    // crop
    static cv::Rect rectOfImg(x1, y1, cropSize, cropSize);
    dstImgMat = dstImgMat(rectOfImg).clone();
...
APP_ERROR DeeplabV3::Process(const std::string &imgPath)
{
    ...
    cv::Mat imageMat;
    FixScaleCrop(srcImageMat, imageMat, 513, resizedImageInfo);
