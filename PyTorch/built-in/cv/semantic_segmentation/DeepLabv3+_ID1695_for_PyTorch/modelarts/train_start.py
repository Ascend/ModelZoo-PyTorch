...
def convert_pth_to_onnx(dataset, num_classes):
    pth_pattern = os.path.join(_CACHE_TRAIN_OUT_URL,
                               f'run/{dataset}/deeplab-resnet/',
                               'model_best.pth.tar')
    ...
    pth = pth_list[-1]
    onnx_path = pth + '.onnx'
    convert(pth, onnx_path, num_classes)
...
def main():
    ...
    train_script = os.path.join(root_dir, 'test/train_full_1p.sh')
    ...
    try:
        ...
        if args.onnx:
            ...
            convert_pth_to_onnx(args.dataset, args.num_classes)
    except Exception as exp:
