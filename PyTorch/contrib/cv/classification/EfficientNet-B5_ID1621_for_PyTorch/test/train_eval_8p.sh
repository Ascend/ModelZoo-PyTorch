...
# 网络名称，同目录名称
Network="EfficientNet-B5_ID1621_for_PyTorch"
# 训练batch_size
batch_size=240
# 训练使用的npu卡数
export RANK_SIZE=8
...
python3 ./tools/test_net.py \
    ...
    TEST.WEIGHTS /home/model_epoch_0042.pyth \
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep 'test_epoch' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $5}'|awk 'END {print}'`
FPS=${FPS%,*}
# 打印，不需要修改
echo "Final Performance images/sec : $FPS"

# 输出训练精度,需要模型审视修改
top1_err=`grep 'test_epoch' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $23}'|awk 'END {print}'`
