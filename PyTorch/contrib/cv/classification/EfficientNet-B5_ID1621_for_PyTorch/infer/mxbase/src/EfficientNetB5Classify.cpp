...
APP_ERROR EfficientNetB5Classify::Init(const InitParam &initParam)
{
...
APP_ERROR EfficientNetB5Classify::DeInit()
{
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR EfficientNetB5Classify::ReadImage(const std::string &imgPath, cv::Mat &imgMat)
{
...
void EfficientNetB5Classify::Resize(const cv::Mat &srcMat, cv::Mat &dstMat)
{
    static constexpr uint32_t resizeWidth = 488;
    static constexpr uint32_t resizeHeight = 488;
    cv::resize(srcMat, dstMat, cv::Size(resizeWidth, resizeHeight), 0, 0, cv::INTER_CUBIC);
}

APP_ERROR EfficientNetB5Classify::CvMatToTensorBase(const cv::Mat &imgMat, MxBase::TensorBase &tensorBase)
{
...
APP_ERROR EfficientNetB5Classify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientNetB5Classify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientNetB5Classify::Process(const std::string &imgPath)
{
    cv::Mat image;
...
std::string EfficientNetB5Classify::FormatResultFile(const std::string &imgPath)
{
...
APP_ERROR EfficientNetB5Classify::SaveResult(const std::string &imgPath, const std::vector<std::vector<MxBase::ClassInfo>> &batchClsInfos)
{
    ...
    for (auto &clsInfos : batchClsInfos) {
	    ...
	    outfile<<"image path: " <<imgPath<<"\n";
	    outfile<<"batchIndex:" << batchIndex<<"\n";
	    outfile<<"className:" << clsInfo.className<<"\n";
            outfile<<"confidence:" << clsInfo.confidence<<"\n";
	    outfile<<"classIndex:" <<  clsInfo.classId<<"\n";
	    topkIndex++;
         }
