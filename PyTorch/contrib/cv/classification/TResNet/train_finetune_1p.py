...
parser.add_argument('--pth-path', metavar='NAME', default='',
                    help='pth path')                    
...
def load_state_dict(checkpoint_path, use_ema=False):
    if checkpoint_path and os.path.isfile(checkpoint_path):
        ...
        print("Success: Loaded {} from checkpoint '{}'".format(state_dict_key, checkpoint_path))
        return state_dict
    else:
        print("No checkpoint found at '{}'".format(checkpoint_path))
        raise FileNotFoundError()

def build_model(pth_path,num_class=1000):
    import timm
    model = timm.create_model('tresnet_m')
    state_dict = load_state_dict(pth_path) #'output/train/20211005-221556-tresnet_m-224/checkpoint-107.pth.tar'
    model.load_state_dict(state_dict, strict=False)
    model.reset_classifier(num_class)
    return model
#@pysnooper.snoop()
def main():
    ...
    model = build_model(args.pth_path,args.num_classes)
...
def train_one_epoch(
    ...
    if args.local_rank == 0:
        print('FPS = {:.2f}\t'.format(args.batch_size *int(os.environ['WORLD_SIZE'])/ batch_time_m.avg))
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if not args.prefetcher:
                 ...
                 #input, target = input.to(args.device,non_blocking=True), target.to(args.device,non_blocking=True)
            if args.channels_last:
                input = input.contiguous(memory_format=torch.channels_last)
