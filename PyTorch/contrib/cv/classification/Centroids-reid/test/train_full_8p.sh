...
#输出性能FPS，需要模型审视修改
ASCEND_DEVICE_ID=0
FPS=`grep -a 'it/s'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep 'Epoch'|awk -F "it/s" '{print $1}'|awk -F ',' '{print $2}'|sed 's/ //g'|awk 'END {print}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'mAP:'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F ":" '{print $NF}'|awk 'END {print}'`
#打印，不需要修改
