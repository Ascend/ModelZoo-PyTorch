...
def fast_gather_3d_v2(input, dim, index):
    if dim == 2:
        index_zero = torch.arange(index.shape[0]).reshape(index.shape[0], 1, 1)
        index_second = torch.arange(index.shape[1]).reshape(1, index.shape[1], 1)
        result = input[index_zero, index_second, index]
        return result
    else:
        print("only support dim=2")
        exit(0)

def fast_gather_4d_v2(input, dim, index):
    if dim == 1:
        index_zero = torch.arange(index.shape[0]).reshape(index.shape[0], 1, 1, 1)
        index_second = torch.arange(index.shape[2]).reshape(1, 1, index.shape[2], 1)
        index_third = torch.arange(index.shape[3]).reshape(1, 1, 1, index.shape[3])
        result = input[index_zero, index, index_second, index_third]
        return result
    else:
...
def get_topk_closest_indice(q_windows, k_windows, topk=1):
    ...
    relative_coord_topk = fast_gather_3d_v2(relative_coords, 2, indice_topk.unsqueeze(0).expand(2, indice_topk.shape[0], indice_topk.shape[1]))
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):
            ...
            k_k_selected = fast_gather_4d_v2(k_k, 1, topk_indice_k.view(B, -1, 1).unsqueeze(-1).expand(topk_indice_k.shape[0], topk_indice_k.shape[1], self.num_heads, C // self.num_heads))
            # v_k_selected = torch.gather(v_k, 1, topk_indice_k.view(B, -1, 1).unsqueeze(-1).repeat(1, 1, self.num_heads, C // self.num_heads))
            v_k_selected = fast_gather_4d_v2(v_k, 1, topk_indice_k.view(B, -1, 1).unsqueeze(-1).expand(topk_indice_k.shape[0], topk_indice_k.shape[1], self.num_heads, C // self.num_heads))
        ...
        attn = matmul_transpose(q_windows, k_all)
        window_area = self.window_size[0] * self.window_size[1]
        window_area_whole = k_all.shape[2]

        topk_rpb_cat_t = torch.cat(topk_rpbs, 2).npu().permute(0, 3, 1, 2).contiguous().unsqueeze(0)
        _, _C, _D, _H, _W = topk_rpb_cat_t.shape
        topk_rpb_cat = topk_rpb_cat_t.expand(B, _C, _D, _H, _W).reshape(attn.shape)
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
                 ...
                 act_layer=FastGELU, norm_layer=nn.LayerNorm, pool_method="none",
...
def profile(model, inputs):
    ...
    with profile(activities=[
            ProfilerActivity.CPU, ProfilerActivity.npu], with_stack=True, record_shapes=True) as prof:
...
if __name__ == '__main__':
    img_size = 224
    x = torch.rand(16, 3, img_size, img_size).npu()
