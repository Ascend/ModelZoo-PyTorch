...
#include "WideResnet50.h"
#include "MxBase/DeviceManager/DeviceManager.h"
#include "MxBase/Log/Log.h"

using namespace MxBase;

APP_ERROR WideResnet50::Init(const InitParam &initParam)
{
...
APP_ERROR WideResnet50::ReadImage(const std::string &imgPath, cv::Mat &imageMat)
{
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR WideResnet50::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat)
{
...
APP_ERROR WideResnet50::Crop(const cv::Mat &srcMat, cv::Mat &dstMat)
{
    static cv::Rect rectOfImg(32, 32, 304, 304);
    dstMat = srcMat(rectOfImg).clone();
    return APP_ERR_OK;
}
APP_ERROR WideResnet50::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase)
{
...
APP_ERROR WideResnet50::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR WideResnet50::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR WideResnet50::SaveResult(const std::string &imgPath, const std::vector<std::vector<MxBase::ClassInfo>> &batchClsInfos)
{
...
APP_ERROR WideResnet50::Process(const std::string &imgPath)
{
    cv::Mat imageMat;
