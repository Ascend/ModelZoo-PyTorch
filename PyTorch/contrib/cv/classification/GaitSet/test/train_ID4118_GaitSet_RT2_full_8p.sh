...
start_time=$(date +%s)
#执行训练脚本，传参需要审视修改
python3 -m torch.distributed.launch --nproc_per_node=$N_NPUS --master_port=654322 train_main.py --dist_backend='hccl' \
    --world_size=$N_NPUS \
    --rank=0 \
    --device_num=$N_NPUS \
    --rt2 \
...
python3 -u test_main.py --iter=${iters} --data_path=${data_path} --batch_size=64 --cache=True >> ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
#训练结束时间，不需要修改
end_time=$(date +%s)
...
grep "Full_Loss" ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk -F'Full_Loss' '{print$2}'|awk -F' ' '{print$1}'|awk 'NR%4==0' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
