...
def npu_timestamp(sync=False, device=None):
    if sync:
...
class BenchmarkRunner:
    def __init__(
            self, model_name, detail=False, device='npu', torchscript=False, precision='float32',
        ...
        self.amp_autocast = torch.npu.amp.autocast if self.use_amp else suppress
        ...
        if 'npu' in self.device:
            self.time_fn = partial(npu_timestamp, device=self.device)
        else:
...
class InferenceBenchmarkRunner(BenchmarkRunner):
    ...
    def __init__(self, model_name, device='npu', torchscript=False, **kwargs):
