...
NVIDIA npu specific speedups adopted from NVIDIA Apex examples
...
# Cache dataset directory
CACHE_DATA_URL = '/cache/data_url/'
# Cache model output directory
...
parser.add_argument('--custom_batchsize', type=int, default=0,
                    help='modelarts custom batchsize for quick train')
parser.add_argument('--custom_epoch', type=int, default=0,
                    help='modelarts custom epoch for quick train')
parser.add_argument('--resume_pth',
                    help='pth file',
                    type=str,
                    default='')
parser.add_argument('--pretrained_load',
                    help='lolad pth',
                    type=str,
                    default='')
...
def main():
    ...
    if args.custom_batchsize > 0 :
        args.batch_size = args.custom_batchsize

    # create the train and eval datasets
    dataset_train = create_dataset(
        args.dataset,
        root=root_path, split=args.train_split, is_training=True,
        batch_size=args.batch_size, repeats=args.epoch_repeats)
    dataset_eval = create_dataset(
        args.dataset, root=root_path, split=args.val_split, is_training=False, batch_size=args.batch_size)
    ...
    #modelarts
    if args.custom_epoch > 0 :
        num_epochs = args.custom_epoch
...
def convert(model_path):
    checkpoint = torch.load(os.path.join(model_path, 'model_best.pth.tar'), map_location='cpu')
    ...
    with amp.disable_casts():
        ...
        torch.onnx.export(model, dummy_input, os.path.join(model_path, 'spnasnet_100_npu_1.onnx'), input_names=input_names, output_names=output_names,
                          opset_version=11)
