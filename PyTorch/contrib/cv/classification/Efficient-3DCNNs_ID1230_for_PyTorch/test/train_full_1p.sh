...
nohup python3 ../main.py \
    ...
    --n_finetune_classes 101 \
	  ...
	  --n_threads 16 \
	  --ft_portion complete \
	  > ${root_path}/results/${ASCEND_DEVICE_ID}/npu_train_full_1p.log 2>&1 &
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep 'fps' ${root_path}/results/${ASCEND_DEVICE_ID}/npu_train_full_1p.log| tail -n 1 |awk '{print $6}'|awk 'END {print}'`
FPS=${FPS%,*}
# 打印，不需要修改
echo "Final Performance images/sec : $FPS"

# 输出训练精度,需要模型审视修改
top1_acc=`grep 'test top1 acc' ${root_path}/results/${ASCEND_DEVICE_ID}/npu_train_full_1p.log|awk '{print $4}'|awk 'END {print}'`
...
grep 'date' ${root_path}/results/${ASCEND_DEVICE_ID}/npu_train_full_1p.log|awk '{print $8}' >> ${root_path}/results/${ASCEND_DEVICE_ID}/train_${CaseName}_loss.txt
# 删除loss值后逗号
