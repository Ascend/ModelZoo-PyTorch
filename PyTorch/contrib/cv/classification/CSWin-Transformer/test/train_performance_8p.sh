...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep Train ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep -a '100/625'|awk -F "100/625" '{print $NF}'|awk -F " " '{print substr($7,0,length($7)-1)}'|awk 'END {print}'`
#打印，不需要修改
