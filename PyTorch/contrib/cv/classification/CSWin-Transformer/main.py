...
def main():
    ...
    args.device = f'npu:{str(args.local_rank)}'
    args.world_size = 1
    ...
    if args.num_gpu > 1:
        if use_amp == 'apex':
            ...
            use_amp = None
        model = nn.DataParallel(model, device_ids=list(range(args.num_gpu))).to(args.device)
    ...
    try:  # train the model
        for epoch in range(start_epoch, num_epochs):
            loader_train, prefetcher_train = create_loader(
                dataset_train,
                input_size=data_config['input_size'],
            ...
            train_metrics = train_epoch(
                epoch, model, loader_train, prefetcher_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                amp_autocast=amp_autocast, loss_scaler=None, model_ema=model_ema, mixup_fn=mixup_fn)
...
def train_epoch(
        epoch, model, loader, prefetcher, optimizer, loss_fn, args,
    ...
    for batch_idx in range(max_iter):
        ...
        if model_ema is not None:
            model_params_fused = optimizer.get_optimizer_combined_params()
            model_ema.update(model, device=args.device, model_params_fused=model_params_fused[0])
        num_updates += 1
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                reduced_loss = loss.data #reduce_tensor(loss.data, args.world_size)
