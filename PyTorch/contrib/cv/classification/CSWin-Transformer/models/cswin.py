...
class LePEAttention(nn.Module):
    ...
    def im2cswin(self, x, trans=False):
        B, N, C = x.shape
        H = W = int(np.sqrt(N))
        x = x.reshape(B, H // self.H_sp, self.H_sp, W //self. W_sp, self.W_sp, C)
        if not trans:
            x = x.permute(0, 1, 3, 2, 4, 5).reshape(-1, self.H_sp* self.W_sp, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        else:
            x = x.permute(0, 1, 3, 2, 4, 5).reshape(-1, self.H_sp* self.W_sp, self.num_heads, C // self.num_heads).permute(0, 2, 3, 1)
        return x
    ...
    def get_lepe(self, x, func):
        ...
        x = x.reshape(B, H // H_sp, H_sp, W // W_sp, W_sp, C).permute(0, 1, 3, 5, 2, 4).reshape(-1, C, H_sp, W_sp) ### B', C, H', W'

        lepe = func(x) ### B', C, H', W'
        lepe = lepe.reshape(-1, self.num_heads, C // self.num_heads, self.H_sp* self.W_sp).permute(0, 1, 3, 2)
    ...
    def forward(self, qkv):
        ...
        q = self.im2cswin(q)
        k = self.im2cswin(k, trans=True)
        v, lepe = self.get_lepe(v, self.get_v)

        q = q * self.scale
        attn = (q @ k)  # B head N C @ B head C N --> B head N N
        attn = nn.functional.softmax(attn, dim=-1, dtype=attn.dtype)
        attn = self.attn_drop(attn)

        x = (attn @ v) + lepe
        B = int(x.shape[0] / (H * W / self.H_sp / self.W_sp))
        x = x.permute(0, 2, 1, 3).reshape(B, H // self.H_sp, W // self.W_sp, self.H_sp, self.W_sp, -1).permute(0, 1, 3, 2, 4, 5).reshape(B, -1, C)
        return x
...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        assert L == H * W, "flatten img_tokens has wrong size"
        img = self.norm1(x)
        qkv = self.qkv(img).reshape(B, -1, 3, C).permute(3, 2, 0, 1)
