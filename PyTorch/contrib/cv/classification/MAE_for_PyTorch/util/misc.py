...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        t = torch.tensor([self.count, self.total]).npu()
        dist.barrier()
        dist.all_reduce(t)
        t = t.tolist()
        self.count = int(t[0])
...
def init_distributed_mode(args):
    if args.dist_on_itp:
        ...
        args.npu = int(os.environ['OMPI_COMM_WORLD_LOCAL_RANK'])
        args.dist_url = "tcp://%s:%s" % (os.environ['MASTER_ADDR'], os.environ['MASTER_PORT'])
        os.environ['LOCAL_RANK'] = str(args.npu)
        ...
        args.npu = args.rank % args.world_size
    else:
...
# imitate timm.utils.ApexScaler(), add norm and update_grad
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        ...
        if update_grad:
            if clip_grad is not None:
                ...
                # norm = optimizer.clip_optimizer_grad_norm_fused(clip_grad)
            else:
                norm = get_grad_norm_(amp.master_params(optimizer))
                # norm = get_grad_norm_v2_(amp.master_params(optimizer), optimizer)
            optimizer.step()
        else:
            norm = None
        return norm
...
# imitate code from timm.utils.NativeScaler(), add norm and update_grad
...
def get_grad_norm_v2_(parameters, optimizers, norm_type=2):
    if isinstance(parameters, torch.Tensor):
        parameters = [parameters]
    norm_type = float(norm_type)
