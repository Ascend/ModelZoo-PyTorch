...
taskset -c $PID_START-$PID_END python -m torch.distributed.launch --nproc_per_node=8 --use_env main.py \
        ...
        --resume ${pth_path}  > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
...
# 最后一个迭代loss值，不需要修改
ActualLoss=`grep -a '* Acc@1' test/output/0/train_0.log|awk 'END {print}'|awk -F "loss" '{print $NF}'|awk -F " " '{print $1}'`
