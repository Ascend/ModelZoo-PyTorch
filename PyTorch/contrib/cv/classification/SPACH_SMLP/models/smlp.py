...
class sMLPBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_h_in = x.reshape([x.shape[0], x.shape[3], x.shape[1], x.shape[2]])
        x_w_in = x.reshape([x.shape[0], x.shape[2], x.shape[1], x.shape[3]])
        #x_h = self.proj_h(x.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        #x_w = self.proh_w(x.permute(0, 2, 1, 3).contiguous()).permute(0, 2, 1, 3).contiguous()
        #x_h = self.proj_h(x_h_in).view(x.shape)
        #x_w = self.proh_w(x_w_in).view(x.shape)
        x_h = self.proj_h(x_h_in).permute(0, 2, 3, 1).contiguous()
        x_w = self.proh_w(x_w_in).permute(0, 2, 1, 3).contiguous()
