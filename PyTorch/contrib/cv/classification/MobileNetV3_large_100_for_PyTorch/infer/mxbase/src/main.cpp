...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/mobilenetv3 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/MobileNetV3.om";
    auto mobilenetv3 = std::make_shared<Mobilenetv3Classify>();
    APP_ERROR ret = mobilenetv3->Init(initParam);
    if (ret != APP_ERR_OK) {
    ...
    for (auto & entry : fs::directory_iterator(binDir)) {
        ...
        ret = mobilenetv3->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "Mobilenetv3Classify process failed, ret=" << ret << ".";
            mobilenetv3->DeInit();
            return ret;
        }
    }
