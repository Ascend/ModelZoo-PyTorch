...
def train(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        images, target = images.to(loc, non_blocking=False), target.to(
            loc, non_blocking=False)
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                                                and args.rank % ngpus_per_node == 0):
        if batch_time.avg > 0:
            print("[npu id:", args.gpu, "]", '* FPS@all {:.3f}, TIME@all {:.3f}'.format(
                ngpus_per_node * args.batch_size / batch_time.avg, batch_time.avg))
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        ...
        shutil.copyfile(filename, 'model_best_acc%.4f_epoch%d.pth.tar' % (
            state['best_acc1'], state['epoch']))
