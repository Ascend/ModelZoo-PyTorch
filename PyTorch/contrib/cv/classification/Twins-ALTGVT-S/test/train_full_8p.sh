...
nohup python3.7 -m torch.distributed.launch --nproc_per_node=8 --use_env main.py \
              --model alt_gvt_small \
              --device npu \
              --batch-size 128 \
              --data-path $data_path \
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
grep "fps:" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk '{print $14}' | awk -F: '{print $2}' >> ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${CaseName}_fps.log
FPS=`cat ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${CaseName}_fps.log  | awk '$1>0'| awk '{a+=$1} END {if (NR != 0) printf("%.3f",a/NR)}'`
# 打印，不需要修改
