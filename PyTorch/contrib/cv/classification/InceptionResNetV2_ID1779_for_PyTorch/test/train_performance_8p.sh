...
# 网络名称，同目录名称
Network="InceptionResNetV2_ID1779_for_PyTorch"
# 训练batch_size
batch_size=1536
# 训练使用的npu卡数
export RANK_SIZE=8
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep 'batch_size'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep 'npu'|awk -F " " '{print $NF}'|awk 'END {print}'`
# 打印，不需要修改
echo "Final Performance images/sec : $FPS"

# 输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'Acc@1' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F " " '{print $8}'|awk 'END {print}'`
# 打印，不需要修改
