...
parser.add_argument('-a', '--arch', metavar='ARCH', default='InceptionResNetV2')
...
def main_worker(npu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        pretrained_dict = torch.load("./model_best.pthtar", map_location="cpu")["state_dict"]
...
def train(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                                                 and args.rank % ngpus_per_node == 0):
        print("[npu id:", args.npu, "]", "[batch_size:", args.batch_size, "]" ,
...
def validate(val_loader, model, criterion, args, ngpus_per_node):
    ...
    with torch.no_grad():
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            print("[batch_size:", args.batch_size, "]", '* FPS@all {:.3f}'.format(ngpus_per_node * args.batch_size / batch_time.avg))
