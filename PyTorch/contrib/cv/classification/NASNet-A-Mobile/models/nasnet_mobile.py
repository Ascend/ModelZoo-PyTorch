...
class CellStem1(nn.Module):
    ...
    def forward(self, x_conv0, x_stem_0):
        ...
        # path 2
        x_path2 = self.path_2.avgpool(x_relu)
...
class NASNetAMobile(nn.Module):
    ...
    def __init__(self, num_classes=1000, stem_filters=32, penultimate_filters=1056, filters_multiplier=2,
        ...
        if self.use_aux:
            self.aux = AuxiliaryHeadImageNet(704, self.num_classes)
    ...
    def features(self, input):
        ...
        if self.training and self.use_aux:
            aux_x = self.aux(x_reduction_cell_1)
...
def nasnetamobile(num_classes=1000, pretrained=None):
...
if __name__ == "__main__":
    ...
    model = NASNetAMobile(use_aux=False)
    input = Variable(torch.randn(256, 3, 224, 224))
    ...
    model = model.npu()
    input = input.npu()
    optimizer = NpuFusedSGD(model.parameters(), lr=1e-3)
    ...
    prof.export_chrome_trace("bei_output.prof") # "output.prof"为输出文件地址
