...
def main(config):
    ...
    # test the output in CPU and NPU
    # model_test(model)
    # return
    ...
    logger.info(f"Save internal {config.SAVE_FREQ}")
    FPS = 0
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        data_loader_train.sampler.set_epoch(epoch)
        FPS = FPS + train_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn,
                                    lr_scheduler)
        if dist.get_rank() == 0 and epoch != 0 and (
    ...
    logger.info(f"Average FPS {FPS / config.TRAIN.EPOCHS}")
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'date time {date_meter.val:.4f} ({date_meter.avg:4f})\t'
