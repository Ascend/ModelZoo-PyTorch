...
APP_ERROR SK_resnet50::Init(const InitParam &initParam) {
...
APP_ERROR SK_resnet50::DeInit() {
    model_->DeInit();
    DeviceManager::GetInstance()->DestroyDevices();
    return APP_ERR_OK;
}

APP_ERROR SK_resnet50::ReadTensorFromFile(const std::string &file, uint32_t *data, uint32_t size) {
...
APP_ERROR SK_resnet50::ReadInputTensor(const std::string &fileName,
...
APP_ERROR SK_resnet50::Inference(std::vector<TensorBase> &inputs, std::vector<TensorBase> &outputs) {
...
APP_ERROR SK_resnet50::PostProcess(const std::vector<TensorBase> &inputs,
...
APP_ERROR SK_resnet50::SaveResult(const std::string &imgPath, 
...
APP_ERROR SK_resnet50::Process(const std::string &imgPath) {
