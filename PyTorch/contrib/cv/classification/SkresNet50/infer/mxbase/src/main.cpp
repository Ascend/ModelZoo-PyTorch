...
int main(int argc, char* argv[]) {
   if (argc <= 1) {
    ...
    initParam.modelPath = "../../convert/sk_resnet50_npu_16.om";
    auto sk_resnet50 = std::make_shared<SK_resnet50>();
    APP_ERROR ret = sk_resnet50->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "sk_resnet50Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ret = sk_resnet50->Process(files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "sk_resnet50Classify process failed, ret=" << ret << ".";
            sk_resnet50->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * files.size() / sk_resnet50->GetInferCostMilliSec();
