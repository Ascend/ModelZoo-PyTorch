...
class Trainer:
    ...
    def _init_state(self) -> None:
        ...
        optimizer = apex.optimizers.NpuFusedSGD(model.parameters(), lr=linear_scaled_lr, momentum=0.9,weight_decay=1e-4)
        lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=30)
        model, optimizer = amp.initialize(model, optimizer, combine_grad=True, opt_level="O1")
        if self._train_cfg.num_tasks> 1:
