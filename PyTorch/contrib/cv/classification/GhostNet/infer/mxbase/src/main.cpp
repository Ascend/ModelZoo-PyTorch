...
int main(int argc, char* argv[])
{
    ...
    initParam.modelPath = "./../data/ghostnet.om";
    auto ghostnet = std::make_shared<GhostNetClassify>();
    ret = ghostnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "GhostNetClassify init failed, ret=" << ret << ".";
        return ret;
    }

    auto startTime = std::chrono::high_resolution_clock::now();
    for (auto &imgFile: imgFiles) {
        ret = ghostnet->Process(imgFile);
        if (ret != APP_ERR_OK) {
            LogError << "GhostNetClassify process failed, ret=" << ret << ".";
            ghostnet->DeInit();
            return ret;
        }
    }
    ...
    double fps = ratio * imgFiles.size() / ghostnet->GetInferCostTimeMs();
