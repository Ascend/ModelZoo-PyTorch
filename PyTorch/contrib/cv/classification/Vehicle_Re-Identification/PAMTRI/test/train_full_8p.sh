...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'mAP'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print}'|awk -F "mAP" '{print $NF}'|awk -F " " '{print $2}'`
#打印，不需要修改
