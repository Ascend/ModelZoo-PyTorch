...
# 输出训练精度,需要模型审视修改
train_accuracy=`tail -n 1 ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}.log | awk '{print $2}'`
#打印，不需要修改
...
# 最后一个迭代loss值，不需要修改
ActualLoss=`tail -n 1 ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}.log | awk '{print $1}'`
