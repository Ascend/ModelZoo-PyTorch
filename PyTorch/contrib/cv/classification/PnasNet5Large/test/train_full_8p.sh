...
ActualFPS=${FPS}
#单迭代训练时长
TrainingTime=`tail -n 1 ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $5}'`
...
#最后一个迭代loss值，不需要修改
ActualLoss=`tail -n 1 ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk '{print $3}'`
