...
__all__ = ['accuracy', 'AverageMeter', 'LR_Scheduler', 'mkdir', 'torch_dist_sum',
           'MixUpWrapper', 'save_checkpoint', 'cached_log_stream', 'PathManager']
...
def torch_dist_sum(gpu, device, *args):
    ...
    for arg in args:
        if isinstance(arg, torch.Tensor):
            if device == 'npu':
                tensor_arg = arg.clone().reshape(-1).to(torch.int32).detach().npu(gpu)
            else:
                tensor_arg = arg.clone().reshape(-1).detach().cuda(gpu)
        else:
            if device == 'npu':
                tensor_arg = torch.tensor(arg).reshape(-1).to(torch.int32).npu(gpu)
            else:
...
class LR_Scheduler(object):
    ...
    def __init__(self, mode, base_lr, num_epochs, iters_per_epoch=0,
                 lr_step=0, warmup_epochs=0, quiet=False, logger=None):
        self.mode = mode
        self.quiet = quiet
        self.logger = logger
        if not quiet:
...
class MixUpWrapper(object):
    ...
    def mixup_loader(self, loader):
        ######################################################################
        # mixup
        def mixup(alpha, num_classes, data, target, device):
            with torch.no_grad():
                bs = data.size(0)
                c = np.random.beta(alpha, alpha)
                if 'npu' in device:
                    perm = torch.randperm(bs).npu()
                else:
        ...
        for input, target in loader:
            if 'npu' in device:
                input = input.to(self.device, non_blocking=True).to(torch.float)
                target = target.to(self.device, non_blocking=True).to(torch.int32)
            else:
                input, target = input.cuda(self.device), target.cuda(self.device)
            target = torch.nn.functional.one_hot(target, self.num_classes)
            i, t = mixup(self.alpha, self.num_classes, input, target, self.device)
            yield i, t
