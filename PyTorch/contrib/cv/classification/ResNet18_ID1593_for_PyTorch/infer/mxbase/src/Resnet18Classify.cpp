...
APP_ERROR Resnet18Classify::Init(const InitParam &initParam) {
...
APP_ERROR Resnet18Classify::DeInit() {
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR Resnet18Classify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Resnet18Classify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Resnet18Classify::ReadTensorFromFile(const std::string &file, float *data) {
    if (data == NULL) {
...
APP_ERROR Resnet18Classify::ReadInputTensor(const std::string &fileName, std::vector<MxBase::TensorBase> *inputs) {
...
APP_ERROR Resnet18Classify::SaveResult(const std::string &imgPath, std::vector<std::vector<MxBase::ClassInfo>> \
                                        &BatchClsInfos) {
