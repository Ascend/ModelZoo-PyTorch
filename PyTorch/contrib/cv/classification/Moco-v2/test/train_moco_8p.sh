...
output_dir=${test_path_dir}/output/train_8p_moco
#################创建日志输出目录，不需要修改#################
mkdir -p ${output_dir}
output_log=${output_dir}/train_8p_moco.log
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'Acc@1' ${output_log}|awk -F " " '{print $15}'|awk 'END {print}'`
#打印，不需要修改
