...
def resize(img, size, interpolation=Image.BILINEAR):
    ...
    if isinstance(size, tuple):
        w, h = img.size
        if (w <= h and w == w2) or (h <= w and h == h2):
            return img
        if w < h:
            h2 = int(w2 * h / w)
            return img.resize((w2, h2), interpolation)
        else:
            w2 = int(h2 * w / h)
            return img.resize((w2, h2), interpolation)
    else:
...
def deepmar_onnx(in_file):
    ...
    std = [0.2285, 0.2235, 0.2245]

    img = Image.open(in_file).convert('RGB')
    img = resize(img, input_size)  # transforms.Resize(256)
    
    img = np.array(img, dtype=np.float32)
    img = center_crop(img, 299, 299)   # transforms.CenterCrop(224)
...
class Predictor():
    ...
    def _predict_gen_protobuf(self,name):
        ...
        img_np = deepmar_onnx(file_path)
        vision_list = MxpiDataType.MxpiVisionList()
        vision_vec = vision_list.visionVec.add()
        vision_vec.visionInfo.format = 0
        vision_vec.visionInfo.width = 299
        vision_vec.visionInfo.height = 299
        vision_vec.visionInfo.widthAligned = 299
        vision_vec.visionInfo.heightAligned = 299
...
def main():
    pipeline_conf = "./inceptionV3.pipeline"
