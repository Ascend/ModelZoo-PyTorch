...
# bnneck on
# with center loss
#taskset -c 0-24 python -m torch.distributed.launch --nproc_per_node=8 tools/train_8P.py --config_file='configs/8P_tri_with_cen.yml' --nodes=1 --npus=8 MODEL.DEVICE_ID "('0')" DATASETS.NAMES "('market1501')" DATASETS.ROOT_DIR "('/root/lmm/data')" OUTPUT_DIR "('./logs/E-a-t-m-256x128-bs16x4-warmup10-erase0_5-labelsmooth_on-laststride1-bnneck_on')"
#!/bin/bash
...
do
	...
	nohup taskset -c $PID_START-$PID_END python3 tools/train_8P.py \
		...
		OUTPUT_DIR "('./logs/E-a-t-m-256x128-bs16x4-warmup10-erase0_5-labelsmooth_on-laststride1-bnneck_on/8p')" > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done
wait
##################获取训练数据##################
# 训练结束时间，不需要修改
