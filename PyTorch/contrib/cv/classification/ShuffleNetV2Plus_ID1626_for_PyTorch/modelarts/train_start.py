...
current_path = os.path.dirname(os.path.realpath(__file__)) # BootfileDirectory, the directory where the startup files are located
project_root = os.path.dirname(current_path) # The root directory of the project, corresponding to the code directory set in the ModelArts Training Console
...
parser.add_argument('--train_url', default="/cache/train_url", type=str, help="setting dir of training output")
...
# ---------modelarts modification-----------------
def convert(pth_file, onnx_path):
...
def convert_pth_to_onnx():
    pth_pattern = os.path.join(CACHE_TRAIN_URL, 'checkpoint.pth.tar')
...
def main():
    ...
    if not os.path.exists(CACHE_TRAIN_URL):
        os.makedirs(CACHE_TRAIN_URL, 0o755)
    args.checkpoint_save_path = CACHE_TRAIN_URL
    # --------------modelarts modification end-----
    ...
    print("training output finish copy to %s." % CACHE_TRAIN_URL)
    # --------------modelarts modification end----------------
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader, train_loader_len, train_sampler = get_pytorch_train_loader(CACHE_DATA_URL,
                                                                             args.batch_size,
                                                                             workers=args.workers,
                                                                             distributed=args.distributed)

    val_loader = get_pytorch_val_loader(CACHE_DATA_URL, args.batch_size, args.workers, distributed=False)
    ...
    if args.resume:
        ...
        mox.file.copy_parallel(args.resume, os.path.join(CACHE_MODEL_URL, "checkpoint.pth.tar"))
        args.resume = os.path.join(CACHE_MODEL_URL, "checkpoint.pth.tar")
        # ------------------modelarts modification---------------------
        if os.path.isfile(args.resume):
            if args.pretrained:
                pretrained_dict = \
                torch.load(args.resume, map_location="cpu")["state_dict"]
                if "module.fc.0.weight" in pretrained_dict:
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    # --------------modelarts modification---------
    file_path = os.path.join(CACHE_TRAIN_URL, filename)
    torch.save(state, file_path)
    if is_best:
        shutil.copyfile(file_path, os.path.join(CACHE_TRAIN_URL, 'model_best_acc%.4f_epoch%d.pth.tar' % (state['best_acc1'], state['epoch'])))
    # --------------modelarts modification end-----
