...
parser.add_argument('--output', default="",metavar='DIR',
                    help='pth和onnx文件')
parser.add_argument('--onnx', default=True, type=bool, metavar='M',
                    help='momentum')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        # model = resnet_0_6_0.resnet152(pretrained=True)
        model = resnet_0_6_0.resnet152(pretrained=False, progress=True)
    ...
    pth_path = '/cache/output'
    if not os.path.exists(pth_path):
        os.makedirs(pth_path)
    mox.file.copy_parallel(args.data, real_path)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.device == 'npu' and args.gpu == 0 and epoch == 4:
            print("Complete 4 epoch training, take time:{}h".format(round((time.time() - start_time) / 3600.0, 2)))
        # pth_path
        pth2onnx_path = args.output
        print(pth2onnx_path)
        ...
        ##############    pth2onnx   ##############
    mox.file.copy_parallel(pth_path, pth2onnx_path)
    CACHE_TRAINING_URL = "/cache/training"
    if args.onnx:
        os.makedirs(CACHE_TRAINING_URL, exist_ok=True)
        print("abspath:", os.path.abspath(pth_path))
        print("output_dir:", os.listdir(pth_path))
        shutil.copy(os.path.join(os.path.abspath(pth_path), 'model_best.pth.tar'), CACHE_TRAINING_URL)
        ...
        mox.file.copy_parallel(CACHE_TRAINING_URL, pth2onnx_path)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    filename_check = os.path.join(filename, 'checkpoint.pth.tar')
    print("ahead save")
    torch.save(state, filename_check)
    print("ahead best")
    if is_best:
        filename_best = os.path.join(filename, 'model_best.pth.tar')
        shutil.copyfile(filename_check, filename_best)
