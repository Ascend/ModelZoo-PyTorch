...
cp 8p_eval.log ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log
...
# 输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'top1' ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${ASCEND_DEVICE_ID}.log/|awk 'END {print}'|awk -F " " '{print $4}'|awk -F ")" '{print $1}'`
#打印，不需要修改
