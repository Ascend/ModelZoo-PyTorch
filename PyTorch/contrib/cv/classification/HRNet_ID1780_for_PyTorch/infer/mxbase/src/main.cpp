...
int main(int argc, char* argv[]) {
    ...
    initParam.modelPath = "../data/model/hrnet.om";
    auto hrnet = std::make_shared<HRNetClassify>();
    APP_ERROR ret = hrnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "HRNetClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(imgDir)) {
        ...
        ret = hrnet->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "HRNetClassify process failed, ret=" << ret << ".";
            hrnet->DeInit();
            return ret;
        }
    }
