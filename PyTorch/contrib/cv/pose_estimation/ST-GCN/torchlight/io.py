...
class IO():
    ...
    def print_log(self, str, print_time=True):
        ...
        if self.print_to_screen:
            print(str)
        if self.save_log:
            with open('{}/stgcn_{}p.log'.format(self.work_dir, int(os.environ['world_size'])), 'a') as f:
                print(str, file=f)
    ...
    def print_timer(self):
        proportion = {
            k: '{:02d}%'.format(
                int(round(v * 100 / sum(self.split_timer.values()))))
        ...
        for k in proportion:
            self.print_log(
                '\t[{}][{}]: {:.4f}'.format(
                    k, proportion[k], self.split_timer[k])
            )
