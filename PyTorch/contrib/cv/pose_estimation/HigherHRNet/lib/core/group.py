...
class HeatmapParser(object):
    ...
    def nms(self, det):
        # print(torch.npu.synchronize(), "1")
        maxm = self.pool(det.cpu())
        # print(torch.npu.synchronize(), "2")
        maxm = torch.eq(maxm.npu(), det).float()
        # print(torch.npu.synchronize(), "3")
        det = det * maxm
        return det
    ...
    def top_k(self, det, tag):
        ...
        #print("**************enter top_k**************")
        det = self.nms(det)
        #print(torch.npu.synchronize(), "nms")
        num_images = det.size(0)
        num_joints = det.size(1)
        h = det.size(2)
        w = det.size(3)
        det = det.view(num_images, num_joints, -1)
        #print(torch.npu.synchronize(), "view")
        det=det.cpu()
        val_k, ind = det.topk(self.params.max_num_people, dim=2)
        val_k=val_k.npu()
        ind=ind.npu()
        #print(torch.npu.synchronize(), "val_k", val_k.shape, val_k)
        tag = tag.view(tag.size(0), tag.size(1), w*h, -1)
        #print(torch.npu.synchronize(), "4")
        if not self.tag_per_joint:
            tag = tag.expand(-1, self.params.num_joints, -1, -1)
        #print(torch.npu.synchronize(), "5", tag.shape)
        """
        tmp = []
        for i in range(tag.size(3)):
            tmp.append(torch.gather(tag[:, :, :, i], 2, ind))
        tag_k = torch.stack(tmp,dim=3)
        """
        tag=tag.cpu()
        ind=ind.cpu()
        tag_k = torch.stack(
        ...
        tag_k=tag_k.npu()
        #print(torch.npu.synchronize(), "6")
        #print(torch.npu.synchronize(), "tag_k", tag_k.shape, tag_k)
        x = ind % w
        y = (ind / w).long()

        ind_k = torch.stack((x, y), dim=3)
        #print(torch.npu.synchronize(), "ind_k", ind_k.shape, ind_k)
