...
def save_batch_image_with_joints_multi(batch_image,
    ...
    for y in range(ymaps):
        for x in range(xmaps):
            if k >= nmaps:
                break
            for n in range(num_person[k].int()):
...
def save_debug_3d_images(config, meta, preds, prefix, is_master_node=False):
    if not config.DEBUG.DEBUG:
        return
    ...
    for i in range(batch_size):
        ...
        if preds is not None:
            # if is_master_node:
            #     print("preds is not none",preds[i])
            pred = preds[i]
            for n in range(len(pred)):
                ...
                if joint[0, 3] >= 0:
                    # if is_master_node:
                    #     print("jinlezhelicaisuanshengli")
                    for k in eval("LIMBS{}".format(len(joint))):
                        x = [float(joint[k[0], 0]), float(joint[k[1], 0])]
                        y = [float(joint[k[0], 1]), float(joint[k[1], 1])]
                        z = [float(joint[k[0], 2]), float(joint[k[1], 2])]
...
def save_debug_3d_cubes(config, meta, root, prefix):
    ...
    for i in range(batch_size):
        ...
        tmp=num_person.int()
        x = roots_gt[:tmp, 0].cpu()
        y = roots_gt[:tmp, 1].cpu()
        z = roots_gt[:tmp, 2].cpu()
