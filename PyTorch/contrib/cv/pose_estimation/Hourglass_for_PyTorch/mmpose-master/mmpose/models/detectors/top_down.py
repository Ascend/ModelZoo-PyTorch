...
class TopDown(BasePose):
    ...
    def forward_train(self, img, target, target_weight, img_metas, **kwargs):
        ...
        img = img.npu()
        target = target.npu()
        target_weight = target_weight.npu()
        ...
        if isinstance(output, list):
            ...
            for i in range(len(output)):  # go in there---xtl
                if target.dim() == 5 and target_weight.dim() == 4:
                    target_i = target[:, i, :, :, :]
                    target_weight_i = target_weight[:, i, :, :]
                else:  # go in there---xtl
                    target_i = target
                ...
                if isinstance(self.loss, nn.Sequential):
                    ...
                    loss_func = self.loss
                # loss_i = 100000000 * \
                #     loss_func(output[i], target_i, target_weight_i)
                loss_i = loss_func(output[i], target_i, target_weight_i)
                if 'loss_weights' in self.train_cfg and self.train_cfg[
                        'loss_weights']:  # not go in there--xtl
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        if isinstance(img_metas, mmcv.parallel.data_container.DataContainer):
