...
cd mmpose-master
./tools/dist_train.sh ./configs/top_down/hourglass/mpii/hourglass52_mpii_384x384.py ${RANK_SIZE} --npus=${RANK_SIZE} > ${test_path_dir}/output/${RANK_SIZE}/train_${RANK_SIZE}.log 2>&1 &
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep -a 'FPS'  ${test_path_dir}/output/${RANK_SIZE}/train_${RANK_SIZE}.log|awk -F " " '{print $11}'|awk 'END {print}'`
#打印，不需要修改
...
#从train_$RANK_SIZE.log提取Loss到train_${CaseName}_loss.txt中，需要根据模型审视
grep Epoch: ${test_path_dir}/output/$RANK_SIZE/train_$RANK_SIZE.log|grep -v Test|awk -F "Loss" '{print $NF}' | awk -F " " '{print $1}' >>  ${test_path_dir}/output/$RANK_SIZE/train_${CaseName}_loss.txt

#最后一个迭代loss值，不需要修改
ActualLoss=`awk 'END {print}'  ${test_path_dir}/output/$RANK_SIZE/train_${CaseName}_loss.txt`
...
echo "ActualFPS = ${ActualFPS}" >>  ${test_path_dir}/output/$RANK_SIZE/${CaseName}.log
echo "TrainingTime = ${TrainingTime}" >>  ${test_path_dir}/output/$RANK_SIZE/${CaseName}.log
