...
def synthetic_train(config, synthetic_train_loader, real_train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(synthetic_train_loader):
        ...
        target_weight = target_weight.npu(non_blocking=True)
...
# --------------------------------------------------------------------------------
def train(args, config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        if not args.is_distributed and args.perf:
            if i > 600:
                break
        data_time.update(time.time() - end)
        ...
        _, avg_acc, cnt, pred = accuracy(output.detach(),
                                         target.detach())
        ...
        if i % config.PRINT_FREQ == 0:
            if world_size > 1:
                ...
                reduced_acc = reduce_tensor(avg_acc, world_size)
                losses.update(reduced_loss.item(), input.size(0))
                acc.update(reduced_acc.item(), cnt)
            ...
            if dist.get_rank() == 0:
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                    ...
                    'Accuracy {acc.val:.3f} ({acc.avg:.3f})'.format(
                        ...
                        speed=input.size(0) * world_size / batch_time.val,
                        data_time=data_time, loss=losses, acc=acc)
                logger.info(msg)
...
#FAQ: 多卡test 最后面的打印精度值很小
def validate_mulitp(config, val_loader, val_dataset, model, criterion, output_dir,
             tb_log_dir, writer_dict=None, epoch=-1, print_prefix=''):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    num_samples = len(val_loader) * config.TEST.BATCH_SIZE_PER_GPU
    all_preds = np.zeros(
        (num_samples, config.MODEL.NUM_JOINTS, 3),
        dtype=np.float32
    )
    ...
    imgnums = []
    idx = 0
    print("validate enter, ",dist.get_rank())
    with torch.no_grad():
        end = time.time()
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            # compute output
            ...
            if world_size <= 1:
                ...
                reduced_acc = avg_acc.data
            losses.update(reduced_loss.item(), num_images)
            acc.update(reduced_acc.item(), cnt)
        ...
        if writer_dict:
            if dist.get_rank() == 0:
                ...
                writer_dict['valid_global_steps'] = global_steps + 1
    perf_indicator_ten = torch.as_tensor(perf_indicator, dtype=torch.float, device=input.device)
    perf_indicator = reduce_tensor(perf_indicator_ten, world_size)
