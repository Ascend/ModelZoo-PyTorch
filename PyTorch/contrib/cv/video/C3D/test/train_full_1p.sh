...
# 输出训练精度,需要模型审视修改
train_accuracy=`grep 'Epoch(val)' ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk -F ' ' '{print $11}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
...
grep "grad_norm" ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk '{print $(NF-2)}' | awk -F ',' '{print $1}' > ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
