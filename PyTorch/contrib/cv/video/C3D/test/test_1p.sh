...
python3 tools/test.py configs/recognition/c3d/c3d_sports1m_16x1x1_45e_ucf101_rgb_1p.py  ${eval_file} --eval top_k_accuracy > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/test_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
##################获取训练数据################
# 训练结束时间，不需要修改
end_time=$(date +%s)
