...
python run.py -e ${train_epochs} -k cpn_ft_h36m_dbb -arc 3,3,3,3,3 -c checkpoint/1p_lr5e-4 -o test/output/1p_lr5e-4 -lr 0.0005 --log log/1p_lr5e-4 --sampler --device-list '0'
...
echo "------------------ Final result ------------------"
# output FPS
FPS=`grep -a 'FPS'  ${test_path_dir}/output/1p_lr5e-4/train_rank0.log|awk -F " " '{print $15}'|awk 'END {print}'`
# print
echo "Final Performance images/sec : $FPS"

# train-accuracy
acc=`grep -a 'Protocol #1'  ${test_path_dir}/output/1p_lr5e-4/train_rank0.log|awk 'END {print}'|awk -F " " '{print $7}'`
# print
...
grep -a 'FPS' ${test_path_dir}/output/1p_lr5e-4/train_rank0.log|awk -F " " '{print $3,$4,$5}'|awk -F "loss:" '{print $NF}'|awk -F "time" '{print $1}'|awk -F "(" '{print $NF}'|awk -F ")" '{print $1}' >> ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
