...
python run.py -e ${train_epochs} -k cpn_ft_h36m_dbb -arc 3,3,3,3,3 -c checkpoint/1p_lr5e-4_perf -o test/output/1p_lr5e-4_perf -lr 0.0005 --log log/1p_lr5e-4_perf --sampler --no-eval --device-list '0'
...
echo "------------------ Final result ------------------"
# output FPS
FPS=`grep -a 'FPS'  ${test_path_dir}/output/1p_lr5e-4_perf/train_rank0.log|awk -F " " '{print $11}'|awk 'END {print}'`
# print
