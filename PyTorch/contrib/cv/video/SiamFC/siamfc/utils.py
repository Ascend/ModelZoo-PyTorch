...
# model_sz=127, original_sz->model_sz
def crop_and_pad(img, cx, cy, model_sz, original_sz, img_mean=None):
    ...
    if left != 0 or right != 0 or top != 0 or bottom != 0:
        ...
        im_patch = cv2.copyMakeBorder(im_patch, top, bottom, left, right, cv2.BORDER_CONSTANT, value=img_mean)
...
# s-x: original size, size-x: final size, size-x-scales: apply multi-scale
def get_pyramid_instance_image(img, center, size_x, size_x_scales, img_mean=None):
    ...
    pyramid = [crop_and_pad(img, center[0], center[1], size_x,
                            size_x_scale, img_mean) for size_x_scale in size_x_scales]
    return pyramid
