...
taskset -c $PID_START-$PID_END python3 -u ./bin/my_train.py \
	...
	--local_rank $RANK_ID \
	--world_size 8 \
	--epoch ${train_epochs} > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log &
...
grep EPOCH ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep -v Test|awk -F "train_loss" '{print $NF}' | awk -F " " '{print $2}' >> ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${Network}_bs${BatchSize}_${RANK_SIZE}'p'_loss.txt

# 最后一个迭代loss值，不需要修改
ActualLoss=`awk 'END {print}' ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_${Network}_bs${BatchSize}_${RANK_SIZE}'p'_loss.txt`
