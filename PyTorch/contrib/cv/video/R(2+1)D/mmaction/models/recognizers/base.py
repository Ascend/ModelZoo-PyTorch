...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def extract_feat(self, imgs):
        ...
        if (hasattr(self.backbone, 'features')
            ...
            self.backbone = self.backbone.npu()
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        #print('--------------------base---------------------')
        #print(str(imgs.shape)+'-------'+str(imgs.dtype))
        #print(str(label.shape)+'-------'+str(label.dtype))
        # print(self)

        # self_cpu = self.npu().float()
        # imgs_cpu = imgs.npu().float()
        # label_cpu = label.npu().float()
        # losses = self_cpu(imgs,label,return_loss=True,**aux_info)
