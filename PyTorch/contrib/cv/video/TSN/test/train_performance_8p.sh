...
# 输出训练精度,需要模型审视修改
train_accuracy=`grep 'top1_acc'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/tsn_train_per_8p_${ASCEND_DEVICE_ID}.log | awk -F 'top' '{print $2}' | awk -F ': ' '{print $2}' | awk -F ',' '{print $1}' | awk 'BEGIN{max = 0}{if($1 > max) max = $1}END{print max}'`
# 打印，不需要修改
