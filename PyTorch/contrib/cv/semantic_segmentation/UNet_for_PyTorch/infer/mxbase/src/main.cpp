...
int main(int argc, char *argv[]) {
    ...
    auto unet = std::make_shared<UNet>();
    APP_ERROR ret = unet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "UNet init failed, ret=" << ret << ".";
        return ret;
    }

    std::string path;
    path =input_data_path+"/images/";

    std::vector<std::string> files;
    ret = ReadFilesFromPath(path, &files);
    if (ret != APP_ERR_OK) {
     ...
     for (uint32_t j = 0; j < files.size(); j++) {//uint32_t
            ...
            ret = unet->Process(path, files[j]);
            if (ret != APP_ERR_OK) {
                LogError << "UNet process failed, ret=" << ret << ".";
                unet->DeInit();
                return ret;
           }
       }
