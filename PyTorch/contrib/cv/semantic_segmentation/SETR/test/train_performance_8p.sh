...
train_log_dir=${currentDir}/work_dirs/training_performance_npu_8p_job_${currtime}
...
if [ $is_resume -gt 0 ]; then
        echo "正在拉起训练"
        resume_work_dirs=${currentDir}/${resume_work_dirs}
...
#输出训练精度,需要模型审视修改
# mIoU=`grep -a 'global ' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "global " '{print $NF}'|awk -F " " 'END {print $1}'`
# echo "mIoU : $mIoU"
#打印，不需要修改
