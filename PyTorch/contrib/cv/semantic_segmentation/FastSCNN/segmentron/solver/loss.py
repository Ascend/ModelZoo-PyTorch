...
class MixSoftmaxCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def _aux_forward(self, *inputs, **kwargs):
        *preds, target = tuple(inputs)
        ...
        #print("prefs[0] shape 0:", preds[0].shape)
        #print("target shape 0:", target.shape)
        output_shape = preds[0].shape
        target_shape = target.shape
        preds[0] = preds[0].permute(0, 2, 3, 1)
        preds[0] = preds[0].reshape(output_shape[0]*output_shape[2]*output_shape[3], output_shape[1])
        target = target.reshape(target_shape[0]*target_shape[1]*target_shape[2])
        #print("prefs[0] shape 1:", preds[0].shape)
        #print("target shape 1:", target.shape)
        loss = super(MixSoftmaxCrossEntropyLoss, self).forward(preds[0], target)
        for i in range(1, len(preds)):
            output_shape = preds[i].shape
            preds[i] = preds[i].permute(0, 2, 3, 1)
            preds[i] = preds[i].reshape(output_shape[0]*output_shape[2]*output_shape[3], output_shape[1])
