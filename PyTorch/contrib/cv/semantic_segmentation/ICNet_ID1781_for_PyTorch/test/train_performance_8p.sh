...
# 网络名称，同目录名称
Network="ICNet_ID1781_for_PyTorch"
# 训练batch_size
batch_size=1024
# 训练使用的npu卡数
export RANK_SIZE=8
...
# 获取pth文件传入使用服务器对应路径下
if [ -f ${more_path1}/resnet50-25c4b509.pth ];then
...
CONFIG_YAML_PATH=./configs/icnet_8p.yaml
RANK_ID_START=0
RANK_SIZE=8
...
nohup taskset -c $PID_START-$PIN_END \
        python3 train.py \
            ...
            $ADDR \
            $RANK_ID \
            $WORKERS_NUM  > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep -a 'Epochs'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F " " '{print $NF}'|awk 'END {print}'`
# 打印，不需要修改
