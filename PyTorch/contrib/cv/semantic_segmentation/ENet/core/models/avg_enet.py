...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsamping:
            '''
            if self.npu:
                identity = x.cpu().to(torch.float32)
                identity, max_indices = self.maxpool(identity)
                identity = identity.npu().to(torch.float16)
            else:
                identity, max_indices = self.maxpool(identity)
            '''
...
class UpsamplingBottleneck(nn.Module):
    ...
    def forward(self, x):#, max_indices):
        ...
        '''
        if self.npu:
            out_up = out_up.to(torch.float32)
            max_indices = max_indices.to(torch.int64).npu()
            out_up = self.upsampling(out_up, max_indices).to(torch.half)
        else:
            out_up = self.upsampling(out_up, max_indices)
        '''
        out_up = self.upsampling(out_up)
