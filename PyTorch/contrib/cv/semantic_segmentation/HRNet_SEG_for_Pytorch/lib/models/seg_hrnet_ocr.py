...
class SpatialGather_Module(nn.Module):
    ...
    def forward(self, feats, probs):
        ...
        probs = F.softmax(self.scale * probs.cpu(), dim=2)
        ocr_context = torch.matmul(probs.npu(), feats)\
...
class SpatialOCR_Module(nn.Module):
    ...
    def __init__(self, 
        ...
        self.object_context_block = ObjectAttentionBlock2D(in_channels,   # 512
                                                           key_channels,  # 256 
                                                           scale, 
                                                           bn_type)
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, device, **kwargs):
        ...
        self.ocr_distri_head = SpatialOCR_Module(in_channels=ocr_mid_channels,   # 512
                                                 key_channels=ocr_key_channels,  # 256
                                                 out_channels=ocr_mid_channels,  # 512
                                                 scale=1,
                                                 dropout=0.05,
                                                 )
    ...
    def init_weights(self, pretrained='',):
        ...
        if os.path.isfile(pretrained):
            if self.device == 'cuda':
                ...
                pretrained_dict = torch.load(pretrained, map_location={'npu:0': 'cpu'})
...
def get_seg_model(cfg, device, **kwargs):
    model = HighResolutionNet(cfg, device, **kwargs)
