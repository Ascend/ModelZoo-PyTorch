...
python3 ./tools/train.py\
    ...
    --options model.pretrained=$data_path/swin_small_patch4_window7_224.pth data.train.dataset.data_root=$data_path runner.max_iters=1000 \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_time=`cat ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | grep '300/1000'| awk -F " " '{print substr($15,0,length($15)-1)}'| awk 'END {print}'`
