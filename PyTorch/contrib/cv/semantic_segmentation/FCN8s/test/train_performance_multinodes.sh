...
NODE_RANK=$node_rank \
ADDR=$master_addr \
PORT=$master_port ./tools/dist_train.sh ./configs/fcn/fcn_r50-d8_512x512_20k_voc12aug.py 8 \
    --work-dir=${cur_path}/output/${Network}/$ASCEND_DEVICE_ID/ckpt \
    --device="npu" \
    --amp \
    --options data_root=${data_path} data.train.data_root=${data_path} data.val.data_root=${data_path} data.test.data_root=${data_path} data.samples_per_gpu=${batch_size} \
    runner.max_iters=1000 checkpoint_config.interval=1000 evaluation.interval=1000 log_config.interval=50 \
...
FPS=`echo "${FPS} * ${RANK_SIZE} * ${nnodes}" |bc`
