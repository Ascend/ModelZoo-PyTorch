...
def parse_args():
    ...
    parser.add_argument("--seed_val", default=1, type=int)
...
def main():
    ...
    os.makedirs(CACHE_TRAIN_URL + '%s' % config['name'], exist_ok=True)
    ...
    if config['pretrained']:
        checkpoint = torch.load(config['pretrained_weight_path'], map_location='cpu')
    ...
    mox.file.copy_parallel(config['data_url'], root_path)

    img_ids = glob(os.path.join(root_path, config['dataset'], 'images', '*' + config['img_ext']))
    ...
    train_dataset = Dataset(
        ...
        img_dir=os.path.join(root_path, config['dataset'], 'images'),
        mask_dir=os.path.join(root_path, config['dataset'], 'masks'),
    ...
    mox.file.copy_parallel(CACHE_TRAIN_URL, config['train_url'])


def save_checkpoint(state, is_best, epoch):
    c_filename = "checkpoint.pth.tar"
    b_filename = CACHE_TRAIN_URL + "model_best.pth.tar"
...
def convert(model_path):
    checkpoint = torch.load(CACHE_TRAIN_URL + "model_best.pth.tar", map_location='cpu')
    checkpoint['state_dict'] = proc_node_module(checkpoint, 'state_dict')
    model = archs.__dict__["NestedUNet"](1, 3)
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()
    print(model)
    ...
    torch.onnx.export(model, dummy_input, os.path.join(model_path, "NestedUNet_npu8p.onnx"), input_names=input_names, output_names=output_names,
                      opset_version=11)
