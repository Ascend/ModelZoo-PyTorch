...
def set_random_seed(seed, deterministic=False, device='cuda'):
    ...
    Args:
        ...
        deterministic (bool): Whether to set the deterministic option for
            ...
            Default: False.
        device (str): which device the model is running on. 'cuda' or 'npu'
            Default: 'cuda'
    """
    random.seed(seed)
    np.random.seed(seed)
...
def train_segmentor(model,
    ...
    meta['device_ids'] = [torch.cuda.current_device()] if meta['device'] == 'cuda' else [torch.npu.current_device()]
