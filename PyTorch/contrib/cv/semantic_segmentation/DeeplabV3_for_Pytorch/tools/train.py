...
def main():
    ...
    # registry apex.optimizer NpuFusedSGD
    if cfg.optimizer['type'] in ['NpuFusedSGD']:
    ...
    # use to compute FPS
    meta['bs'] = cfg.data.samples_per_gpu
    if cfg.device_num is not None:
        meta['num_devices'] = cfg.device_num
    elif distributed:
        meta['num_devices'] = torch.cuda.device_count() if args.device == 'cuda' \
                                                        else torch.npu.device_count()
    else:
        meta['num_devices'] = len(cfg.gpu_ids)
