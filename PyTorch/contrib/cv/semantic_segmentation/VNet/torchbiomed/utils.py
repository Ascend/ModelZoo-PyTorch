...
def resample_volume(img, spacing_old, spacing_new, bounds=None):
    ...
    resize_factor = np.array(spacing_old) / np.array(spacing_new)
...
def save_updated_image(img_arr, path, origin, spacing, useCompression=False):
    ...
    sitk.WriteImage(itk_scaled_img, path, useCompression=useCompression)
