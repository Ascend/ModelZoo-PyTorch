...
def balanced_mask_loss_ce(mask, pseudo_gt, gt_labels, ignore_index=255):
    ...
    #mask_gt = torch.argmax(pseudo_gt.cpu(), 1)
    mask_gt = torch.argmax(pseudo_gt.cpu(), 1)
    mask_gt = mask_gt.npu()
    ...
    # BCE loss
    loss = F.cross_entropy(mask.cpu(), mask_gt.cpu(), ignore_index=ignore_index, reduction="none")
    loss = loss.npu()
    loss = loss.view(bs, -1)
...
def network_factory(cfg):
    ...
    class SoftMaxAE(backbone):
        ...
        def forward(self, y, y_raw=None, labels=None):
            ...
            return cls, cls_fg, {"cam": masks, "dec": masks_dec}, self._mask_logits, pseudo_gt
