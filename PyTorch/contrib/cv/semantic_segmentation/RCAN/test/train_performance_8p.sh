...
batch_size=1280 # 训练batch_size，多卡可以设置为1280，单卡设置为160
...
train_accuracy=`grep -a 'PSNR:' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "PSNR:" '{print $NF}' | awk -F " " '{print $1}' | awk 'END {print}'`
fi
