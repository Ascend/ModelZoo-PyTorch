...
--lr 8e-4 \
--num-epochs 150 \
--rank_id $RANK_ID > erfnet_8p.log 2>&1 &
done
wait
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep 'now epoch' erfnet_8p.log|tail -n 1|awk '{print $7}'|awk 'END {print}'`
FPS=${FPS%,*}
# 打印，不需要修改
echo "Final Performance images/sec : $FPS"

# 输出训练精度,需要模型审视修改
iou=`grep 'EPOCH IoU' erfnet_8p.log|tail -n 1|awk '{print $6}'|awk 'END {print}'`
# 打印，不需要修改
