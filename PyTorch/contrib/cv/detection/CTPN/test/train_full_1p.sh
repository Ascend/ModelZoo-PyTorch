...
python3 train.py \
        ...
        --world-size 1 \
        --data-path=${data_path} \
        --batch-size=1 >${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
...
echo "------------------ Final result ------------------"
# 单迭代训练时长
OverallFPS=`grep -a 'overallFPS after training:'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print}' | awk -F "overallFPS after training:" '{print $NF}'| awk -F " " '{print $1}'`
# 输出性能FPS，需要模型审视修改
FPS=`awk 'BEGIN{printf "%.2f\n", '${OverallFPS}'}'`
# 打印，不需要修改
echo "Final Performance FPS : ${OverallFPS}"
