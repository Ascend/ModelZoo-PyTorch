...
class S3FD(nn.Module):
    ...
    def __init__(self, phase, base, extras, head, num_classes, use_device):
    ...
    def forward(self, x):
        ...
        if self.use_device == 'npu':
            self.priors = Variable(self.priorbox.forward().npu(), volatile=True)
        else:
            self.priors = Variable(self.priorbox.forward(), volatile=True)
        ...
        if self.phase == 'test':
            if self.use_device != 'npu':
                ...
                print("use:.npu()")
                output = self.detect.forward(
                    loc.view(loc.size(0), -1, 4).npu(),  # loc preds
                    self.softmax(conf.view(conf.size(0), -1,
                                           self.num_classes)).npu(),  # conf preds
                    self.priors.type(type(x.data)).npu(),  # default boxes
                    self.use_device
                ).npu()
...
def build_s3fd(phase, num_classes=2, use_device='gpu'):
    """
    build s3fd model
    """
    print(use_device)
    ...
    return S3FD(phase, base_, extras_, head_, num_classes, use_device)
