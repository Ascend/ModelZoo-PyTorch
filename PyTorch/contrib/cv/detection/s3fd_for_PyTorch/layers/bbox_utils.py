...
def intersect(box_a, box_b):
    ...
    # print("box_a.dtype:")
    # print(box_a.dtype)
    # print("box_a.storage().npu_format():")
    # print(box_a.storage().npu_format())
    ...
    # print("box_b.dtype:")
    # print(box_b.dtype)
    # print("box_b.storage().npu_format():")
    # print(box_b.storage().npu_format())
...
def nms(boxes, scores, overlap=0.5, top_k=200, use_device='gpu'):
    ...
    while idx.numel() > 0:
        ...
        if use_device == 'npu':
            xx1 = torch.clamp(xx1, min=x1[i].npu().data)
            yy1 = torch.clamp(yy1, min=y1[i].npu().data)
            xx2 = torch.clamp(xx2, max=x2[i].npu().data)
            yy2 = torch.clamp(yy2, max=y2[i].npu().data)
        else:
