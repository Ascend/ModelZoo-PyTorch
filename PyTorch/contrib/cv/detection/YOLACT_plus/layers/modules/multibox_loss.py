...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, net, predictions, wrapper, wrapper_mask):
        ...
        if cfg.train_boxes:
            ...
            loc_p = loc_data.view(-1, 4)
            loc_t = loc_t.view(-1, 4)
            losses['B'] = F.smooth_l1_loss(loc_p, loc_t, reduction='none') * cfg.bbox_alpha
            losses['B'][pos_idx.view(-1, 4) == False] = 0
            losses['B'] = losses['B'].sum()
        ...
        for idx in range(batch_size):
            ...
            with torch.no_grad():
                ...
                for obj_idx in range(downsampled_masks.size(0)):
                    segment_t[cur_class_t[obj_idx]] = torch.max(segment_t[cur_class_t[obj_idx]].float(),
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        loss_c = F.cross_entropy(conf_data.view(-1, self.num_classes), conf_t.view(-1), reduction='none').view(-1,
                                                                                                               57744)
        loss_c[(pos + neg).gt(0) == False] = 0
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data,
