...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if undo_transform:
        ...
        if args.cuda:
            img_gpu = torch.Tensor(img_numpy).npu()
        else:
            img_gpu = torch.Tensor(img_numpy)
        print('img_shape:', img_gpu)
    else:
        img_gpu = img / 255.0
        h, w, _ = img.shape
        print('h, w, _ :', img.shape)
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    if not args.output_coco_json:
        with timer.env('Prepare gt'):
            gt_boxes = torch.Tensor(gt[:, :4]) if args.cuda else torch.Tensor(gt[:, :4])
            gt_boxes[:, [0, 2]] *= w
            gt_boxes[:, [1, 3]] *= h
            gt_classes = list(gt[:, 4].astype(int))
            gt_masks = torch.Tensor(gt_masks).view(-1, h*w) if args.cuda else torch.Tensor(gt_masks).view(-1, h*w)
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    if args.cuda:
        frame = torch.from_numpy(cv2.imread(path)).npu().float()
    else:
        frame = torch.from_numpy(cv2.imread(path)).float()
    print('frame: ', frame)
    batch = FastBaseTransform()(frame.unsqueeze(0))
    print('batch_pred:', batch)
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    net = CustomDataParallel(net).npu()
    transform = torch.nn.DataParallel(FastBaseTransform()).npu()
    ...
    def transform_frame(frames):
        with torch.no_grad():
            frames = [torch.from_numpy(frame).npu().float() for frame in frames]
...
def evaluate(net:Yolact, dataset, train_mode=False, trainCuda = True):
    ...
    try:
        ...
        for it, image_idx in enumerate(tqdm(dataset_indices)):
            ...
            with timer.env('Load Data'):
                ...
                if args.cuda:
                    if train_mode:
                        batch = batch.npu() if trainCuda else batch
                    else:
                        batch = batch.npu()
            ...
            if preds is not None and preds[0] is not None:
                if preds[0]['box'] is not None:
                    preds[0]['box'] = preds[0]['box'].cpu()
                if preds[0]['mask'] is not None:
                    preds[0]['mask'] = preds[0]['mask'].cpu()
                if preds[0]['class'] is not None:
                    preds[0]['class'] = preds[0]['class'].cpu()
                if preds[0]['score'] is not None:
                    preds[0]['score'] = preds[0]['score'].cpu()
                if preds[0]['proto'] is not None:
                    preds[0]['proto'] = preds[0]['proto'].cpu()
...
if __name__ == '__main__':
    ...
    if args.data_path:
        cfg.dataset.valid_images = args.data_path + '/images/'
        cfg.dataset.valid_info = args.data_path + '/annotations/instances_val2017.json'
