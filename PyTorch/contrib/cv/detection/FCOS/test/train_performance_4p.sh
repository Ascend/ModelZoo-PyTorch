...
PORT=29888 ./tools/dist_train.sh ./configs/fcos/fcos_r50_caffe_fpn_4x4_1x_coco.py 4 \
    --npu-ids 0 \
    --cfg-options optimizer.lr=0.01 data.samples_per_gpu=${batch_size} total_epochs=1 data_root=$data_path \
    --seed 0 \
    --opt-level O1 \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_time=`grep -a 'time:' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk -F 'time: ' '{print $2}'|awk -F ',' '{print $1}'|tail -n 30|awk '{sum += $1} END {print sum/NR}'`
