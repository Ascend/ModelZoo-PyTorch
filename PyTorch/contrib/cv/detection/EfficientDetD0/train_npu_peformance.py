...
parser.add_argument('--resume', default='', type=str, metavar='PATH', # /home/sjh/effdetcode/output/train/8pbs16-checkpoint/model_best.pth.tar
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if True: # args.local_rank == 0
                ...
                if len(rates) == int(step/args.log_interval)+1:
                    value = sum(rates[2:-1])/(len(rates)-3)
                    print("[npu id:", args.gpu, "]",
                    'TrainAvgFPS: {} [{:>4d}/{} ({:>3.0f}%)]  '
                    "AvgFPS:{}".format(
                        epoch,
