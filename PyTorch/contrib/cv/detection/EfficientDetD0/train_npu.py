...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if use_amp == 'apex':
        ...
        amp_autocast = torch.npu.amp.autocast  #### copy to line422
    ...
    try:
        for epoch in range(start_epoch, num_epochs):   # num_epochs
            ...
            print("[npu id:", args.gpu, "]","before train_epoch")
            ...
            if saver is not None:
                ...
                if not args.resume and epoch == 129:
                    pthpath = os.path.join(output_dir, 'last.pth.tar')
                    print("pthpath",pthpath)
                    os.system("cp {} ./".format(pthpath))
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if True: # args.local_rank == 0
                ...
                if len(rates) == int(len(loader)/args.log_interval)+1:
                    value = sum(rates[1:-1])/(len(rates)-2)
                    print("[npu id:", args.gpu, "]",
                    'TrainAvgFPS: {} [{:>4d}/{} ({:>3.0f}%)]  '
                    "AvgFPS:{}".format(
                        epoch,
