    ...
    def _create_proposals_from_boxes(self, boxes, image_sizes):
        ...
        for boxes_per_image, image_size in zip(boxes, image_sizes):
            ...
            if self.training:
                ...
                boxes_num = len(boxes_per_image)
                num_per_image = boxes_per_image.tensor.shape[0]
                MAX_LEN = 512
                if num_per_image < MAX_LEN:
                    diff_num = MAX_LEN - num_per_image
                    boxes_per_image_new = torch.cat([boxes_per_image.tensor.cpu(), torch.zeros([diff_num, 4], dtype=torch.float16)], dim = 0)
                    boxes_per_image.tensor = boxes_per_image_new.npu()
                else:
                    select_idx = torch.randperm(boxes_num)[:MAX_LEN]
                    boxes_per_image.tensor = boxes_per_image.tensor[select_idx]
