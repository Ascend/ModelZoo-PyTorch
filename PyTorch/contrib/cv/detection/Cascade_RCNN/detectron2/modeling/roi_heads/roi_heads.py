...
from detectron2.structures import Boxes, ImageList, Instances, pairwise_iou, BitMasks, PolygonMasks
...
from ..sampling import subsample_labels_proposals
...
def select_foreground_proposals_fix_shape(
    ...
    each containing a `gt_classes` field,
    return a list of Instances that contain
    only instances with `gt_classes != -1 &&
    gt_classes != bg_label`.

    Args:
        proposals (list[Instances]): A list of N Instances,
        where N is the number of
            images in the batch.
        bg_label: label index of background class.

    Returns:
        list[Instances]: N Instances,
        each contains only the selected foreground instances.
        list[Tensor]: N boolean vector, correspond to the selection mask of
        each Instances object. True for selected instances.
    """
    assert isinstance(proposals, (list, tuple))
    ...
    for proposals_per_image in proposals:
        ...
        fg_selection_mask = (gt_classes != -1) & (gt_classes != bg_label)
        fix_num = 128 # max 128
        fg_selection_mask_num = fg_selection_mask.sum().item()
...
class ROIHeads(torch.nn.Module):
    ...
    def _sample_proposals(
        ...
        if has_gt:
            gt_classes = gt_classes[matched_idxs.long()].int()
        ...
        sampled_idxs = subsample_labels_proposals(gt_classes, self.batch_size_per_image, self.positive_fraction, self.num_classes)
