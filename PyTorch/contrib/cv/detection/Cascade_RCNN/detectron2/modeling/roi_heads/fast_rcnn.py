...
def fast_rcnn_inference_single_image(
    ...
    """
    valid_mask = torch.isfinite(boxes.float()).all(dim=1) & torch.isfinite(scores.float()).all(dim=1)
    ...
    boxes = boxes.tensor.view(-1, num_bbox_reg_classes, 4)  # R x C x 4 (1000, 80, 4)
    
    if boxes.dtype == torch.float32:
        boxes = boxes.reshape(1, 1000, num_bbox_reg_classes, 4).half()
        scores = scores.reshape(1, scores.shape[0], 80 * num_bbox_reg_classes).half()
    else:
...
class FastRCNNOutputs:
    ...
    def softmax_cross_entropy_loss(self):
        ...
        if self._no_instances:
            ...
            return F.cross_entropy(self.pred_class_logits.float(), self.gt_classes.int(), reduction="mean")
