...
from ..sampling import subsample_labels_anchors
...
class RPN(nn.Module):
    ...
    def _subsample_labels(self, label):
        ...
        """
        pos_idx, neg_idx = subsample_labels_anchors(
    ...
    def label_and_sample_anchors(
        ...
        for image_size_i, gt_boxes_i in zip(image_sizes, gt_boxes):
            ...
            if len(gt_boxes_i) == 0:
                ...
                matched_gt_boxes_i = torch.index_select(gt_boxes_i.tensor, 0, matched_idxs.long())
    ...
    def losses(
        ...
        if self.box_reg_loss_type == "smooth_l1":
            ...
            if self.amp and (self.opt_level == "O1" or self.opt_level == "O2"):
                ...
                input = cat(pred_anchor_deltas, dim=1).float() * \
                        pos_mask[:, :, None].float()
                target = gt_anchor_deltas.float() * pos_mask[:, :, None].float()
