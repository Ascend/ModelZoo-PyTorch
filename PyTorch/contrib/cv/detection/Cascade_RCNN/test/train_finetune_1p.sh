...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "d2.utils.events" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | grep "total_loss" | awk -F "total_loss" '{print $2}' | awk '{print $2}' >> ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${CaseName}_fps.log
