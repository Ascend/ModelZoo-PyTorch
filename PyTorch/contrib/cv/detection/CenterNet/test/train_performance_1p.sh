...
taskset -c $PID_START-$PID_END python3.7  main_npu_8p.py ctdet --exp_id pascal_resdcn18_384 --arch resdcn_18 --device_list='0' --dataset pascal --num_epochs 5 --lr_step 45,60,75 --port='34578' --world_size 1  --batch_size $batch_size --num_workers ${KERNEL_NUM} --local_rank $RANK_ID > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_performence_8p_${ASCEND_DEVICE_ID}.log 2>&1 &
done
