...
if [ x"${etp_flag}" != x"true" ];then
    ...
    if  [ -d ${pth_path} ]; then
        python3.7 tools/eval.py -n yolox-x -d 1 -b ${batch_size} --cf ${pth_path} --conf 0.001  > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
    else
	    python3.7 tools/eval.py -n yolox-x -d 1 -b ${batch_size} -c ${pth_path} --conf 0.001  > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
    fi
wait
