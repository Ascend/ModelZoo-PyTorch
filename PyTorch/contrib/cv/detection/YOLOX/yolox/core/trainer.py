...
def hook_func(name, save_dict, module):
    def hook_function(module, inputs, outputs):
        ...
        print_tensor(inputs_key, inputs)
        ...
        print_tensor(outputs_key, outputs)
...
class Trainer:
    def __init__(self, exp, args):
        ...
        self.maxx_epoch = args.maxx_epoch
        ...
        self.local_rank = args.device_id
        self.device = "npu:{}".format(self.local_rank)
        print('----- rank', self.rank, self.local_rank)
    ...
    def train_one_iter(self):
        ...
        if self.use_model_ema:
            ...
            if 'npu' in str(device):
                params_fp32_fused = self.optimizer.get_model_combined_params()
                self.ema_model.update(self.model, device=device, model_params_fused=params_fp32_fused[0])
            else:
    ...
    def after_train(self):
        logger.info(
            "Training of experiment is done and the last AP is {:.2f}".format(self.best_ap * 100)
        )
