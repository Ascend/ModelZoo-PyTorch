...
do
    ...
    time taskset -c $PID_START-$PID_END python3 train_8p.py --train-folder ${data_path} --continue-training --batch-size ${batch_size} --batches-before-train 2\
    --num-workers $KERNEL_NUM > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${i}.log 2>&1 &
    let rank++
done
