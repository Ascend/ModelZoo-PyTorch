...
bash ./tools/dist_test.sh \
    ...
    ${pth_path} \
    8 \
    --eval bbox > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'bbox_mAP'  ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk 'END {print}' | awk '{print $2}' | awk -F ")" '{print $1}'`
#打印，不需要修改
