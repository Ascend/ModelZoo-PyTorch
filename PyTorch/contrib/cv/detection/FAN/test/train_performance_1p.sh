...
do
    ...
    elif [[ $para == --landmarks_type* ]];then
        landmarks_type=`echo ${para#*=}`
    fi
done
...
print_log="./test/output/train_${ASCEND_DEVICE_ID}.log"
python3 eval.py --data_path ${data_path} --landmarks_type ${landmarks_type} --steps=${train_steps}
python3 eval.py --data_path ${data_path} --landmarks_type ${landmarks_type} --steps=${train_steps} 1>${print_log} 2>&1
...
echo "Network = ${Network}" >> $cur_path/output/${CaseName}.log
echo "RankSize = ${RANK_SIZE}" >> $cur_path/output/${CaseName}.log
echo "BatchSize = ${batch_size}" >> $cur_path/output/${CaseName}.log
echo "DeviceType = `uname -m`" >> $cur_path/output/${CaseName}.log
echo "CaseName = ${CaseName}" >> $cur_path/output/${CaseName}.log
echo "ActualFPS = ${FPS}" >> $cur_path/output/${CaseName}.log
echo "TrainingTime = ${StepTime}" >> $cur_path/output/${CaseName}.log
# echo "ActualLoss = ${ActualLoss}" >> $cur_path/test/output/${CaseName}.log
echo "E2ETrainingTime = ${e2e_time}" >> $cur_path/output/${CaseName}.log
# echo "TrainAccuracy = ${train_accuracy}" >> $cur_path/test/output/${CaseName}.log