...
class Box2BoxTransform(object):
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        """
        #print("============enter apply deltas===============")
        #print(boxes.shape)
        boxes = boxes.to(deltas.dtype)
        boxes=boxes.cpu().float()
        deltas=deltas.cpu().float()
        ...
        elif dw.dtype == torch.float16:
            # dw = torch.clamp(dw.to(torch.float32),
                             # max=self.scale_clamp).to(torch.float16)
            # dh = torch.clamp(dh.to(torch.float32),
                             # max=self.scale_clamp).to(torch.float16)
            dw = torch.clamp(dw,max=self.scale_clamp)
            dh = torch.clamp(dh,max=self.scale_clamp)
        #print("dw",dw)
        ...
        pred_boxes=pred_boxes.npu().half()
        return pred_boxes
