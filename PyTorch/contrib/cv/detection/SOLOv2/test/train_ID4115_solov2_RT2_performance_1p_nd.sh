...
cd ${cur_path}/../
python3 tools/train.py configs/solov2/solov2_r50_fpn_8gpu_1x.py --opt-level $apex --autoscale-lr --seed 0 --total_epochs 1 --batch_size=$batch_size\
      ...
      --start_step=$start_step --stop_step=$stop_step --profiling=$profiling --rt2_bin=$rt2_bin --ND=$ND> ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
...
DeviceType=`uname -m`
if [[ $apex == "O0" ]];then
        CaseName=${Network}_bs${BatchSize}_${RANK_SIZE}'p'_'fp32'_'perf_nd'
else
