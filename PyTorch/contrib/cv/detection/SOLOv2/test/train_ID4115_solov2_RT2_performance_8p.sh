...
#export ASCEND_GLOBAL_LOG_LEVEL_ETP_ETP_ETP_ETP=3
...
train_accuracy=`grep  'mmcv.runner.runner' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | grep loss | awk -F "loss: " '{print $NF}' | awk -F "," '{print $1}' | awk 'END {print}'`
#打印，不需要修改
