...
export NPUID=0
export RANK=0
python3 tools/train.py configs/solov2/solov2_r50_fpn_8gpu_1x.py --opt-level $apex --autoscale-lr --seed 0 --total_epochs 1 \
      --batch_size=${batch_size} --data_root=$data_path --gpu-ids ${device_id} --resume_from test/work_dirs/solov2_release_r50_fpn_8gpu_1x/latest.pth \
      --fine-tune > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
#python tools/test_ins.py configs/solo/solo_r50_fpn_8gpu_1x.py  work_dirs/solo_release_r50_fpn_8gpu_1x/latest.pth --show \
#      --out  results_solo.pkl --eval segm >> ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
#wait
