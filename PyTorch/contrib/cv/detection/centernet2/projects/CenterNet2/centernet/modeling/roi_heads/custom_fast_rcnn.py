...
class CustomFastRCNNOutputs(FastRCNNOutputs):
    def __init__(
        ...
        #     assert self.gt_classes.max() <= cfg.MODEL.ROI_HEADS.NUM_CLASSES, self.gt_classes.max() #error
    ...
    def softmax_cross_entropy_loss(self):
        ...
        if self._no_instances:
            ...
            # self._log_accuracy()
            # _log_classification_stats(self.pred_class_logits, self.gt_classes)
            # return F.cross_entropy(self.pred_class_logits, self.gt_classes, reduction="sum") / self.mask_num
    ...
    def box_reg_loss(self):
        ...
        fg_inds = (self.gt_classes >= 0) & (self.gt_classes < bg_class_ind) #fixed shape
        ...
        if self.box_reg_loss_type == "smooth_l1":
            ...
            #     self.pred_proposal_deltas[fg_inds[:, None], gt_class_cols.type(torch.long)],
            #     gt_proposal_deltas[fg_inds],
            #     self.smooth_l1_beta,
            #     reduction="sum",
            # )
            loss_box_reg = smooth_l1_loss(
                self.pred_proposal_deltas[:, gt_class_cols.long()],
                gt_proposal_deltas,
                self.smooth_l1_beta,
                reduction=None
                )
            loss_box_reg = (loss_box_reg * fg_inds.unsqueeze(1).float()).sum()
            # import pdb; pdb.set_trace()
            # fixed shape end
        ...
        # loss_box_reg = loss_box_reg / self.mask_num
        return loss_box_reg
...
class CustomFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def losses(self, predictions, proposals, mask_num = None, use_advanced_loss=True):
        """
