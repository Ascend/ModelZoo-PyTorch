...
def launch(main_func, num_gpus_per_machine, num_machines=1, machine_rank=0, dist_url=None, args=()):
    ...
    if world_size > 1:
        ...
        #     daemon=False,
        # )
        _distributed_worker(args[0].local_rank, main_func, world_size, num_gpus_per_machine, machine_rank, dist_url, args)
    else:
        main_func(*args)
...
def _distributed_worker(
    ...
    assert torch.npu.is_available(), "npu is not available. Please check your installation."
    torch.npu.set_device(local_rank)
    global_rank = machine_rank * num_gpus_per_machine + local_rank
    # try:
    dist.init_process_group(
        backend="HCCL", init_method=dist_url, world_size=world_size, rank=global_rank
    )
