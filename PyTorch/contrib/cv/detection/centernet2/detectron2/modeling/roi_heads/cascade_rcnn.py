...
class CascadeROIHeads(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals, targets=None):
        ...
        for k in range(self.num_cascade_stages):
            if k > 0:
                ...
                for pp in proposals:
                    pp.proposal_boxes.clip((1344,1344)) #fixed shape
    ...
    def _create_proposals_from_boxes(self, boxes, image_sizes):
        ...
        for boxes_per_image, image_size in zip(boxes, image_sizes):
            ...
            boxes_per_image.clip([1344,1344]) #fixed shape
            if self.training:
                ...
                boxes_mask = boxes_per_image.nonempty()
                mask_num = mask_num + boxes_mask.sum()
                boxes_per_image.tensor = boxes_per_image.tensor * boxes_mask.unsqueeze(1).float()
                # boxes_per_image.clip([1344,1344])
                #fixed shape end
            prop = Instances(image_size)
            prop.proposal_boxes = boxes_per_image
            proposals.append(prop)
        return mask_num, proposals
