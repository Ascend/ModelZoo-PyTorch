...
def fast_rcnn_inference_single_image(
    ...
    Return bounding-box detection results by thresholding
    ...
    """
    valid_mask = torch.isfinite(boxes.float()).all(dim=1) & \
                 torch.isfinite(scores.float()).all(dim=1)
    ...
    boxes = boxes.tensor.view(-1,
                              num_bbox_reg_classes, 4)  # R x C x 4 (1000, 80, 4)


    if boxes.dtype == torch.float32:
        boxes = boxes.reshape(1, 1000, num_bbox_reg_classes, 4).half()
        scores = scores.reshape(
            1,scores.shape[0], 80 * num_bbox_reg_classes).half()
    else:
        boxes = boxes.reshape(1, 1000, num_bbox_reg_classes, 4)
        scores = scores.reshape(1,scores.shape[0],num_bbox_reg_classes)
...
class FastRCNNOutputs:
    ...
    def softmax_cross_entropy_loss(self):
        ...
        if self._no_instances:
            ...
            return F.cross_entropy(self.pred_class_logits, self.gt_classes.int(), reduction="mean")
    ...
    def box_reg_loss(self):
        ...
        if cls_agnostic_bbox_reg:
            ...
            gt_classes_num = self.gt_classes.shape[0]
            
            gt_class_cols_all = box_dim * ((self.gt_classes == bg_class_ind).float())[:, None] \
                                + torch.arange(box_dim, device=device)
            
            gt_class_cols_all_mask = ((gt_class_cols_all >= 0) & (gt_class_cols_all < 4))
            gt_class_cols_all[~gt_class_cols_all_mask] = 0
            # gt_class_cols_all = torch.arange(box_dim, device=device)
            
            fg_inds_all = torch.arange(gt_classes_num, device=device)
        else:
            gt_classes_num = self.gt_classes.shape[0]
            gt_class_cols_all = box_dim * self.gt_classes[:, None] \
                                + torch.arange(box_dim, device=device)
            gt_class_cols_all_mask = ((gt_class_cols_all >= 0) & (gt_class_cols_all < 320))
            gt_class_cols_all[~gt_class_cols_all_mask] = 0
            fg_inds_all = torch.arange(gt_classes_num, device=device)
        ...
        if self.box_reg_loss_type == "smooth_l1":
            ...
            input_ = self.pred_proposal_deltas[fg_inds_all.long()[:, None], gt_class_cols_all.long()]
            target = gt_proposal_deltas
            input_[~fg_inds] = 0
            target[~fg_inds] = 0
            loss_box_reg = smooth_l1_loss(input_, target, self.smooth_l1_beta, reduction="sum")


        elif self.box_reg_loss_type == "giou":
            loss_box_reg = giou_loss(
                self._predict_boxes()[fg_inds[:, None].long(), gt_class_cols.long()],
                self.gt_boxes.tensor[fg_inds.long()],
                reduction="sum",
            )
        else:
