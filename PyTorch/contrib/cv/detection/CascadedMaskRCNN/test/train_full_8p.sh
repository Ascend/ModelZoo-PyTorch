...
python tools/train_net.py \
        --config-file configs/Misc/cascade_mask_rcnn_R_50_FPN_1x.yaml \
        ...
        SOLVER.BASE_LR 0.2 > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
grep "d2.utils.events" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | grep "fps:" | awk '{print $38}' >> ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${CaseName}_fps.log
FPS=`cat ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${CaseName}_fps.log | awk '{a+=$1} END {if (NR != 0) printf("%.3f",a/NR)}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep -A 3 "Evaluation results for segm:" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | tail -n 1 | awk '{print $2}'`
#打印，不需要修改
