...
rm -rf kernel_meta/
python3 ./tools/train.py configs/ssd/ssd300_coco_npu.py --cfg-options total_epochs=${train_epochs} data.samples_per_gpu=${batch_size} --gpu-ids ${device_id} --no-validate --max_steps=1000 > ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
