...
class Polygons(object):
    ...
    def crop_and_resize_and_decode(self, box, size):
        ...
        if isinstance(self.polygons, torch.Tensor):
            self.polygons = [self.polygons]
        cropped_polygons = []
        for poly in self.polygons:
            p = poly.numpy().copy()
            p[0::2] = (p[0::2] - box[0]) * w_scale  # .clamp(min=0, max=w)
            p[1::2] = (p[1::2] - box[1]) * h_scale  # .clamp(min=0, max=h)
            cropped_polygons.append(p)
        rles = mask_utils.frPyObjects(cropped_polygons, out_h, out_w)
...
class SegmentationMask(object):
    ...
    def mask_ind_op(self, fg_selection_mask):
        for i in range(len(fg_selection_mask)):
            if not fg_selection_mask[i]:
                self.polygons[i].polygons = [torch.zeros([6], dtype=torch.float16)]

        return SegmentationMask(self.polygons, size=self.size, mode=self.mode)
    ...
    def expand(self, pad):
        self.polygons += [Polygons(p, self.size, mode=None) for p in [torch.zeros([1], dtype=torch.float16)] * pad]
