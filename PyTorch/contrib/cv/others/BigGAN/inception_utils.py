...
def load_inception_net(parallel=False, device='cuda', use_fp16=False, opt_level='O2'):
    ...
    if use_fp16:
        print("use fp16 inception model, level {:s}...".format(opt_level))
        inception_model = amp.initialize(inception_model, opt_level=opt_level, combine_grad=True)
...
def prepare_inception_metrics(dataset, parallel, no_fid=False, device='cuda', use_fp16=False, opt_level='O2'):
    ...
    # Load network
    net = load_inception_net(parallel, device=device, use_fp16=use_fp16, opt_level=opt_level)

    def get_inception_metrics(sample, num_inception_images, num_splits=10,
                              prints=True, use_torch=False):
        if prints:
            print('Gathering activations...', "device:{}".format(device))
        pool, logits, labels = accumulate_inception_activations(sample, net, num_inception_images)
        if prints:
            print('Calculating Inception Score...', "device:{}".format(device))
        ...
        if no_fid:
            ...
            if prints:
                print('Covariances calculated, getting FID...', "device:{}".format(device))
            if use_torch:
