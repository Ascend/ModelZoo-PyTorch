...
# 性能相关数据计算
Time=`grep "time" ${print_log} | awk -F ":" '{print $2}' | tr -d "s" | awk '{sum+=$1} END {print sum/NR}'`
StepTime=`awk 'BEGIN{printf "%.2f\n", '${Time}'/'13'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "loss" ${print_log} | grep -v "batch" | awk '{print $4}' | tr -d "," | awk 'END {print $1}'`

# 提取所有loss打印信息
grep "loss" ${print_log} | grep -v "batch" | awk '{print $4}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
