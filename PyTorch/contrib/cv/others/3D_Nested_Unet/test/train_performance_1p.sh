...
# 启动本脚本的示例: bash train_performance_1p.sh
# 单独调用nnUNet_train的示例: nnUNet_train 3d_fullres nnUNetPlusPlusTrainerV2 Task003_Liver 0

##########基础配置及超参数##########

# 可改参数。输出的性能日志文件的路径。
output_log="./test/3D_Nested_Unet_npu_1p_perf.log"
...
date
start_time=$(date +%s)
echo "train_performance_1p start."
nnUNet_train 3d_fullres nnUNetPlusPlusTrainerV2 Task003_Liver 1 --other_use fps > ${output_log} 2>&1 &
wait
nnUNet_train 3d_fullres nnUNetPlusPlusTrainerV2 Task003_Liver 1 --other_use fps
echo "train_performance_1p.sh end."
date
end_time=$(date +%s)
