...
# 性能相关数据计算
EpochTime=`grep -rn "Time Taken:" ${print_log} | awk -F 'Time Taken: ' '{print $2}' | awk '{print $1}' | tail -n +2 | awk '{sum+=$1} END {print sum/NR}'`
StepTime=`awk 'BEGIN{printf "%.2f\n", '${EpochTime}'/'7116'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`


# 提取所有loss打印信息
grep "loss_D_real" ${print_log} | awk '{print $22}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt


loss_G_Adv=`grep -rn "loss_G_Adv" ${print_log} | awk '{print $8}' | awk '{sum+=$1} END {print sum/NR}'`
loss_G_Cycle=`grep -rn "loss_G_Cycle" ${print_log} | awk '{print $10}' | awk '{sum+=$1} END {print sum/NR}'`
loss_G_Rec=`grep -rn "loss_G_Rec" ${print_log} | awk '{print $12}' | awk '{sum+=$1} END {print sum/NR}'`
loss_G_identity_reconst=`grep -rn "loss_G_identity_reconst" ${print_log} | awk '{print $14}' | awk '{sum+=$1} END {print sum/NR}'`
loss_G_age_reconst=`grep -rn "loss_G_age_reconst" ${print_log} | awk '{print $16}' | awk '{sum+=$1} END {print sum/NR}'`

loss_D_real=`grep "loss_D_real" ${print_log} | awk '{print $18}'| awk '{sum+=$1} END {print sum/NR}'`
loss_D_fake=`grep "loss_D_real" ${print_log} | awk '{print $20}'| awk '{sum+=$1} END {print sum/NR}'`
loss_D_reg=`grep "loss_D_real" ${print_log} | awk '{print $22}'| awk '{sum+=$1} END {print sum/NR}'`
loss=`awk 'BEGIN{printf "%.2f\n", '(${loss_D_real}+${loss_D_fake}+${loss_D_reg})'/'3'}'`
