...
def define_G(input_nc, output_nc, ngf, n_downsample_global=2,
             id_enc_norm='pixel', npu_ids=[], padding_type='reflect',
    ...
    if len(npu_ids) > 0:
        ...
        # 如果该机器的npu可用则将辨别器放在npu上
        netG.npu(npu_ids[0])

    # print(init_type)
    # 生成器中的Conv2d和linear层初始化方式为kaiming
    netG.apply(weights_init(init_type))

    return netG

def define_D(input_nc, ndf, n_layers=6, numClasses=2, npu_ids=[],
             init_type='gaussian'):
    ...
    # 辨别器中Conv2d和linear层初始化方式为gaussian
    netD.apply(weights_init('gaussian'))

    '''
    def print_weight(m):
        if isinstance(m, StyledConvBlock):
            print("weight", m.weight.data)
            print("bias:", m.bias.data)
            print("next...")

    netD.apply(print_weight)
    '''
...
class StyledConvBlock(nn.Module):
    def __init__(self, fin, fout, latent_dim=256, padding='reflect', upsample=False, downsample=False,
                 actvn='lrelu', use_pixel_norm=False, normalize_affine_output=False, modulated_conv=False, kersize=False):
        ...
        if self.modulated_conv:
            self.conv0 = conv2d(fin, fout, kernel_size=3, padding_type=padding, upsample=upsample,#k=3 ,1
                                latent_dim=latent_dim, normalize_mlp=normalize_affine_output)
        elif kersize:
            conv0 = conv2d(fin, fout, kernel_size=3)  # k=3 ,1
            if self.upsample:
                seq0 = [self.upsampler, padding_layer(1), conv0, Blur(fout)]
                # seq0 = [self.upsampler, conv0, Blur(fout)]
            else:
                seq0 = [padding_layer(1), conv0]
                # seq0 = [conv0]
            self.conv0 = nn.Sequential(*seq0)
        else:
            conv0 = conv2d(fin, fout, kernel_size=1) #k=3 ,1
            if self.upsample:
                # seq0 = [self.upsampler, padding_layer(1), conv0, Blur(fout)]
                seq0 = [self.upsampler, conv0, Blur(fout)]
            else:
        ...
        if self.modulated_conv:
            self.conv1 = conv2d(fout, fout, kernel_size=3, padding_type=padding, downsample=downsample,#k=3 ,1
                                latent_dim=latent_dim, normalize_mlp=normalize_affine_output)
        elif kersize:
            conv1 = conv2d(fout, fout, kernel_size=3)  # k=3 ,1
            if self.downsample:
                seq1 = [Blur(fout), padding_layer(1), conv1, self.downsampler]
                # seq1 = [Blur(fout), conv1, self.downsampler]
            else:
                seq1 = [padding_layer(1), conv1]
                # seq1 = [conv1]
            self.conv1 = nn.Sequential(*seq1)
        else:
            conv1 = conv2d(fout, fout, kernel_size=1)#k=3 ,1
            if self.downsample:
                # seq1 = [Blur(fout), padding_layer(1), conv1, self.downsampler]
                seq1 = [Blur(fout), conv1, self.downsampler]
            else:
...
class StyledDecoder(nn.Module):
    def __init__(self, output_nc, ngf=64, style_dim=50, latent_dim=256, n_downsampling=2,
        ...
        mult = 2**n_downsampling # 2的n_downsampling次幂
    ...
    def forward(self, id_features, target_age=None, traverse=False, deploy=False, interp_step=0.5):
        if target_age is not None:
            if traverse:
                alphas = torch.arange(1,0,step=-interp_step).view(-1,1).npu()
                interps = len(alphas)
...
class StyleGANDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=6, numClasses=2, padding_type='reflect'):
        ...
        nf_prev = nf
        nf = min(nf * 2, 512)
        sequence += [StyledConvBlock(nf_prev, nf, downsample=True, actvn=activation, kersize=True)]
