            ...
            try:
                ...
                for self.iter in range(start_iter, max_iter):
                   ...
                   #         torch.npu.synchronize()
                    #    prof.export_chrome_trace("loss_dict6.prof")
                    #    break
...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        #print(torch.npu.synchronize(), "backward ")
        # add prof 
        ...
        if self.cfg.AMP:
           ...
           losses.backward()
        # prof.export_chrome_trace("backward.prof")
        # end
        # use a new stream so the ops don't wait for DDP
        with torch.npu.stream(
            torch.npu.Stream()
        ) if losses.device.type == "npu" else _nullcontext():
