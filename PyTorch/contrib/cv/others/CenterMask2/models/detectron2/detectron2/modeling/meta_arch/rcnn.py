...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.proposal_generator:
            ...
            proposal_losses = {}
        print(torch.npu.synchronize(),"proposals end")
        #print("features", features.shape)
        #print("proposals", proposals.shape)
        #state = {"features": features, "proposals": proposals}
        #torch.save(state, "/home/zsclzy/centermask2/model3.pth")

        #with torch.autograd.profiler.profile(record_shapes = True, use_npu=True) as prof:
        #hook_for_model(self.roi_heads)
        _, detector_losses = self.roi_heads(images, features, proposals, gt_instances)
        #print(prof.key_averages().table(sort_by="self_cpu_time_total"))
        #prof.export_chrome_trace("detector_npu.prof")
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        images = self.preprocess_image_v3(batched_inputs)
    ...
    def preprocess_image_v3(self, batched_inputs):
        """
        use dataset results.
        """
        ...
        for image in images:
            ...
            put_size = [0, 1344-image_size[-1],
                        0, 1344-image_size[-2]]
            image_pre = F.pad(image, put_size, value=0)
            image_pre = image_pre.unsqueeze(0)
            image_pres.append(image_pre)
            image_sizes.append(torch.tensor(image.shape[-2:]).tolist())
        images = torch.cat(image_pres, dim=0)
        return ImageList(images, image_sizes)
