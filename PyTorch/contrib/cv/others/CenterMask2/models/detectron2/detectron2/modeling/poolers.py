...
"""
To export ROIPooler to torchscript, in this file, 
variables that should be annotated with
`Union[List[Boxes], List[RotatedBoxes]]` 
are only annotated with `List[Boxes]`.

TODO: Correct these annotations 
when torchscript support `Union`.
...
def assign_boxes_to_levels(
    ...
    return level_assignments.to(torch.int32) - min_level
...
class ROIPooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes]):
        ...
        for level, pooler in enumerate(self.level_poolers):
            ...
            if inds_num != 0:
                temp = torch_npu.npu_format_cast(pooler(x[level], pooler_fmt_boxes), 0)
                if dtype == torch.float16:
                    output = torch.where(
                        inds_mask[:, None, None, None].repeat(
                            1, num_channels, output_size, output_size),
                        temp.half(), output)
