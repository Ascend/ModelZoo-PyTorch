...
sed -i "14s#datasets#${data_path}/datasets#g" ${PWD}/options/train/LPTN/train_FiveK.yml
sed -i "15s#datasets#${data_path}/datasets#g" ${PWD}/options/train/LPTN/train_FiveK.yml
sed -i "37s#datasets#${data_path}/datasets#g" ${PWD}/options/train/LPTN/train_FiveK.yml
sed -i "38s#datasets#${data_path}/datasets#g" ${PWD}/options/train/LPTN/train_FiveK.yml

sed -i "17s#datasets#${data_path}/datasets#g" ${PWD}/options/test/LPTN/test_FiveK.yml
sed -i "18s#datasets#${data_path}/datasets#g" ${PWD}/options/test/LPTN/test_FiveK.yml
#sed -i "31s#experiments/pretrained_models/net_g_FiveK_numhigh3.pth#${data_path}/datasets/FiveK/net_g_FiveK_numhigh3.pth#g" ${PWD}/options/test/LPTN/test_FiveK.yml
sed -i "17s#datasets#${data_path}/datasets#g" ${PWD}/options/test/LPTN/test_speed_FiveK.yml
sed -i "18s#datasets#${data_path}/datasets#g" ${PWD}/options/test/LPTN/test_speed_FiveK.yml
sed -i "31s#experiments/pretrained_models/net_g_FiveK_numhigh3.pth#${data_path}/datasets/FiveK/net_g_FiveK_numhigh3.pth#g" ${PWD}/options/test/LPTN/test_speed_FiveK.yml
# 训练开始时间记录，不需要修改
start_time=$(date +%s)
##########################################################
#########第3行 至 100行，请一定不要、不要、不要修改##########
...
then
    ...
    python3 test_speed.py --opt=${PWD}/options/test/LPTN/test_speed_FiveK.yml 1>>${print_log} 2>&1
fi

# 性能相关数据计算
StepTime=`grep "Testing" ${print_log} | tail -n +3 | awk -F "duration: " '{print $2}' | awk '{sum+=$1} END {print sum/NR}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 精度相关数据计算
train_accuracy=`grep "# psnr" ${print_log} | awk '{print $NF}' | tail -n 1`
# 提取所有loss打印信息
grep "l_d_real" ${print_log} | awk -F "l_d_real: " '{print $2}' | awk '{print $1}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
