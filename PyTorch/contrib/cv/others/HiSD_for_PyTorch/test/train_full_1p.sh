...
#修改配置的数据集路径
sed -i "s#datasets#${data_path}/datasets#g" ${PWD}/configs/celeba-hq.yaml
sed -i "s#/home/test_user06/temp_store#${data_path}#g" ${data_path}/datasets/*.txt
# 训练开始时间记录，不需要修改
start_time=$(date +%s)
##########################################################
#########第3行 至 100行，请一定不要、不要、不要修改##########
...
batch_size=8
total_iterations=62800
checkpoint="./outputs/celeba-hq/checkpoints/gen_00062800.pt"
if [ x"${modelarts_flag}" != x ];
then
    python3 ./core/train.py --batchsize ${batch_size} --total_iterations ${total_iterations} >${print_log} 2>&1
    python3 ./core/test.py --input_path ${test_img_dir} --output_path ${output_path} --checkpoint ${checkpoint}
    python3 ./pytorch_fid/fid_score.py --path ${test_img_dir} ${output_path} 1>>${print_log} 2>&1
else
    python3 ./core/train.py --config=${PWD}/configs/celeba-hq.yaml --batchsize=${batch_size} --total_iterations=${total_iterations} --gpus=${NPU_CALCULATE_DEVICE} 1>${print_log} 2>&1
    python3 ./core/test.py  --config=${PWD}/configs/celeba-hq.yaml --input_path=${data_path}/test_imgs --output_path=${PWD}/results --checkpoint=${checkpoint} 1>>${print_log} 2>&1
    python3 ./pytorch_fid/fid_score.py --gpu=${NPU_CALCULATE_DEVICE} --path ${PWD}/results ${data_path}/test_imgs 1>>${print_log} 2>&1
fi

#还原初始数据集路径
sed -i "s#${data_path}#/home/test_user06/temp_store#g" ${data_path}/datasets/*.txt
...
# 精度相关数据计算
train_accuracy=`grep "Final FID:" ${print_log}  | awk '{print $NF}' | tr -d "FID:"`
# 提取所有loss打印信息
grep "Gen Loss:" ${print_log} | awk -F ":" '{print $2}' | awk -F "," '{print $1}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
