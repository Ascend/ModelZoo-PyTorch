...
output_dir=${test_path_dir}/output/finetune_1p
#################创建日志输出目录，不需要修改#################
...
output_log=${output_dir}/train_finetune_1p.log
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'PSNR'  ${output_log}| tail -1 |awk -F "PSNR:" '{print $2}'|awk -F " " '{print $1}'`
#打印，不需要修改
...
grep L1: ${output_log} >>  ${output_dir}/train_${CaseName}_loss.txt
