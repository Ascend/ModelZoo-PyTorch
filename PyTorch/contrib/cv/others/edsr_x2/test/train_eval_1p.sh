...
output_dir=${test_path_dir}/output/eval_1p
#################创建日志输出目录，不需要修改#################
...
output_log=${output_dir}/test_eval_1p.log
...
python3 \
        ...
        --save ${output_dir}/npu_1p/test \
        --test_only > ${output_log} 2>&1 &
...
# 输出训练精度,需要模型审视修改
test_accuracy=`grep -a 'PSNR'  ${output_log} | tail -1 |awk -F "PSNR:" '{print $2}'|awk -F " " '{print $1}'`
#打印，不需要修改
