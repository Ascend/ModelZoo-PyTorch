...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 train_npu.py --datapath=${data_path}
else
    python3.7 train_npu.py --datapath=${data_path} 1>${print_log} 2>&1
fi

# 性能相关数据计算
EpochTime=`grep -A 1 "Avg. Loss" ${print_log} | grep -v "Avg. Loss" | awk '{sum+=$1} END {print sum/NR}'`
