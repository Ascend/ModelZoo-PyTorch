...
class SRFlowModel(BaseModel):
    def __init__(self, opt, step):
        ...
        if opt['dist']:
            self.netG = DistributedDataParallel(self.netG, device_ids=[self.rank], output_device=self.rank, find_unused_parameters=True, broadcast_buffers=False
                                                )
        else:
    ...
    def get_sr(self, lq, heat=None, seed=None, z=None, epses=None):
        return self.get_sr_with_z(lq.to(self.device), heat, seed, z, epses)[0]
