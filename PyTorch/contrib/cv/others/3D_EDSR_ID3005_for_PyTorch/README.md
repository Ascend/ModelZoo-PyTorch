...
**描述（Description）：基于PyTorch框架的单图像超分辨率的增强深度残差网络训练代码**

<h2 id="概述.md">概述</h2>

## 简述<a name="section194554031510"></a>

增强的深度超分辨率网络（EDSR），其性能超过了目前最先进的SR方法。我们模型的显著性能改进是由于通过删除中不必要的模块而进行的优化传统残差网络。业绩进一步通过在稳定的同时扩展模型大小来改进培训程序。我们还提出了一种新的多尺度深度超分辨率系统（MDSR）及训练方法，它可以重建不同的高分辨率图像单个模型中的上调因子。建议的方法在基准数据集上显示出优于最先进的方法的性能，并通过w证明了其卓越性.
...
- 参考实现：
  ...
  https://github.com/sci-sjj/EDSRmodelling

- 适配昇腾 AI 处理器的实现：
  
  https://gitee.com/ascend/ModelZoo-PyTorch/tree/master/PyTorch/contrib/cv/others/3D_EDSR_ID3005_for_PyTorch 
...
1、用户自行准备好数据集，https://github.com/sci-sjj/EDSRmodelling，依据(Generating LR, Cubic and HR data)获取数据集Core1_Subvol1_HR.tif，Core1_Subvol1_LR.tif。

2、3D_EDSR训练的模型及数据集可以参考"简述 -> 参考实现"
...
- 开始训练。
    ...
    - 单卡训练
      ...
      2. 启动训练。
         ...
         启动单卡训练 （脚本为3D_EDSR_ID3005_for_PyTorch / test / train_full_1p.sh）
