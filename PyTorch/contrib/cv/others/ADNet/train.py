...
parser.add_argument("--loss_scale", default=64, type=int)
'''
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            if opt.is_distributed == 0 and epoch == 0 and i == 6:
                with torch.autograd.profiler.profile(use_npu=True) as prof:
                    ...
                    # print('name:',name,'-->grad_value:',param.grad)
                    optimizer.step()
               ...
               # print(noise.size())
                imgn_train = img_train + noise
                #print("type of img_train",img_train.dtype)
                #print("type of noise ", noise.dtype)
                #print("img_train",img_train)
                #print("noise",noise)
                #print("imgn_train",imgn_train)
                img_train, imgn_train = Variable(img_train.npu()), Variable(imgn_train.npu())
                noise = Variable(noise.npu())
                out_train = model(imgn_train)
                loss = (criterion(out_train.cpu(), img_train.cpu()) / (imgn_train.size()[0] * 2)).npu()
