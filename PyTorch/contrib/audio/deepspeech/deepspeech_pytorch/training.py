...
def train(cfg):
    ...
    torch.npu.manual_seed_all(cfg.training.seed)
    np.random.seed(cfg.training.seed)
    random.seed(cfg.training.seed)

    main_proc = True
    device = torch.device("cpu" if cfg.training.no_cuda else "npu")
    ...
    if is_distributed:
        ...
        print(f"Setting NPU Device to {device_id}")
    ...
    print(f"len of training dataset {len(train_dataset)}")
    ...
    if is_distributed:
        model = DistributedDataParallel(model, device_ids=[device_id], broadcast_buffers=False)
    print(model)
    ...
    for epoch in range(state.epoch, cfg.training.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=state.training_step):
            ...
            if epoch == 5 and i == 0 and is_distributed is None:
                with torch.autograd.profiler.profile(use_npu=True) as prof:
                    out, output_sizes = model(inputs, input_sizes)
                    out = out.cpu()  # diff put out to cpu cause op error
                ...
                out, output_sizes = model(inputs, input_sizes)
                out = out.cpu()  # diff put out to cpu
                out = out.transpose(0, 1)  # TxNxH
        ...
        print('Training Summary Epoch: [{0}]\t'
              ...
              'Average Loss {loss:.3f}\t'.format(epoch + 1,
                                                 fps=ngpus_per_node * ngpus_per_node * cfg.data.batch_size / batch_time.avg,
                                                 epoch_time=epoch_time, loss=state.avg_loss))
