...
def train_model(model, db_gen, optimizer, epoch, args, device, lr_scheduler, criterion):    
    ...
    with tqdm(total = len(db_gen), ncols = 70) as pbar:
        ...
        for m_batch, m_label in db_gen:
            ...
            m_batch, m_label = m_batch.npu(), m_label.npu()
            ...
            if args.use_prof:
                with torch.autograd.profiler.profile(use_npu = True) as prof:
                    ...
                    if args.amp_mode:
                        ...
                        loss.backward()
                    optimizer.step()
                save_prof = args.save_dir + args.name + '/' + args.save_profile
                prof_path = save_prof + "torch_trace.json"
        ...
        # calculate the FPS
        FPS = args.frame / (time_total * 1000)
