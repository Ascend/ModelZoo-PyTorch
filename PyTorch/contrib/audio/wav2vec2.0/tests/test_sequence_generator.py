...
@unittest.skipUnless(torch.npu.is_available(), "")
class TestRepeatNgramBlocking(TestSequenceGeneratorBase):
    ...
    def test_finds_repetitive_tokens(self):
        ...
        generated_tok = torch.tensor(
            [[2, 2, 2, 2], [3, 3, 3, 3]], dtype=torch.long, device="npu"
        )
        lprobs = torch.zeros((beam_size * bsz, vocab_size), device="npu")
    ...
    def test_ngram_blocking_same_as_default_implem(self):
        ...
        for _ in range(2):
            ...
            lprobs = torch.zeros((beam_size * batch_size, vocab_size), device="npu")
