    ...
    if cfg.pipeline_model_parallel:
        ...
        with open_dict(cfg):
            cfg.distributed_num_procs = min(
                torch.npu.device_count(), cfg.distributed_world_size
            )
...
def _infer_single_node_init(cfg: DistributedTrainingConfig):
    assert (
        cfg.distributed_world_size <= torch.npu.device_count()
    ), f"world size is {cfg.distributed_world_size} but have {torch.npu.device_count()} available devices"
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    ...
    if torch.npu.is_available() and not cfg.common.cpu and not cfg.common.tpu:
...
def all_gather_list(data, group=None, max_size=16384):
    ...
    if (
        ...
        all_gather_list._buffer = torch.npu.IntTensor(buffer_size)
        all_gather_list._cpu_buffer = torch.IntTensor(max_size).pin_memory()
    ...
    cpu_buffer[:size] = torch.IntTensor(list(header + enc))
    start = rank * max_size
...
def all_reduce_dict(data: Mapping[str, Any], device, group) -> Dict[str, Any]:
    ...
    for k in data_keys:
        ...
        if not torch.is_tensor(t):
            cpu_data[k] = torch.tensor(t, dtype=torch.int)
        elif t.device.type != device.type:
            cpu_data[k] = t.to(dtype=torch.int)
        else:
            device_data[k] = t.to(dtype=torch.int)
