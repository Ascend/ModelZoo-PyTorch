...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.npu = torch.npu.is_available() and not cfg.common.cpu and not self.tpu
        if self.npu:
        ...
        if self.npu:
            self.npu_env = utils.CudaEnvironment()
            if self.data_parallel_world_size > 1:
                self.npu_env_arr = distributed_utils.all_gather_list(
                    self.npu_env, group=distributed_utils.get_global_group()
                )
            else:
                self.npu_env_arr = [self.npu_env]
            if self.data_parallel_rank == 0:
                utils.CudaEnvironment.pretty_print_cuda_env_list(self.npu_env_arr)
        else:
            self.npu_env = None
            self.npu_env_arr = None
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("multiply-grads"):
                ...
                self.optimizer.multiply_grads(float(numer) / (float(sample_size) or 1.0))
        ...
        if not overflow or self.cfg.distributed_training.ddp_backend == "slowmo":
            ...
            if self.tpu:
                ...
                if self.npu and self.npu_env is not None:
                    # log minimum free memory over the iteration
                    gb_used = torch.npu.max_memory_allocated() / 1024 / 1024 / 1024
                    torch.npu.reset_peak_memory_stats()
                    gb_free = self.npu_env.total_memory_in_GB - gb_used
                    metrics.log_scalar(
                        "gb_free", gb_free, priority=1500, round=1, weight=0
                    )
    ...
    def clip_grad_norm(self, clip_norm):
        def agg_norm_fn(total_norm):
            total_norm = total_norm.npu().float() ** 2
