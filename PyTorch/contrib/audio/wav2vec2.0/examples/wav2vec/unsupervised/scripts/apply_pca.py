...
def main():
    ...
    pca_A = torch.from_numpy(np.load(args.pca_path + "_A.npy")).npu()
    pca_b = torch.from_numpy(np.load(args.pca_path + "_b.npy")).npu()
    ...
    with torch.no_grad():
        for b in tqdm.trange(batches):
            ...
            x = torch.from_numpy(features[start:end]).npu()
