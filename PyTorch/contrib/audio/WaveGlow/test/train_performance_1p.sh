...
do
    if [[ $para == --device_id* ]];then
        ...
        data_path=`echo ${para#*=}`
    elif [[ $para == --output_directory* ]];then
        output_directory=`echo ${para#*=}`
    fi
done
...
if [[ $output_directory == "" ]];then
    echo "[Error] para \"output_directory\" must be confing"
    exit 1
fi
...
# 训练准备，加载数据集
ls ${data_path}/wavs/*.wav | tail -n+10 > train_files.txt &&
...
# #输出训练精度,需要模型审视修改
train_accuracy=`grep -a loss ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "\t" '{print $2}'|awk -F ":" '{print $2}'|awk 'END {print}'`
# #打印，不需要修改
