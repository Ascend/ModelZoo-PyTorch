...
class EmbeddingCPU(nn.Embedding):
    def __init__(self, num_embeddings, embedding_dim, padding_idx=None):
        super(EmbeddingCPU, self).__init__(num_embeddings, embedding_dim, padding_idx)

    def forward(self, x):
        device = x.device
        self.weight = self.weight.cpu()
        x = x.cpu()
        out = super(EmbeddingCPU, self).forward(x).to(device)
        return out
...
class FFTransformer(nn.Module):
    ...
    def forward(self, dec_inp, seq_lens=None, conditioning=0):
        ...
        if self.word_emb is None:
            inp = dec_inp
            mask = mask_from_lens(seq_lens, 900).unsqueeze(2).to(inp.device)
        else:
