...
def multi_init(device_id, world_size, gpu_ranks, train_npu):
    print(gpu_ranks)
    dist_init_method = 'tcp://localhost:10000'
    dist_world_size = world_size
    if train_npu:
        torch.distributed.init_process_group(
        backend='hccl', init_method=dist_init_method,
...
def all_gather_list(data, max_size=4096):
    ...
    if not hasattr(all_gather_list, '_in_buffer') or \
        ...
        all_gather_list._in_buffer = torch.npu.IntTensor(max_size)
        all_gather_list._out_buffers = [
            torch.npu.IntTensor(max_size)
    ...
    in_buffer[2:enc_size+2] = torch.IntTensor(list(enc))
