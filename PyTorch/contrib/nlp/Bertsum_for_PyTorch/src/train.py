...
def run(args, device_id, error_queue):
    ...
    try:
        gpu_rank = distributed.multi_init(device_id, args.world_size, args.gpu_ranks, args.train_npu)
...
def validate(args,  device_id, pt, step):
    device = "npu" if args.train_npu else "cpu"
    if (pt != ''):
        test_from = pt
    else:
    ...
    if args.train_with_amp:
        model = amp.initialize(model, opt_level="O1", loss_scale=128.0)
...
def train(args, device_id):
    ...
    if args.train_with_amp:    
        model, optimizer = amp.initialize(model, optim.optimizer, opt_level="O1", loss_scale=128.0, combine_grad=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-train_npu", type=str2bool, nargs='?', const=True, default=False)
    parser.add_argument("-train_with_amp", type=str2bool, nargs='?', const=True, default=False)
    parser.add_argument("-prof", type=str2bool, nargs='?', const=True, default=False)
    ...
    init_logger(args.log_file)
    device = "npu" if args.train_npu else "cpu"
    device_id = 0 if device == "npu" else -1
