...
class Batch(object):   
    def _pad(self, data, pad_id, width=-1, is_label=False):
        if (width == -1):
            if is_label:
    ...
    def __init__(self, data=None, device=None,  is_test=False):
        ...
        if data is not None:
            ...
            labels = torch.tensor(self._pad(pre_labels, 0, is_label=True))
            segs = torch.tensor(self._pad(pre_segs, 0))
            mask = ~ (src == 0)
            clss = torch.tensor(self._pad(pre_clss, -1, is_label=True))
            mask_cls = ~ (clss == -1)
            clss[clss == -1] = 0
