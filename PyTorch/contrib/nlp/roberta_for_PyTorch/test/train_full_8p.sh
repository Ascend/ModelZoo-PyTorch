...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a ' accuracy ' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep -a ' valid '|tail -1|awk -F 'accuracy' '{print $2}'|awk -F '|' '{print $1}'`
#打印，不需要修改
