...
then
    ...
    python3 train.py --data_path=${data_path} --output_path=${output_path} --num_epoch=${train_epochs} 1>${print_log} 2>&1
fi

# 性能相关数据计算
Time1 = `grep "Training Epoch 2" ${print_log} | awk -F 'sec/step' '{print $2}' | tr -d " " | awk 'END {print $NF}'`
Time2 = `grep "Training Epoch 2" ${print_log} | awk -F 'sec/step' '{print $2}' | tr -d " " | head -n 1`
Time = `awk 'BEGIN{printf "%.4f\n", '${Time1}'-'${Tim2}'}'`
StepTime=`awk 'BEGIN{printf "%.4f\n", '${Time}'/'4855'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`

# 提取所有loss打印信息
grep "Loss " ${print_log} | awk '{print $7}' | tr -d "," > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
