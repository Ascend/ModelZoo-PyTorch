...
def main(args):
    if args.local_rank == -1 or args.no_cuda:
        ...
        n_npu = 8
    logger.info("train on device {}, rank {}".format(device, args.local_rank))
    ...
    if not os.path.exists(args.output_dir) and n_npu > 1 and dist.get_rank() == 0:
        os.makedirs(args.output_dir)

    if not os.path.exists(args.output_dir) and n_npu == 1:
        os.makedirs(args.output_dir)
        ...
        if os.path.exists(args.data_file):
          with open(args.data_file, "rb") as handle:
            ...
            eval_features = convert_examples_to_features(
                ...
                is_training=False)
            if n_npu > 1 and dist.get_rank() == 0:
    ...
    if args.do_train:
        start_time = time.time()
        if os.path.exists(args.traindata_file):
          with open(args.traindata_file, "rb") as handle:
              train_examples, train_features = pickle.load(handle)
        else:
            ...
            train_features = convert_examples_to_features(
                    ...
                    is_training=True)
            if n_npu > 1 and dist.get_rank() == 0:
                with open(args.traindata_file, "wb") as handle:
                    pickle.dump((train_examples, train_features), handle)
            if n_npu == 1:
                with open(args.traindata_file, "wb") as handle:
                    pickle.dump((train_examples, train_features), handle)
        print("after load train_features: ", time.time() - start_time)  # 109.45532488822937
        ...
        if n_npu > 1:
            ...
            train_dataloader = torch.utils.data.DataLoader(
                    ...
                    pin_memory=False,
                    sampler=train_sampler if args.do_train else None,
                    drop_last=True)
        else:
        ...
        for lr in lrs:
            ...
            if args.fp16:
                optimizer = NpuFusedAdamW(optimizer_grouped_parameters, lr=lr)
                is_ddp = True if args.local_rank != -1 else False
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                FPS_epoch = (num_steps - 5) * args.train_batch_size * n_npu/ float(epoch_time)
                logger.info(f"FPS_epoch {FPS_epoch}")
                if args.num_train_epochs == 1:# 仅测试性能时，num_train_epochs=1，此时输出FPS后就结束程序
                    sys.exit(0)
...
if __name__ == "__main__":
        ...
        # add for npu
        parser.add_argument("--data_file", default="eval_features.pkl", type=str, help="filename to save eval_features")
        parser.add_argument("--traindata_file", default="train_features.pkl", type=str, help="filename to save train_features")
