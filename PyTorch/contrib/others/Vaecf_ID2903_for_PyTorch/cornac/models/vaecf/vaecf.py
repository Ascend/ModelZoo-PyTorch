...
def learn(
    ...
    print("use current device {}".format(device))
    from apex import amp
    import apex
    #optimizer = torch.optim.Adam(params=vae.parameters(), lr=learn_rate)
    optimizer = apex.optimizers.NpuFusedAdam(params=vae.parameters(), lr=learn_rate)
    vae, optimizer = amp.initialize(vae, optimizer, opt_level="O1", loss_scale=32.0, combine_grad=True,)
    ...
    for _ in progress_bar:
        ...
        for batch_id, u_ids in enumerate(
            ...
            count += len(u_batch)
            if batch_id % 10 == 0:
                progress_bar.set_postfix(loss=11)
                progress_bar.set_postfix(loss=(sum_loss / count))
            # 打印profiler结果信息
            # print(prof)
            # 导出chrome_trace文件到指定路径
            # output_path = '/home/test_user06/vaecf/profile_data.json'
