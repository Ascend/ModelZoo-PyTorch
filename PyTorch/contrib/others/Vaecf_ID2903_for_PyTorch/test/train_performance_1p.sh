...
if [ x"${modelarts_flag}" != x ];
then
     python3.7 ./run.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps}
else
    python3.7 ./run.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps} 1>${print_log} 2>&1
fi
