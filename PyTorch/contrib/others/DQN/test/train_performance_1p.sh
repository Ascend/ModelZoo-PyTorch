...
echo "------------------ Final result ------------------"
#输出性能FPS
FPS=`grep -a 'INFO: steps' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_performance_1p_${ASCEND_DEVICE_ID}.txt | tail -1 | awk -F " " '{print $9}'`
#打印
...
#训练用例信息
DeviceType=`uname -m`
CaseName=${Network}_bs_${RANK_SIZE}'p'_'perf'
