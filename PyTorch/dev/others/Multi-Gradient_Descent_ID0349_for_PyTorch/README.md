...
**描述（Description）：基于PyTorch框架深度学习模型的训练代码**

<h2 id="概述.md">概述</h2>

-   该框架将使用户能够轻松创建，训练，测试和部署以多目标为重点的深度学习模型。该框架易于使用和理解。它是以“Lego fashion”开发的，这意味着我们已经提供了开箱即用的模型、损失函数、指标，可轻松用于构建和训练模型，但我们也为用户提供了充分的灵活性来定义他们自己的模型，损失，指标仅通过实现抽象类。
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/ascend/modelzoo/tree/master/built-in/PyTorch/Research/recommendation/Multi-Gradient_Descent_ID0349_for_PyTorch
        branch=master
        commit_id=
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./Multi-Gradient_Descent_ID0349_for_PyTorch/test/train_performance_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于./Multi-Gradient_Descent_ID0349_for_PyTorch/test/train_performance_1p.sh） 
