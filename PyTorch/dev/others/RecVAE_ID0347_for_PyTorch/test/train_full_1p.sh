...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="RecVAE_ID0347_for_PyTorch"
#训练epoch
train_epochs=50
#训练batch_size
batch_size=500
...
#训练开始时间，不需要修改
start_time=$(date +%s)
nohup python3 run.py --n-epochs=${train_epochs} --dataset=${data_path} ${PREC} \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "epoch" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |grep -wvE "step:234|step:1|step:2"|awk -F "time: " '{print $2}' |tail -n +3|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
...
#获取精度：
train_accuracy=`grep "best valid" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk -F "best valid: " '{print $2}'|awk -F " |" '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#性能看护结果汇总
#训练用例信息，不需要修改
...
grep "loss: " $path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F "loss: " '{print $2}'|awk -F ", time" '{print $1}' > $path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
