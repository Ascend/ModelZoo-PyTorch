...
export MASTER_PORT=`expr 23456 + $ASCEND_DEVICE_ID`
export RANK=0
export WORLD_SIZE=1
...
m=$((${train_epochs}-1))
checkpoint_path=${cur_path}/../output/moby__swin_tiny__patch4_window7_224__odpr02_tdpr0_cm099_ct02_queue4096_proj2_pred2/default/ckpt_epoch_$m.pth
...
do
    ...
    wait
    nohup python3 moby_linear.py --eval --resume ${checkpoint_path} --cfg configs/moby_swin_tiny.yaml --local_rank 0 --data-path $data_path --batch-size ${batch_size} >> ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
