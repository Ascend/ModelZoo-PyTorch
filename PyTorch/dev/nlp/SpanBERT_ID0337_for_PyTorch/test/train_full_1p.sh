...
#训练开始时间，不需要修改
start_time=$(date +%s)
nohup python3 ${cur_path}/code/run_squad.py  --data_path=${data_path} --do_train --do_eval --model spanbert-base-cased \
         --train_file train-v1.1.json --dev_file dev-v1.1.json \
         --train_batch_size $batch_size --eval_batch_size $batch_size \
         --learning_rate 2e-5 --num_train_epochs 4 --max_seq_length 512 \
         --doc_stride 128 --eval_metric f1 --eval_per_epoch 10 --fp16 \
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep '!!! Best' $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk -F '):' '{print $2}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
