    ...
    def forward(self, input):
        if not self.bias is None:
            if input.dim() == 2:
                ...
                return self.biased_act_fn(self.bias, torch.npu_linear(input, self.weight, None)).view(input_shape[0],
                                                                                                      input_shape[1],
                                                                                                      self.out_features)
        else:
            if input.dim() == 2:
                return self.act_fn(torch.npu_linear(input, self.weight, None))
            else:
                input_shape = input.size()
                input = input.view(-1, self.in_features)
                return self.act_fn(torch.npu_linear(input, self.weight, None)).view(input_shape[0], input_shape[1],
                                                                                    self.out_features)
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x, batch):
        new_x_shape = (batch, x.size()[0] // batch) + (self.num_attention_heads, self.attention_head_size)
        return x.npu_confusion_transpose((0, 2, 1, 3), new_x_shape, False)

    def transpose_key_for_scores(self, x, batch):
        new_x_shape = (batch, x.size()[0] // batch) + (self.num_attention_heads, self.attention_head_size)
        return x.npu_confusion_transpose((0, 2, 3, 1), new_x_shape, False)

    def forward(self, hidden_states, attention_mask, batch):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)

        query_layer = self.transpose_for_scores(mixed_query_layer, batch)
        key_layer = self.transpose_key_for_scores(mixed_key_layer, batch)
        value_layer = self.transpose_for_scores(mixed_value_layer, batch)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, batch):
        self_output = self.self(input_tensor, attention_mask, batch)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, batch):
        attention_output = self.attention(hidden_states, attention_mask, batch)
    ...
    def forward(self, hidden_states, attention_mask, batch):
        ...
        if self._checkpoint_activations:
            ...
            for i,layer_module in enumerate(self.layer):
                hidden_states = layer_module(hidden_states, attention_mask, batch)
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states, batch):
        ...
        # to the first token.
        first_token_tensor = hidden_states[:batch, :]
...
class BertPreTrainedModel(nn.Module):
    ...
    def init_bert_weights(self, module):
        ...
        """
        if isinstance(module, (NpuLinear, nn.Embedding)):
        ...
        if isinstance(module, NpuLinear) and module.bias is not None:
            module.bias.data.zero_()
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, batch):
        ...
        extended_attention_mask = extended_attention_mask.expand(attention_mask_shape[0], self.num_attention_heads,
                                                                 attention_mask_shape[1], attention_mask_shape[1]).\
            clone().npu_format_cast(29)
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        embedding_output = self.embeddings(input_ids, token_type_ids)
        embedding_output_shape = embedding_output.size()
        embedding_output = embedding_output.view(embedding_output_shape[0]*embedding_output_shape[1],-1)
        encoded_layers = self.encoder(embedding_output, extended_attention_mask, batch)
        sequence_output = encoded_layers[-1]
        pooled_output = self.pooler(sequence_output, batch)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = NpuLinear(config.hidden_size, num_labels)
        self.apply(self.init_bert_weights)
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config, num_choices):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = NpuLinear(config.hidden_size, 1)
        self.apply(self.init_bert_weights)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = NpuLinear(config.hidden_size, 2)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids, attention_mask):
        batch, _ = attention_mask.size()
        encoded_layers, _ = self.bert(input_ids, token_type_ids, attention_mask, batch)
        sequence_output = encoded_layers[-1]
        logits = self.qa_outputs(sequence_output)
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1).view(batch, -1)
        end_logits = end_logits.squeeze(-1).view(batch, -1)
