...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Swin-Transformer_ID2375_for_PyTorch"
#训练epoch
train_epochs=30
#训练batch_size
batch_size=32
...
sed -i "s|if idx > 50: break|if idx > 50: pass|g" main.py
...
do
    ...
    nohup python3 main.py --cfg configs/swin_mlp_tiny_c12_patch4_window8_256.yaml --local_rank $ASCEND_DEVICE_ID --data-path $data_path --batch-size 32 MODEL.DEVICE npu  > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
sed -i "s|if idx > 50: pass|if idx > 50: break|g" main.py
