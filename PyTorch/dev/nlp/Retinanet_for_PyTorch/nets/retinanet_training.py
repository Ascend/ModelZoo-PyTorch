...
def get_target(anchor, bbox_annotation, classification, npu):
    #------------------------------------------------------#
    #   计算真实框和先验框的交并比
...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations, alpha = 0.25, gamma = 2.0, npu = True):
        #---------------------------#
        #   获得batch_size的大小
        #---------------------------#
        ...
        for j in range(batch_size):
            ...
            if len(bbox_annotation) == 0:
                ...
                #-------------------------------------------------------#
                if npu:
                    regression_losses.append(torch.tensor(0).to(dtype).npu())
                else:
            ...
            targets, num_positive_anchors, positive_indices, assigned_annotations = get_target(anchor, 
                                                                                        bbox_annotation, classification, npu)
