...
do
    ...
    nohup python3 train.py $data_path/iwslt14.tokenized.de-en \
        ...
        --distributed-world-size 8 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
FPS=`grep -rn "wps=" ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F "wps=" '{print $2}' | awk -F "," '{print $1}' | awk '{if(NR>=38){print}}' | awk 'END {print}' |sed s/[[:space:]]//g`
