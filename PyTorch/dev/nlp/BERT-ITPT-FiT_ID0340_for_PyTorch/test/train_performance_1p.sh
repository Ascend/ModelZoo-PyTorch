...
#获取编译时间
CompileTime=`grep "Train-Time: " $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | head -2 | awk -F 'Train-Time: ' '{print $2}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
