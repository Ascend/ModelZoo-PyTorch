...
#训练开始时间，不需要修改
start_time=$(date +%s)
nohup python3 train.py --backbone resnet --lr 0.007 --workers 4 --epochs 50 --device-id ${ASCEND_DEVICE_ID} --batch-size 16 --checkname deeplab-resnet --eval-interval 1 --dataset pascal --data_path=${data_path} > $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
wait
#训练结束时间，不需要修改
end_time=$(date +%s)
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep epoch: $path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Train-time:" '{print$2}' | tail -n+5|awk '{sum+=$1} END {print"",sum/NR}' | sed s/[[:space:]]//g`
FPS=`python3 -c "print(${batch_size}/${TrainingTime})"`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep mIoU: $path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F "mIoU:" '{print$2}' | awk -F "," '{print$1}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
