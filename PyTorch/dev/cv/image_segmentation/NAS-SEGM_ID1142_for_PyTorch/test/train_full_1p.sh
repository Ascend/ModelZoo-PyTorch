...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
Time=`grep "Avg. Loss" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F "Avg. Time" '{print $2}' | cut -b 3-7 |awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
