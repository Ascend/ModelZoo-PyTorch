...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DINO_xcit_small12-linear_ID3269_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=128
...
nohup python3  ${cur_path}/../eval_linear.py \
    --pretrained_weights $ckpt_path/dino_xcit_small_12_p16_pretrain.pth \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
Time=`grep Epoch $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep eta|awk -F 'time: ' '{print $2}'|awk '{print $1}'|tail -n +3|awk '{sum+=$1} END {print sum/NR}'|sed s/[[:space:]]//g`
FPS=`awk 'BEGIN{printf "%.2f\n",'${batch_size}'/'${Time}'}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#获取编译时间
CompileTime=`grep "Epoch:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -1 | awk -F "time:" '{print $2}' | awk -F " " '{print $1}' |sed s/[[:space:]]//g`

#输出训练精度,需要模型审视修改
#train_accuracy=`grep accuracy $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep -v mlp_log|awk 'END {print $5}'| sed 's/,//g' |cut -c 1-5`
#打印，不需要修改
...
DeviceType=`uname -m`
ModelParam=ps16
CaseName=${Network}_bs${BatchSize}_${ModelParam}_${RANK_SIZE}'p'_'perf'
