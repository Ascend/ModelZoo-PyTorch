...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DINO_xcit_medium24_ID3265_for_PyTorch"
#训练epoch
train_epochs=10
#训练batch_size
batch_size=8
...
# need to enable use_fp16, but wait for pytorch1.8
nohup python3 ${DISTRIBUTED}  ${cur_path}/../main_dino.py \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
Time=`grep time $cur_path/output/0/train_0.log|grep eta|awk -F 'time: ' '{print $2}'|awk '{print $1}'|tail -n +3|awk '{sum+=$1} END {print sum/NR}'|sed s/[[:space:]]//g`
...
#稳定性精度看护结果汇总
#输出训练精度,需要模型审视修改
train_accuracy=`grep "Top1" $cur_path/output/0/test.log|awk -F 'Top1: ' '{print $2}'|awk -F ',' '{print $1}'| awk 'BEGIN {max = 0} {if ($1+0>max+0) max=$1 fi} END {print "", max}' | sed 's/[[:space:]]//g'`
#打印，不需要修改
