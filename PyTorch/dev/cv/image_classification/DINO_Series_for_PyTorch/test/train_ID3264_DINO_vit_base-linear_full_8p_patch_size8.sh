...
DISTRIBUTED="-m torch.distributed.launch --nproc_per_node=8"
...
nohup python3 ${DISTRIBUTED}  ${cur_path}/../eval_linear.py \
    ...
    --avgpool_patchtokens true > ${cur_path}/output/0/train_0.log 2>&1 &      
wait
#conda deactivate
#训练结束时间，不需要修改
end_time=$(date +%s)
...
DeviceType=`uname -m`
ModelParam=ps8
CaseName=${Network}_bs${BatchSize}_${ModelParam}_${RANK_SIZE}'p'_'acc'
