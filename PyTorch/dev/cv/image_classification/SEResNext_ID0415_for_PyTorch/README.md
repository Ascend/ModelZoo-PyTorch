...
**描述（Description）：基于PyTorch框架的图像分类的卷积网络训练代码**

<h2 id="概述.md">概述</h2>

-    图像分类是将图像分类为几个预定义类别之一的任务，通常还会给出输入属于某个类别的概率。这项任务对于理解和分析图像至关重要，而借助我们复杂的视觉系统，它可以毫不费力地实现。当今最强大的图像分类模型是使用某种形式的卷积神经网络（CNN）构建的，而卷积神经网络也是计算机视觉中许多其他任务的基础。
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./SEresNext_ID0415_for_PyTorch/test/train_performance_1p.sh），示例如下。data_path为用户数据集的路径。
            
        
        ```
        `nohup python3 ./main.py \
                ${data_path} \
                --raport-file ${data_path}/raport.json \
                -j8 \
                -p 100 \
        ...
        2.2 单卡训练指令（脚本位于./SEresNext_ID0415_for_PyTorch/test/train_performance_1p.sh） 
