...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DBPN_ID2917_for_PyTorch"
#训练epoch
train_epochs=400
#训练batch_size
batch_size=1
...
do
    ...
    nohup python3 main.py --data_dir $data_path --batchSize $batch_size --nEpochs $train_epochs \
    --distributed --lr=$learning_rate --input_dir $data_path > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "PSNR" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "PSNR:" '{print $2}'|awk '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
