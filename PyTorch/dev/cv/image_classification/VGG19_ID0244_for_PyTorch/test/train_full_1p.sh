...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="VGG19_ID0244_for_PyTorch"
#训练epoch
train_epochs=90
#训练batch_size
batch_size=32
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep -a 'img/s'  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $10}'|awk '{sum+=$1} END {print sum/NR}'`
#打印，不需要修改
...
grep Epoch: $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk -F ":" '{print $8}' | awk -F "(" '{print $2}' | awk -F ")" '{print $1}' >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
