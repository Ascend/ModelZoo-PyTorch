...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="deit_ID2467_for_PyTorch"
#训练epoch
train_epochs=300
#训练batch_size
batch_size=128  
...
#参数修改
sed -i "s|print_freq = 10|print_freq = 1|g" ${cur_path}/engine.py
...
#参数回退
sed -i "s|print_freq = 1|print_freq = 10|g" ${cur_path}/engine.py
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep fps $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'fps: ' '{print $2}'|awk '{print $1}'|tail -n +5|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep Acc@1 $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'Acc@1 ' '{print $2}'|awk '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
