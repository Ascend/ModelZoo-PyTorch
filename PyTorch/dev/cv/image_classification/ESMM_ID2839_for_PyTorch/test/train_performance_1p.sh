...
sed -i "s|./data|$data_path|g" utils.py
sed -i "s|epoch = 10|epoch = 1|g" main.py
cp $data_path/model* $cur_path/../model
...
#恢复参数
sed -i "s|$data_path|./data|g" utils.py
sed -i "s|epoch = 1|epoch = 10|g" main.py
rm -rf $cur_path/../model/*
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
time=`grep Epoch  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "time/step:" '{print $2}'|tail -n +3|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
FPS=`awk 'BEGIN{printf "%.2f\n",'${batch_size}'/'${time}'}'`

#获取编译时间
CompileTime=`grep "Epoch:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -2 | awk -F "time/step:" '{print $2}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`

#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep 'val loss' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |awk -F "marry auc is " '{print $2}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
