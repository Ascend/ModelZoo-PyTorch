...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "train_accuracy:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $6}'|cut -c 1-6|awk 'BEGIN {max = 0} {if ($1>max) max=$1 fi} END {printf "%.4f\n", max}'`
