...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep FPS@all ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep -v Test|awk -F " " '{print $11}'|sed s/[[]:space:]//g`
# 打印，不需要修改
echo "Final Performance images/sec : $FPS"

#获取编译时间
CompileTime=`grep "Epoch:"  ${test_path_dir}/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | head -12 | tail -1 | awk -F " " '{print $5}'|sed s/[[:space:]]//g`
