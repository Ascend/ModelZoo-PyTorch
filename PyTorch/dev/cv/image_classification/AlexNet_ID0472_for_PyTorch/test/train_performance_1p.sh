...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="AlexNet_ID0472_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=128
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep -a 'Epoch:'  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "img/s: " '{print $2}'|awk -F " " '{print $1}'|awk '{sum+=$1} END {print sum/NR}'`
#打印，不需要修改
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'Test:' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "acc1:" '{print $2}'|awk -F " " '{print $1}'|awk '{if(length !=0) print $0}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
...
grep -a 'Epoch:'  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "loss: " '{print $2}'|awk -F " " '{print $1}'| awk '{if(length !=0) print $0}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
