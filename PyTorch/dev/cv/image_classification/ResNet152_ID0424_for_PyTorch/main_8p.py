...
def main():
    ...
    loc = 'npu:{}'.format(NPU_CALCULATE_DEVICE)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.gpu is None:
                ...
                checkpoint = torch.load(args.resume, map_location=f'npu:{NPU_CALCULATE_DEVICE}')
            args.start_epoch = checkpoint['epoch']
            best_acc1 = checkpoint['best_acc1']
            if args.gpu is not None:
                # best_acc1 may be from a checkpoint from a different GPU
                best_acc1 = best_acc1.to(f'npu:{NPU_CALCULATE_DEVICE}')
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=train_loader_batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True, sampler=train_loader_sampler, drop_last = True)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            images = images.npu(f'npu:{NPU_CALCULATE_DEVICE}', non_blocking=True)
