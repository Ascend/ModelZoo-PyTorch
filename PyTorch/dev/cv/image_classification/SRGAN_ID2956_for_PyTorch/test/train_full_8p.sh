...
sed -i "s|lr = 1e-4|lr = $learning_rate|g" train_srgan.py
...
sed -i "s|lr = $learning_rate|lr = 1e-4|g" train_srgan.py
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep PSNR $cur_path/test/output/${ASCEND_DEVICE_ID}/eval_${ASCEND_DEVICE_ID}.log|awk -F 'PSNR - ' '{print $2}'`
#打印，不需要修改
