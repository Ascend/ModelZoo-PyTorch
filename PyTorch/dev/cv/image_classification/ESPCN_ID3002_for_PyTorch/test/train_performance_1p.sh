...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="ESPCN_ID3002_for_PyTorch"
#训练epoch
train_epochs=2
#训练batch_size
batch_size=16
...
#输出CompileTime
CompileTime=`grep "time/step(s)" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log |head -n 2|awk -F ',' '{print$4}'|awk -F ':' '{sum+=$2} END {print sum}'`
