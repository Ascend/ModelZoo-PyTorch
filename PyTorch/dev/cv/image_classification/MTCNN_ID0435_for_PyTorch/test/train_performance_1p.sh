...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="MTCNN_ID0435_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=32
...
sed -i "s|../datasets|$data_path|g" data_loader/data_loaders.py
sed -i "s|: 1000|: 1|g" config.json
...
do
    ...
    nohup python3 train.py --config config.json --apex  --loss_scale 128 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait

#恢复
sed -i "s|$data_path|../datasets|g" data_loader/data_loaders.py
sed -i '29s/1/1000/' config.json
#conda deactivate
#训练结束时间，不需要修改
end_time=$(date +%s)
...
#获取编译时间
CompileTime=`grep "Epoch:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -2 | awk -F ":" '{print $5}' | awk -F "," '{print $1}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
