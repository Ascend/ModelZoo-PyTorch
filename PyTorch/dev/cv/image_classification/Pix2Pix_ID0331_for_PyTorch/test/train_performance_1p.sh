...
do
    ...
    nohup python3 implementations/pix2pix/pix2pix.py --data_path=${data_path} --n_epochs=${train_epochs} --apex --loss_scale 128 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
if [ $? -ne 0 ];then
  exit 1
fi
done
...
#获取编译时间
CompileTime=`grep "time:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | head -2 | awk -F "time:" '{print $2}' | awk -F "]" '{print $1}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
