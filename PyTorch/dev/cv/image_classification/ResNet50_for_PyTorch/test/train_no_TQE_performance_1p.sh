...
if [[ $precision_mode == "must_keep_origin_dtype" ]];then
        CaseName=${Network}_bs${BatchSize}_${RANK_SIZE}'p'_'fp32'_notqe_'perf'
else
        CaseName=${Network}_bs${BatchSize}_${RANK_SIZE}'p'_notqe_'perf'
fi
...
#获取编译时间
CompileTime=`grep "Epoch:" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -2 | awk -F ' ' '{print $5}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
