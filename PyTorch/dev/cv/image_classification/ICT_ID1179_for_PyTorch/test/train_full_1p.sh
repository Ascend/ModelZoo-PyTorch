...
#输出性能FPS，需要模型审视修改
TrainingTime=0
acc=`grep Epoch $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep -v "\[0/450"| awk -F "Time" '{print $2}'| awk '{print$1}'|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
