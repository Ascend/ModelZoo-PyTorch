...
start=$(date +%s)
nohup python3  main.py  --dataset cifar10  --num_labeled 400 --num_valid_samples 500 --root_dir experiments/ --data_dir $data_path --batch_size 100  --arch cnn13 --dropout 0.0 --mixup_consistency 100.0 --pseudo_label mean_teacher  --consistency_rampup_starts 0 --consistency_rampup_ends 100 --epochs 1  --lr_rampdown_epochs 450 --print_freq 200 --momentum 0.9 --lr 0.1 --ema_decay 0.999  --mixup_sup_alpha 1.0 --mixup_usup_alpha 1.0 > $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
end=$(date +%s)
e2e_time=$(( $end - $start ))
...
#输出性能FPS，需要模型审视修改
TrainingTime=0
perf=`grep Epoch $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep -v "\[0/450"| awk -F "Time" '{print $2}'| awk '{print$1}'|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
echo "$perf"
...
#精度值
train_accuracy=`grep "* Prec@1" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk '{print $3}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
