...
#进入训练脚本目录，需要模型审视修改
cd $cur_path/../
sed -i "s|VOCdevkit/VOC2007|$data_path/VOCdevkit/VOC2007|g" ${cur_path}/../config.json
sed -i "s|VOCdevkit/VOC2012|$data_path/VOCdevkit/VOC2012|g" ${cur_path}/../config.json
sed -i "s|MobileNetV2|MobileNetV1|g"${cur_path}/../config.json
#修改epoch，冒烟
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "FPS = " $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "FPS = " '{print $2}'|awk -F ", step_time" '{print $1}' |awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
#打印，不需要修改
...
grep "Loss " $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Loss " '{print $2}'|cut -c 1-6 > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
