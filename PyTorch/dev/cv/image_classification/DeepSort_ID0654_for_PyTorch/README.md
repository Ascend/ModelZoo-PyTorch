...
**描述（Description）：基于PyTorch框架的DeepSort深层排序训练代码**

<h2 id="概述.md">概述</h2>

-    DeepSort是用pytorch实现的深层排序。
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/ascend/modelzoo/tree/master/built-in/PyTorch/Research/cv/image_classification/DeepSort_ID0654_for_PyTorch
        branch=master
        commit_id= 
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./DeepSort_ID0654_for_PyTorc/test/train_performance_1p.sh），示例如下。data_path为用户数据集的路径：
            
        
        ```
        `nohup python3 deep/train.py --data-dir $data_path/data`
        ```
        
        
        
        2.2 单卡训练指令（脚本位于./DeepSort_ID0654_for_PyTorc/test/train_performance_1p.sh） 
