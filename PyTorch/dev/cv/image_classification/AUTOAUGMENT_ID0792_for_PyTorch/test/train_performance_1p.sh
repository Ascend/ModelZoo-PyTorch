...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="AUTOAUGMENT_ID0792_for_PyTorch"
#训练epoch
train_epochs=3
#训练batch_size
batch_size=128
...
sed -i "s|'~/data'|'$data_path'|g" train.py
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "fps" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F " " 'END {print $8}'|cut -c 1-9 `
#打印，不需要修改
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "val_acc" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $NF}'|awk 'BEGIN {max = 0} {if ($1>max) max =$1 fi} END {printf "%.4f\n", max}'`
#打印，不需要修改
...
grep "loss " $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $2}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
