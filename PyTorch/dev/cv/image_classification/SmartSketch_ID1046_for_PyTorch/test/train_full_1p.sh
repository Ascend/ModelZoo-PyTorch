...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "time:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "time: " 'END {print $5}'|awk -F ")" '{print $1}'|awk '{sum+=$1} END {print sum/NR*1000}'`
