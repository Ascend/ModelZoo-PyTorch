...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="DeiT_ID1558_for_PyTorch"
#训练epoch
train_epochs=300
#训练batch_size
batch_size=64
...
#参数修改
#sed -i "s|print_freq = 10|print_freq = $print_freq|g" $cur_path/../engine.py
#sed -i "s|pass|break|g" $cur_path/../engine.py
for((RANK_ID=$RANK_ID_START;RANK_ID<$((RANK_SIZE+RANK_ID_START));RANK_ID++));
do
    #设置环境变量，不需要修改
    export ASCEND_DEVICE_ID=$RANK_ID
    export NPU_CALCULATE_DEVICE=$ASCEND_DEVICE_ID
    export RANK=$NPU_CALCULATE_DEVICE
...
#参数回退
#sed -i "s|print_freq = $print_freq|print_freq = 10|g" $cur_path/../engine.py
#sed -i "s|break|pass|g" $cur_path/../engine.py
...
grep "Epoch:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep eta|awk -F "loss:" '{print $2}'|awk -F "(" '{print $1}'|sed s/[[:space:]]//g > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
