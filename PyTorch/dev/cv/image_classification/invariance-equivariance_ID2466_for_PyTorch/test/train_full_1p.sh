...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="invariance-equivariance_ID2466_for_PyTorch"
#训练epoch
train_epochs=100
#训练batch_size
batch_size=64
...
#sed -i "s|continue|break|g" main.py
...
do
    ...
    nohup python3 train.py --model resnet12 --model_path save --dataset CIFAR-FS --data_root $data_path --n_aug_support_samples 5 --n_ways 5 --n_shots 1 --epochs $train_epochs --lr_decay_epochs 60 --gamma 1.0 --contrast_temp 1.0 --mvavg_rate 0.99 --memfeature_size 64 --batch_size $batch_size --tags CIFAR-FS,INV_EQ > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait

#恢复参数
#sed -i "s|$data_path|./data|g" examples/cats_and_dogs.py
...
train_accuracy=`grep "FPS" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Acc@1=" '{print $2}'|awk -F "," '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
