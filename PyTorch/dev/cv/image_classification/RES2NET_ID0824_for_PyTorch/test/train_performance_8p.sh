...
do
    ...
    nohup python3 main.py \
        ...
        --log_dir=${cur_path}/test/output/$ASCEND_DEVICE_ID \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "Epoch:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "FPS: " '{print $2}'| awk -F " " '{print $1}' | tail -n 500 | awk '{sum+=$1} END {print sum/500}'`
#打印，不需要修改
...
train_accuracy1=`grep " * Prec@1" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk '{print $4}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
