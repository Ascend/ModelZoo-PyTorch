...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="RES2NET_ID0824_for_PyTorch"
#训练epoch
train_epochs=2
#训练batch_size
batch_size=256
...
do
    ...
    nohup python3 main.py \
        ...
        --model_save_dir ${cur_path}/test/output/$ASCEND_DEVICE_ID/ckpt > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "Epoch:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "FPS: " '{print $2}'|awk '{sum+=$1} END {print sum/NR}'|sed s/[[:space:]]//g`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#获取编译时间
CompileTime=`grep "Epoch:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -2 | awk -F "Time:" '{print $2}' | awk -F " " '{print $1}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`

#输出训练精度,需要模型审视修改
#train_accuracy=`grep "Prec@5:" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F ":" 'END {print $8}'|cut -c 1-6`
train_accuracy1=`grep " * Prec@1" $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $4}'`
#打印，不需要修改
...
grep "Loss:" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Loss:" '{print$2}' | awk '{print$1}' > $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
