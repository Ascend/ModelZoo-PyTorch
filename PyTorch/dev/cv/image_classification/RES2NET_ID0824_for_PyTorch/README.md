...
**描述（Description）：基于PyTorch框架的Res2Net训练代码**

<h2 id="概述.md">概述</h2>

-   这个 repo 是 Res2Net 在 ImageNet 上的训练代码。论文中列出的所有结果均使用此脚本进行训练。Res2Net 的训练脚本基于旧版本的 pytorch 示例。由于 pytorch 示例已更新到新版本，为确保社区重现我们在论文中的结果，我们现在开源我们的 ImageNet 训练代码。 请注意，为了公平比较，我们遵循了通用设置，并没有在我们的论文中使用任何其他数据增强技巧，例如混合和热身。
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/ascend/modelzoo/tree/master/built-in/PyTorch/Research/cv/image_classification/RES2NET_ID0824_for_PyTorch
        branch=master
        commit_id=
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./RES2NET_ID0824_for_PyTorch/test/train_performance_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于./RES2NET_ID0824_for_PyTorch/test/train_performance_1p.sh） 
