...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Token-to-Token-ViT_ID2668_for_PyTorch"
#训练epoch
train_epochs=20
#训练batch_size
batch_size=64
...
train_accuracy=`grep "val_acc"  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "val_acc:" '{print $2}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
