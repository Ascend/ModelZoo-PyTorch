...
#输出训练精度,需要模型审视修改
train_accuracy=`grep val_iou $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print $5}'`
#打印，不需要修改
...
grep -rns loss $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep -v once|grep -v "/9" |grep val_loss -B 1|grep -v -|grep -v val > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
