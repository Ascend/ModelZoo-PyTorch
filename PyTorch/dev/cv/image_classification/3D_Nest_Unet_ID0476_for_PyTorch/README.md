...
**描述（Description）：基于PyTorch框架的3D_Nest_Unet图像分割训练代码**

<h2 id="概述.md">概述</h2>

-    该存储库包含基于UNet ++的图像分割模型的代码：一种在PyTorch中实现的用于医学图像分割的嵌套U-Net体系结构
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/ascend/modelzoo/tree/master/built-in/PyTorch/Research/cv/image_classification/3D_Nest_Unet_ID0476_for_PyTorch
        branch=master
        commit_id= 
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./3D_Nest_Unet_ID0476_for_PyTorch/test/train_performance_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于./3D_Nest_Unet_ID0476_for_PyTorch/test/train_performance_1p.sh） 
