...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
Timeavg=`grep "Epoch:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F " " '{print $5}' |tail -n +2|awk '{sum+=$1} END {print sum/NR}'|sed s/[[:space:]]//g `
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a '* Acc@1' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Acc@1" '{print$2}' | awk '{print$1}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
...
grep Epoch: $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Loss" '{print$2}' | awk -F "e" '{print$1}' > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
