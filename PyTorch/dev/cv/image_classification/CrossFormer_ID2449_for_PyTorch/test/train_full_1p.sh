...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="CrossFormer_ID2449_for_PyTorch"
#训练epoch
train_epochs=25
#训练batch_size
batch_size=128
...
sed -i "s|_C.TRAIN.EPOCHS = 300|_C.TRAIN.EPOCHS = 25|g" config.py
...
do
    ...
    nohup python3 main.py --cfg configs/tiny_patch4_group7_224.yaml --batch-size $batch_size --local_rank $ASCEND_DEVICE_ID --data-path $data_path --output ./output --amp-opt-level O1 > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
train_accuracy=`grep "Max accuracy:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Max accuracy:" '{print $2}'|awk -F "%" '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
