...
**描述（Description）：基于PyTorch框架的residual_adapters深度神经网络训练代码**

<h2 id="概述.md">概述</h2>

-    深度神经网络的一个实际限制是它们对单个任务和视觉领域的高度专业化。为了克服这个限制，在这些论文中，我们建议考虑神经网络的通用参数族，它们仍然包含专门的问题特定模型，但仅在少量参数上有所不同。我们研究了此类参数化的不同设计，包括串联和并联残差适配器。我们表明，为了最大限度地提高性能，有必要同时适应深度网络的浅层和深层，但所需的变化非常小。我们还表明，这些通用参数化对于迁移学习非常有效，它们优于传统的微调技术。
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/cwx_test/research_PT/tree/master/built-in/PyTorch/Research/debugging_model/residual_adapters_ID1598_for_PyTorch
        branch=master
        commit_id=
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./residual_adapters_ID1598_for_PyTorch/test/train_performance_1p.sh），示例如下。
        ...
        2.2 单卡训练指令（脚本位于./residual_adapters_ID1598_for_PyTorch/test/train_performance_1p.sh） 
