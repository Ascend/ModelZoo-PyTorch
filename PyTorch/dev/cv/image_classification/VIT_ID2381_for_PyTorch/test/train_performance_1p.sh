...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="VIT_ID2381_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=64
...
#输出编译性能
CompileTime=`grep Epoch $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| head -n +2|awk -F ":" '{print $5}'|awk -F " " '{print$1}'|awk '{sum+=$1} END{print sum}'`
