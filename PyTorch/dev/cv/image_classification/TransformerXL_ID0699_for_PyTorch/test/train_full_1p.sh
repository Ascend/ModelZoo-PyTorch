...
nohup python3 -m torch.distributed.launch \
	...
	--fp16 \
	--data=$data_path \
	--max_step=40000 > $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
end=$(date +%s)
e2e_time=$(( $end - $start ))
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep Eval $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk '{print$17}' | awk 'NR==1{min=$1;next}{min=min<$1?min:$1}END{print min}'`
