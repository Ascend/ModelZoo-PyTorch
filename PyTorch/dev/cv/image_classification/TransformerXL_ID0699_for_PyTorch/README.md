...
**描述（Description）：基于PyTorch框架的Transformer语言建模训练代码**

<h2 id="概述.md">概述</h2>

-    语言建模（LM）是自然语言处理（NLP）任务，它确定给定单词序列在句子中出现的概率。
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/xiaohuihui221111/research_model/tree/master/built-in/PyTorch/Research/debugging_model/Transformer-XL_ID0699_for_PyTorch
        branch=master
        commit_id=
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./Transformer-XL_ID0699_for_PyTorch/test/train_performance_1p.sh），示例如下。cur_path为训练代码路径，data_path为数据集路径
        ...
        2.2 单卡训练指令（脚本位于./Transformer-XL_ID0699_for_PyTorch/test/train_performance_1p.sh） 
