...
start=$(date +%s)
nohup python3 main.py $PREC --batchsize 4 --nEpochs $train_epochs > $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
end=$(date +%s)
e2e_time=$(( $end - $start ))
...
acc=`grep -a "Timer" $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |awk -F "Timer: " '{print $2}'|awk -F " sec" '{print$1}'|tail -n+3|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
