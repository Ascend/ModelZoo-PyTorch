...
**描述（Description）：基于PyTorch框架MSPN训练代码**

<h2 id="概述.md">概述</h2>

-    在这项工作中，我们设计了一个有效的网络 MSPN 来完成人体姿态估计任务。

现有的姿态估计方法分为两类：单阶段和多阶段方法。虽然多阶段方法似乎更适合该任务，但它们在当前实践中的性能不如单阶段方法。这项工作研究了这个问题。我们认为，当前多阶段方法的性能不尽如人意是由于各种设计选择的不足。我们提出了几项改进，包括单阶段模块设计、跨阶段特征聚合和粗到细监督。
...
-   适配昇腾 AI 处理器的实现：【深加工】
        ...
        ```
        https://gitee.com/ascend/modelzoo/tree/master/built-in/PyTorch/Research/cv/image_classification/MSPN_ID0960_for_PyTorch
        branch=master
        commit_id=
        ```
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./MSPN_ID0960_for_PyTorch/test/train_performance_1p.sh），示例如下。
            
        
        ```
        `nohup python3 -m torch.distributed.launch --nproc_per_node=2 exps/mspn.2xstg.mpii/train.py`
        ```
        
        
        
        2.2 单卡训练指令（脚本位于./MSPN_ID0960_for_PyTorch/test/train_performance_1p.sh） 
