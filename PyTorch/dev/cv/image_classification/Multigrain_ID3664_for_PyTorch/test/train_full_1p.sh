...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Multigrain_ID3664_for_PyTorch"
#训练epoch
train_epochs=40
#训练batch_size
batch_size=256
...
do
    ...
    --expdir experiments/joint_3B_0.5 --repeated-augmentations 3 \
    --pooling-exponent 3 --classif-weight 0.5 \
    --apex \
    --use-npu-fused-sgd \
    --train-step $train_steps \
    --imagenet-path $data_path \
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep "train_top1 " $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "train_top1 " '{print$2}'|awk -F "(" '{print$1}' |tail -5| awk '{sum+=$1} END {print"",sum/NR}'`
#打印，不需要修改
