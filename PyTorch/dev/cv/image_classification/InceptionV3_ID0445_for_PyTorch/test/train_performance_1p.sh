...
#获取编译时间
CompileFps=`grep "img/s" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | head -1 | awk -F "img/s" '{print $2}'|awk '{print$2}' |sed s/[[:space:]]//g`
CompileTime=`awk 'BEGIN{printf "%.2f\n",'${batch_size}'/'${CompileFps}'}'`

#输出训练精度,需要模型审视修改
train_accuracy=`grep -a '* Acc@1' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Acc@1" '{print $2}'|cut -c 2-6|awk 'BEGIN {max = 0} {if ($1+0>max+0) max=$1 fi} END {print max}'`
