...
do
    ...
    nohup python3 train.py --train-dir $data_path/train --val-dir $data_path/val --batch-size 256  --total-iters 120000 --full --device_id=$ASCEND_DEVICE_ID > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
Time=`grep "train_time = "  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk -F "train_time = " '{print $2}'|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
