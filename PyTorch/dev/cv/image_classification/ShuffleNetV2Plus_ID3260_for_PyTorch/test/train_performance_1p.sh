...
do
    ...
    nohup python3 train.py --train-dir $data_path/train --val-dir $data_path/val --batch-size 256 --total-iters 10 --display-interval 1 --device_id=$ASCEND_DEVICE_ID  > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
#输出编译时间
CompileTime=`grep "train_time = "  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| head -2 |awk -F "train_time = " '{print $2}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
