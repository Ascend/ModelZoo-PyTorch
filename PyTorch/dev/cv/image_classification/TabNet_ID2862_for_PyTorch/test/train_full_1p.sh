...
#输出训练精度,需要模型审视修改
train_accuracy=`grep valid_auc $cur_path/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'valid_auc: ' '{print $2}'|awk '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
