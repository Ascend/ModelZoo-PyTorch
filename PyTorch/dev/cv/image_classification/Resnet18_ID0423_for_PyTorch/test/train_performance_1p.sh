...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Resnet18_ID0423_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=32
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep -a 'img/s'  $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F "img/s" '{print $2}'|awk '{print$2}' |tail -n+2 |awk '{sum+=$1} END {print sum/NR}'|sed s/[[:space:]]//g`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#获取编译时间
CompileFps=`grep 'img/s'  $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | head -1 | awk -F "img/s:" '{print $2}'|awk -F " " '{print $1}' |sed s/[[:space:]]//g`
