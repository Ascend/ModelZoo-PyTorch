...
#sed -i "s|NUM = 5|NUM = 1|g" run_classifier.py
...
do
    ...
    nohup python3 examples/cifar/main.py $data_path -a resnet50  --gpu ${ASCEND_DEVICE_ID} --batch-size $batch_size --epochs $train_epochs > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
time=`grep "Epoch:"  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Time  " '{print $2}'|awk '{print $1}'|tail -n +2|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`
...
train_accuracy=`grep "* Acc@1" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Acc@1 " '{print $2}'|awk  '{print $1}'|awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
#打印，不需要修改
