...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep -a 'Epoch:'  $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "img/s" '{print$2}' | awk '{print$1}' | tail -n+2 | awk '{sum+=$1} END {print"",sum/NR}' | sed s/[[:space:]]//g`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep -a '* Prec@1' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F "Prec@1" '{print$2}' | awk '{print$1}' | awk 'NR==1{max=$1;next}{max=max>$1?max:$1}END{print max}'`
