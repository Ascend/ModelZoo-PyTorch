...
line=`grep "import torch" ${cur_path}/train_8p.py -n | tail -1|awk -F ':' '{print $1}'`
...
sed -i "s|iteration > max_iter|iteration > $train_steps|g" $cur_path/train_8p.py 
wait
...
sed -i "s|break|pass|g" $cur_path/train_8p.py
sed -i "s|iteration > $train_steps|iteration > max_iter|g" $cur_path/train_8p.py 
wait
