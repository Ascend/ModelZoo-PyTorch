...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="PointNet_ID0430_for_PyTorch"
#训练epoch
train_epochs=250
#训练batch_size
batch_size=32
...
do
    ...
    python3 ./utils/train_classification.py --dataset $data_path/shapenetcore_partanno_segmentation_benchmark_v0 \
    --dataset_type shapenet \
    $PREC \
    --apex \
    --apex-opt-level O2 \
    --loss-scale-value 128.0 \
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep final $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F " " '{print $3}'`
#打印，不需要修改
