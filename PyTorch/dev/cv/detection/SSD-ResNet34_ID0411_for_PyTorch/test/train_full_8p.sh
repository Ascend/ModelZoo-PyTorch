...
sed -i 's/lr=1e-3/lr=8e-3/g' train.py
...
do
    ...
    python3 train.py --ddp --strides 2 2 2 2 1 1 --batch-size 8 --image-size 800 1200 --data $data_path/../coco --epochs $train_epochs > $cur_path/output/$ASCEND_DEVICE_ID/train_${i}.log 2>&1 &
    let rank++
done 
wait
...
sed -i 's/lr=8e-3/lr=1e-3/g' train.py
#结果打印，不需要修改
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
