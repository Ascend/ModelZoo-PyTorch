...
do
    ...
    nohup python3.7 train.py ${PREC} --device_id ${ASCEND_DEVICE_ID} > ${cur_path}/test/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
FPS=`python3 -c "print(${batch_size}/8/(${temp}/${train_steps}))"`
#打印，不需要修改
