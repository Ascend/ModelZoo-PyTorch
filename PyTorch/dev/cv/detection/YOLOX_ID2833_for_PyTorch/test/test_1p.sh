...
PORT=29500 ./tools/dist_test.sh configs/yolox/yolox_m_8x8_300e_coco.py \
    ...
    --gpu-id=${device_id} > ${test_path_dir}/test_${ASCEND_DEVICE_ID}.log 2>&1 &
...
#输出训练精度,需要模型审视修改
train_accuracy=`grep -a 'Average Precision' ${test_path_dir}/test_${ASCEND_DEVICE_ID}.log|awk "NR==1"|awk -F ' ' '{print $NF}'`
#打印，不需要修改
