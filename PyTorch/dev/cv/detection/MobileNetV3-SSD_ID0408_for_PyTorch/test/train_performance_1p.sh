...
do
    ...
    nohup python3 train_ssd.py   --dataset_type open_images --datasets $data_path/open_images --apex --loss_scale 128 --net mb3-ssd-lite --scheduler cosine --lr 0.01 --t_max 100 \
	    --validation_epochs 5 --num_epochs 1 --base_net_lr 0.001 --batch_size 5  > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
grep "FPS: " $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk -F "Average Loss: " '{print $2}'|awk  -F ", " '{print$1}'  > $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
