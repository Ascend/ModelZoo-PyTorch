...
sed -i "s|learning_rate = 1e-3|learning_rate = 8e-3|g" ${cur_path}/../haparam.py
...
do
    ...
    python3 train.py --output_directory ${cur_path}/output/$ASCEND_DEVICE_ID/ckpt \
        ...
        --n_gpus 8 \
        --epochs $train_epochs \
        --distributed_run True > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done
wait
...
ASCEND_DEVICE_ID=0
sed -i "s|learning_rate = 8e-3|learning_rate = 1e-3|g" ${cur_path}/../haparam.py
#结果打印，不需要修改
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "Train loss" $cur_path/output/0/train_0.log |awk -F " " '{print $8}'|awk -F "s" '{print $1}'|tail -n +3|awk '{sum+=$1}END {print"",sum/NR}'|sed s/[[:space:]]//g`
...
grep Loss $cur_path/output/0/train_0.log|grep Train|awk -F 'Loss ' '{print$2}' |awk '{print$1}'|awk '{if(length !=0) print $0}' > $cur_path/output/0/train_${CaseName}_loss.txt

#最后一个迭代loss值，不需要修改
ActualLoss=`awk 'END {print}' $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt`

#输出精度
train_accuracy=`tail -5 $cur_path/output/0/train_${CaseName}_loss.txt| awk '{sum+=$1} END {print sum/NR}'`
