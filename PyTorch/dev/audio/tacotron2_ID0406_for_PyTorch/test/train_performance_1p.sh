...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="tacotron2_ID0406_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=48
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep train_items_per_sec  ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F ":" '{print$NF}'| tail -n +2|awk '{sum+=$1} END {print"",sum/NR}'|sed s/[[:space:]]//g`

#打印，不需要修改
echo "Final Performance images/sec : $FPS"
#输出编译时间
CompileTime=`grep train_iter_time  ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| head -2 |awk -F "train_iter_time : " '{print $2}' | awk '{sum+=$1} END {print"",sum}' |sed s/[[:space:]]//g`
...
grep train_loss $cur_path/output/nvlog.json|awk -F "train_loss\": " '{print$2}'|awk -F "}" '{print $1}'> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
