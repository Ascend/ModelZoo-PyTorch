...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Fastspeech_ID1545_for_PyTorch"
#训练epoch
train_epochs=1
#训练batch_size
batch_size=1024
...
sed -i "s|epochs = 2000|epochs = 1|g" hparams.py
...
do
    ...
    nohup python3 train.py --apex --use-npu-fused-adam --perf > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
sed -i "s|epochs = 1|epochs = 2000|g" hparams.py
...
#输出训练精度,需要模型审视修改
train_accuracy=`awk 'END {print}' $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt`
#打印，不需要修改
echo "Final Train Accuracy : ${train_accuracy}"
