    ...
    # 读取pr_filelist.txt的内容
        ...
        for key, value in dict1.items():
            if float(value) >= FileSizeLimit:
                print('{},size of file is {}M and greater than {}M,please check and delete it！'.format(key,value, FileSizeLimit))
                filesize_check = 1
            else:
                continue
        fooc.close()
                ...
                # 判断文件中是否存在关键字LICENSE/license
                    ...
                    if model_dir.exists():
                        with open(str(model_dir), 'r',encoding='gb18030',errors='ignore') as foob:
                            content = foob.read()
                            if (LICENSE in content) or (license in content) or (License in content) or (Licence in content) or (licence in content) or (LICENCE in content):
                                continue
                            else:
                                license_check = 1
                                print('{},The keyword license no exists in the file,please check and add it!'.format(model_dir_str))
                            foob.close()
                    else:
                        pass
...
def check_firstlevel_file(path_pr_list,fram_str,modelzoo_dir):
    ...
    with open(path_pr_list, 'r') as fooa:
        ...
        if firstlevel_check4 == 4:
            print('{},The {} file is non-existent in the model code of the file,Please follow the guide to add {}:'.format(fram_unst_dname, g, g))
            print('https://gitee.com/ascend/modelzoo/blob/master/contrib/CONTRIBUTING.md')
            firstlevel_check = 1
        if firstlevel_check5 == 1:
            print('{},The {} file is non-existent in the model code of the file,Please check and add {}:'.format(fram_unst_dname, h, h))
            firstlevel_check = 1
    ...
    with open(path_pr_list, 'r') as fooa:
        ...
        for filepath_inprlist in fooa:
            ...
            if pr_filelist0_str.exists():
                funk_file_typr = ['.log', '.pbtxt', '.pb', '.h5', '.so', '.zip', '.tar', '.event','.tar.gz','.swp','.ipynb','.pyc','.novalocal','.bin','.pth','.onnx','.npy','.om','.pkl','.pt','.mat','.tfrecord']
...
def check_link(path_pr_list, fram_str,modelzoo_dir,onelink):
    ...
    with open(path_pr_list, 'r') as fooa:
        ...
        for filepath_inprlist in fooa:
            ...
            if ('README' not in file_name) and ('readme' not in file_name) and ('Readme' not in file_name) and ('ReadMe' not in file_name):
                    ...
                    with open(str(pr_filelist0_str), 'r',encoding='gb18030',errors='ignore') as foo:
                        for words in foo:
                            if onelink in words:
                                ...
                                print('{},This is an internal links that includes {},please check this line that: {}'.format(filepath_inprlist,link,words))
                            else:
                                continue
                        foo.close()
                else:
    ...
    with open(path_pr_list, 'r') as fooa:
        ...
        for fram_file_dir in fooa:
            ...
            if pr_filelist0_str.exists():
                ...
                if re.findall(r'train\w*.sh', env_file_name) or re.findall(r'infer\w*.sh', env_file_name):
                    with open(str(pr_filelist0_str), 'r', encoding='gb18030', errors='ignore') as fooc:
                        for words in fooc:
                            ...
                            if re.findall(r'\w*export install_path\w*', words) or re.findall(r'\w*export LD_LIBRARY_PATH\w*', words) or re.findall(r'\w*export PATH\w*', words) \
                                or re.findall(r'\w*export ASCEND_OPP_PATH\w*', words):
                                print('{}, There are non compliant configuration environment variables, please check the line that is: {}'.format(
                                        fram_file_dir, words))
                                sensitive_check = 1
                            if re.findall(r'\w*export PYTHONPATH\w*', words) and re.findall(r'\w*install_path\w*', words):
                                print('{}, There are non compliant configuration environment variables, please check the line that is: {}'.format(
                                        fram_file_dir, words))
                                sensitive_check = 1
                        fooc.close()
                ...
                # 数据集合规性检查、扫描数据集版权合规
                if ('README' in file_name) or ('readme' in file_name) or ('Readme' in file_name) or ('ReadMe' in file_name):
                    with open(str(pr_filelist0_str), 'r', encoding='gb18030', errors='ignore') as fooc:
                        for words in fooc:
                            ...
                            for black_key in blackhttp_list:
                                if 'contrib/' in str(fram_file_dir) and 'pan.baidu.com' == black_key:
                                    ...
                                    if black_key in words:
                                        print('{}, The dataset used is not copyright compliant in the file, please check the line that is: {}'.format(
                                                fram_file_dir, words))
                                        sensitive_check = 1
    ...
    with open(path_pr_list, 'r') as fooa:
        for filepath_inprlist in fooa:
            ...
            if model_fram == '' and 'built-in' in pr_filelist0_str and 'Common' not in pr_filelist0_str:#高校网络名称不规范处理
                ...
                print('{},Network name of the file naming is not standard, please check and modify it!'.format(pr_filelist0_str))
                break
                # 配置需要读取的文件
                    ...
                    if a in content1 and Path(fram_path).exists():
                        ...
                        if (b in content2) and (c in content2) and (d in content2):
                            ...
                            try:
                                ...
                                print('When Check Modelzoo Level,Return ERROR,Please check contents of modelzoo_level.txt ')
                                modelzoo_level_check = 1
                                break
                            # 如果#modelzoo_level.txt文件中存在关键字段，判断字段是否为空
                            if level_dict[str('FuncStatus')] != '' and level_dict[str('PerfStatus')] != '' and level_dict[str('PrecisionStatus')] != '':
                                # 功能、性能、精度都OK，网路归属Official
                                if level_dict[str('FuncStatus')] != 'NOK' and level_dict[str('PrecisionStatus')] != 'NOK':
                                    #高校不校验
                                    if level_dict[str('FuncStatus')] == 'OK' and level_dict[str('PerfStatus')] == 'OK' and level_dict[str('PrecisionStatus')] == 'OK':
                                    ...
                                    elif level_dict[str('FuncStatus')] == 'OK' and level_dict[str('PerfStatus')] == 'PERFECT' and level_dict[str('PrecisionStatus')] == 'OK':
                                ...
                                # 功能not OK,网路归属非Modelzoo领域
                                    ...
                                    if level_dict[str('FuncStatus')] == 'NOK' and 'built-in' in pr_filelist0_str:
                                        ...
                                        print('{},FuncStatus is NOK ,You can not put the code under the directory of modelzoo!'.format(get_model_fram(pr_filelist0_str)))
                                    if level_dict[str('PrecisionStatus')] == 'NOK' and 'built-in' in pr_filelist0_str:
                                        modelzoo_level_check5 = 1
                                        print('{},PrecisionStatus is NOK ,You can not put the code under the directory of modelzoo!'.format(get_model_fram(pr_filelist0_str)))
                            else:
                                if level_dict[str('FuncStatus')] == '':
                                    modelzoo_level_check5 = 1
                                    print('{},The value of the FuncStatus is null in the file,please check and enter the correct value!'.format(pr_levelfile_path))
                                if level_dict[str('PerfStatus')] == '':
                                    modelzoo_level_check5 = 1
                                    print('{},The value of the PerfStatus is null in the file,please check and enter the correct value!'.format(pr_levelfile_path))
                                if level_dict[str('PrecisionStatus')] == '':
                                    modelzoo_level_check5 = 1
                                    print('{},The value of the PrecisionStatus is null in the file,please check and enter the correct value!'.format(pr_levelfile_path))
                        else:
                            if b not in content2:
                                modelzoo_level_check5 = 1
                                print('{},The keyword of the FuncStatus is not exist in the file,please check and add it!'.format(pr_levelfile_path))
                            if c not in content2:
                                modelzoo_level_check5 = 1
                                print('{},The keyword of the PerfStatus is not exist in the file,please check and  add it!'.format(pr_levelfile_path))
                            if d not in content2:
                                modelzoo_level_check5 = 1
                                print('{},The keyword of the PrecisionStatus is not exist in the file,please check and add it!'.format(pr_levelfile_path))
                    else:
                        modelzoo_level_check7 = 1
            if modelzoo_level_check7 == 1:
                modelzoo_level_check = 1
                print('{},The {} file is not exist,Please follow the guide to add {}:'.format(get_model_fram(pr_filelist0_str), a, a))
                print('https://gitee.com/ascend/modelzoo/blob/master/contrib/CONTRIBUTING.md')
                break
            if modelzoo_level_check4 == 1 and 'built-in' in pr_filelist0_str:
                modelzoo_level_check = 1
                print('{},All FuncStatus PerfStatus PrecisionStatus are OK ,You should put the code under the Official directory!'.format(get_model_fram(pr_filelist0_str)))
                break
            ...
            if modelzoo_level_check2 == 1 and 'built-in' in pr_filelist0_str:
                modelzoo_level_check = 1
                print('{},The optimal performance or accuracy are not completely achieved by studying the class model ,You should put the code under the Research directory!'.format(get_model_fram(pr_filelist0_str)))
                break
            if modelzoo_level_check1 == 1 and 'built-in' in pr_filelist0_str:
                modelzoo_level_check = 1
                print('{},PerfStatus is PERFECT and FuncStatus PrecisionStatus are OK ,You should put the code under the Benchmark directory!'.format(get_model_fram(pr_filelist0_str)))
                break
    ...
    with open(path_pr_list, 'r') as fooa:
        for filepath_inprlist in fooa:
            ...
            if file_word_check7 == 1:
                file_word_check = 1
                print('{},The train_full(1p or 8p) file is not exist,please check and add it!'.format(get_model_fram(pr_filelist0_str) + '/test'))
                break
                        ...
                        for line in open(fullfile_path,encoding='gb18030',errors='ignore'):
                            if '#' not in line and 'taskset -c' in line:
                                ...
                                if notneed_param[0] in line:
                                    if ('a=RANK_ID*$' not in file_contents) and ('a=$' not in file_contents) and ('a=RANK_ID_n*$' not in file_contents):
                                            i = 1
                                            core_binding_check = 1
