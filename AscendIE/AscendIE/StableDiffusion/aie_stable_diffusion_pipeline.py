...
class AIEStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def parse_onnx_model(self, onnx_path):
    ...
    def build_engines_onnx(self, clip_onnx_path, unet_onnx_path, vae_onnx_path):
        ...
        vae_model_data = self.parse_onnx_model(vae_onnx_path)
    ...
    def ascendie_infer(
        ...
        **kwargs,
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if self.use_parallel_inferencing:
                noise_pred = torch.from_numpy(
                    np.frombuffer(output_data, dtype=np.float32).reshape(
                        (1, 4, 64, 64)))
            else:
                noise_pred = torch.from_numpy(
                    np.frombuffer(output_data, dtype=np.float32).reshape(
        ...
        image = torch.from_numpy(
            np.frombuffer(image_buffer, dtype=np.float32).reshape(
                (1, 3, 512, 512)))
    ...
    def _encode_prompt(
        ...
        text_embeddings = torch.from_numpy(
            np.frombuffer(output_data, dtype=np.float32).reshape((1, 77, 1024)))
