...
class DecoderIter(torch.nn.Module):
    ...
    def decode(self, decoder_input, in_attention_hidden, in_attention_cell,
        ...
        decoder_iter_outputs = (decoder_output, gate_prediction, out_attention_hidden,
...
def main():
    parser = argparse.ArgumentParser(description='PyTorch Tacotron 2 export to TRT')
    ...
    traced_encoder = torch.jit.trace(encoder.eval(), dummy_input)
    encoder_output_path = os.path.join(output_path, "traced_tacotron2dyn_encoder.ts") 
    traced_encoder.save(encoder_output_path)
    ...
    traced_decoder = torch.jit.trace(decoder_iter.eval(), dummy_input) 
    decoder_output_path = os.path.join(output_path, "traced_tacotron2dyn_decoder.ts")
    traced_decoder.save(decoder_output_path)
    ...
    traced_posnet = torch.jit.trace(postnet.eval(), dummy_input)  
    posnet_output_path = os.path.join(output_path, "traced_tacotron2dyn_posnet.ts")
    traced_posnet.save(posnet_output_path)
