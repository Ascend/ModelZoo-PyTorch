...
def export_torch_aie(model_path, batch_size, soc_version, save_path="./"):
    ...
    trace_model.eval()
    input_info = [torch_aie.Input((batch_size, 3, 800, 1216))]
    ...
    print("[SOLOV2_INFO] torch_aie compile for SOLOv2 finished, model saved in: ", os.path.join(save_path, saved_name))


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--torch-script-path', type=str, default='./solov2_torchscript.pt', help='trace model path')
...
def main():
    print("[SOLOV2_INFO] torch_aie compile for SOLOv2 start")
    opt_args = parse_opt()
