...
AIE_MODEL_DEFAULT_NAME = 'aie_model.pt'
...
def main():
    ...
    Eval(experiment, experiment_args, cmd=args, verbose=args['verbose']).eval(args['trace_compile'])
...
def compile_aie_model(torch_model, batch_size):
    ...
    aie_model.save(AIE_MODEL_DEFAULT_NAME)
    return aie_model
...
class Eval:
    ...
    def eval(self, trace_compile=True):
        ...
        if trace_compile:
            aie_model = compile_aie_model(model, 1)
        else:
            aie_model = torch.jit.load(AIE_MODEL_DEFAULT_NAME)
            ...
            for _, data_loader in self.data_loaders.items():
                ...
                for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
                    if self.args['test_speed']:
                        time_cost = self.report_speed(aie_model, batch, times=50)
                        continue
