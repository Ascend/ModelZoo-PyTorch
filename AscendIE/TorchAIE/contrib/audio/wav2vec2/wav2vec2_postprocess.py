...
def post_process(model_out_dir, index_token_map, batch_i_filename_map):
    ...
    for file in filenames:
        ...
        batch_i = os.path.basename(file).split('.')[0][7:]    #original : [0:-2],    ouput file name eg: output_batch_13
...
if __name__ == '__main__':
    ...
    batch_i_filename_map_path = os.path.join(data_path, "batch_i_filename_map_bs{}.json".format(batch_size))
    ...
    ground_truth_texts_path = os.path.join(data_path,  "ground_truth_texts.txt")
    ...
    print(f"Err: {wer}")
    print(f"Acc: {1 - wer}")
    if abs(1- wer - 0.97) < 0.001:
        print("Accuracy ok, with max permitted error 0.001.")
